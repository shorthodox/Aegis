...
**Symbol:** STORJ/USDT  |  **Generated:** 2026-06-07 16:39:38
...
> Measured precision is 30.6% — estimated 3.4pp below achievable ceiling of ~34.0%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  30.6%
...
Achievable precision       :  ~34.0%  (+3.4pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 112 holdout BUY signals fired
   - PASS — 112 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=112, buy_win_rate=71.4%
   - PASS — 112 trades, 71.4% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -23.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 30.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.2pp |
| OOF → Holdout gap | +53.1pp |
| thr_buy / thr_sell | 49.765 / 49.416 |
| Blocked signals | 0 (76 would-win / 66 would-lose) |

> ⚠ Gate is DESTROYING 23.2pp of precision. Blocked signals (53.8%) would have beaten gated (30.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (83.8%) exceeds holdout (30.6%) by 53.1pp.
...
**Symbol:** STORJ/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.6%  Sharpe=18.18
**Expected after fixes:** Precision≈36.4%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=59.5% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 57.6% | ⚠ |
| Dev OOF Precision | 83.8% | ✓ |
| Holdout Precision | 30.6% | ✗ |
| Holdout Coverage | 68.8% | ✓ |
| 95% CI Precision | [27.8%, 33.7%] | — |
| OOF→Holdout Gap | -53.1% | ⚠ degradation |
| Holdout Fired | 920 trades | ✓ |
| SELL Win Rate | 77.4% (261 trades) | ✓ |
| BUY Win Rate | 71.4% (112 trades) | ✓ |
| Sharpe (annualised) | 18.18 | ✓ |
| Max Drawdown | 4846928848.41% | ✗ |
| Profit Factor | 2.36 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.3391% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-11.74) | ✓ significant |

### Class Distribution
- HOLD: **59.5%** — ⚠ severe imbalance
- SELL: **24.0%**
- BUY:  **16.6%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 59.5% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 142 | 76 | 66 | 53.8% | +97.71% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0719 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1637 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 30.7% | -0.44 | ⚠ overconfident |
| 80-90% | 40.7% | -0.44 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 462 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.164 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +53.11pp |
...
| Holdout Sharpe | 18.18 | ✓ |
| Holdout Max DD | 4846928848.41% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=59.5% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 30.6%  →  **Expected precision (all fixes):** 36.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 920 |
| Rejected signals | 417 |
| Selected precision | 30.6% |
| Rejected precision | 18.5% |
| Meta gate lift (precision) | +12.2% |
| Selected expectancy | +0.339% |
| Rejected expectancy | +0.015% |
| Selected Sharpe | +19.83 |
| Rejected Sharpe | +1.30 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 49.8 | 49.4 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep STORJ/USDT vs BTC/ETH Comparison
