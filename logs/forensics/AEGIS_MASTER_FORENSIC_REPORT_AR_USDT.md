...
**Symbol:** AR/USDT  |  **Generated:** 2026-06-07 16:35:48
...
> Measured precision is 32.4% — estimated 3.5pp below achievable ceiling of ~35.9%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.1pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  32.4%
...
Achievable precision       :  ~35.9%  (+3.5pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 110 holdout BUY signals fired
   - PASS — 110 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=110, buy_win_rate=67.3%
   - PASS — 110 trades, 67.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -21.4pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 32.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -21.4pp |
| OOF → Holdout gap | +46.7pp |
| thr_buy / thr_sell | 46.604 / 46.573 |
| Blocked signals | 0 (150 would-win / 129 would-lose) |

> ⚠ Gate is DESTROYING 21.4pp of precision. Blocked signals (53.8%) would have beaten gated (32.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (79.1%) exceeds holdout (32.4%) by 46.7pp.
...
**Symbol:** AR/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=32.4%  Sharpe=13.56
**Expected after fixes:** Precision≈38.2%  (+5.8pp)
...
| Dev OOF Precision | 79.1% | ✓ |
| Holdout Precision | 32.4% | ✗ |
| Holdout Coverage | 37.7% | ✓ |
| 95% CI Precision | [28.5%, 36.6%] | — |
| OOF→Holdout Gap | -46.7% | ⚠ degradation |
| Holdout Fired | 503 trades | ✓ |
| SELL Win Rate | 76.7% (116 trades) | ✓ |
| BUY Win Rate | 67.3% (110 trades) | ✓ |
| Sharpe (annualised) | 13.56 | ✓ |
| Max Drawdown | 2961401693.45% | ✗ |
| Profit Factor | 2.07 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.5213% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-7.89) | ✓ significant |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0594 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1118 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 32.4% | -0.43 | ⚠ overconfident |
| 80-90% | 42.4% | -0.43 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 254 dev samples):** `isotonic`
...
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +46.73pp |
...
| Holdout Sharpe | 13.56 | ✓ |
| Holdout Max DD | 2961401693.45% | ✗ |
...
**Base precision:** 32.4%  →  **Expected precision (all fixes):** 38.2%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 503 |
| Rejected signals | 831 |
| Selected precision | 32.4% |
| Rejected precision | 26.6% |
| Meta gate lift (precision) | +5.8% |
| Selected expectancy | +0.521% |
| Rejected expectancy | +0.191% |
| Selected Sharpe | +20.01 |
| Rejected Sharpe | +10.66 |
...
## Section 19 — Deep AR/USDT vs BTC/ETH Comparison
