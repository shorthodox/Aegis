...
**Symbol:** PEPE/USDT  |  **Generated:** 2026-06-07 16:39:31
...
> Measured precision is 26.0% — estimated 7.7pp below achievable ceiling of ~33.7%. Primary contributors: Barrier skew suppresses BUY labels (−2.5pp), HOLD over-representation in labels (−2.8pp)

#### Precision Waterfall

```
Measured holdout precision :  26.0%

  [✅ FIXED       ]  −2.5pp prec    Barrier skew suppresses BUY labels
  [✅ FIXED       ]  −2.8pp prec    HOLD over-representation in labels
...
Achievable precision       :  ~33.7%  (+7.7pp precision)
Recall deficit (BUY side)  :  −11.6pp  (signals not firing)
```
...
✅ **3 issues already applied** in the codebase (expected gain: +8.8pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 137 BUY proposals / 285 total directional
   - PASS — primary fires BUY on some bars.

✅ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(109)=38 ≥ min_fires=35`
   - Value: 38 max fires vs 35 required
   - PASS — 38 ≥ 35.
...
🔴 **Gate status: HURTING**  (lift: -27.8pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 26.0% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -27.8pp |
| OOF → Holdout gap | +49.0pp |
| thr_buy / thr_sell | 73.180 / 46.361 |
| Blocked signals | 0 (144 would-win / 125 would-lose) |

> ⚠ Gate is DESTROYING 27.8pp of precision. Blocked signals (53.8%) would have beaten gated (26.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (75.0%) exceeds holdout (26.0%) by 49.0pp.
...
**Symbol:** PEPE/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=26.0%  Sharpe=4.38
**Expected after fixes:** Precision≈31.8%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 68/100
   > HOLD=68.9% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 61.9% | ⚠ |
| Dev OOF Precision | 26.0% | ✗ |
| Holdout Precision | 26.0% | ✗ |
| Holdout Coverage | 12.9% | ✓ |
| 95% CI Precision | [20.1%, 33.0%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 173 trades | ✓ |
| SELL Win Rate | 68.2% (66 trades) | ✓ |
| BUY Win Rate | 0.0% (0 trades) | ✗ disabled |
| Sharpe (annualised) | 4.38 | ✓ |
| Max Drawdown | 9530488958.28% | ✗ |
| Profit Factor | 1.42 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0910% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.31) | ✓ significant |

### Class Distribution
- HOLD: **68.9%** — ⚠ severe imbalance
- SELL: **19.5%**
- BUY:  **11.6%** — ⚠ minority class

### Issues Detected
- **CRITICAL** — BUY side fully disabled (buy_n=0). Model is SELL-only. Directional asymmetry score: 80/100.
- **WARNING** — Class imbalance: 68.9% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       285  (100%)
Blocked by Meta Gate:      -  269  (94%)
Blocked by Quality (<55):  -   22
Blocked by HMM:            -   14
Blocked by Confluence:     -   11
Blocked by Fake Breakout:  -    8
...
Average hold-signal realized return: **2.1606%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 269 | 144 | 125 | 53.8% | +581.19% |
| quality | 22 | 13 | 9 | 61.5% | +47.53% ⚠ |
| hmm | 14 | 8 | 6 | 57.6% | +30.25% ⚠ |
| confluence | 11 | 6 | 5 | 54.5% | +23.77% |
| fake_breakout | 8 | 4 | 4 | 60.0% | +17.28% ⚠ |
| portfolio | 5 | 2 | 3 | 57.6% | +10.80% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0464 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1254 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 26.0% | -0.49 | ⚠ overconfident |
| 80-90% | 36.0% | -0.49 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 28 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.125 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +48.99pp |
...
**3 root causes identified.**  Combined top-5 impact score: **218/500**
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 68/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=68.9% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 26.0%  →  **Expected precision (all fixes):** 31.8%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 173 |
| Rejected signals | 1169 |
| Selected precision | 26.0% |
| Rejected precision | 19.7% |
| Meta gate lift (precision) | +6.3% |
| Selected expectancy | +0.091% |
| Rejected expectancy | +0.194% |
| Selected Sharpe | +11.02 |
| Rejected Sharpe | +17.47 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 100.0 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep PEPE/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 56.8% |
| Actual Precision | 26.0% |
| Gap | -30.8% |
| Coverage | 1.8% |
