...
**Symbol:** CHZ/USDT  |  **Generated:** 2026-06-07 16:35:55
...
> Measured precision is 30.0% — estimated 7.8pp below achievable ceiling of ~37.8%. Primary contributors: HOLD over-representation in labels (−3.0pp), Barrier skew suppresses BUY labels (−2.4pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  30.0%
...
Achievable precision       :  ~37.8%  (+7.8pp precision)
Recall deficit (BUY side)  :  −3.6pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +6.5pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 11 holdout BUY signals fired
   - PASS — 11 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=11, buy_win_rate=81.8%
   - PASS — 11 trades, 81.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -23.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 30.0% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.9pp |
| OOF → Holdout gap | +51.0pp |
| thr_buy / thr_sell | 48.684 / 48.590 |
| Blocked signals | 0 (208 would-win / 180 would-lose) |

> ⚠ Gate is DESTROYING 23.9pp of precision. Blocked signals (53.8%) would have beaten gated (30.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (81.0%) exceeds holdout (30.0%) by 51.0pp.
...
**Symbol:** CHZ/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.0%  Sharpe=4.39
**Expected after fixes:** Precision≈35.8%  (+5.8pp)

### Top 5 Problems

1. 🔴 **Severe Class Imbalance (HOLD dominates)** — Score: 70/100
   > HOLD=70.3% of labels. Meta model sees 60% zero-labels → calibration distorted.

2. 🟡 **HMM Regime Collapse** — Score: 62/100
   > Max state concentration=100.0%. HMM assigning most bars to one state.

3. 🟡 **Low Holdout Sample Size** — Score: 20/100
   > Only 40 holdout signals. 95% CI=[18.1%, 45.4%] (width=27.4%).
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 61.1% | ⚠ |
| Dev OOF Precision | 81.0% | ✓ |
| Holdout Precision | 30.0% | ✗ |
| Holdout Coverage | 3.0% | ✓ |
| 95% CI Precision | [18.1%, 45.4%] | — |
| OOF→Holdout Gap | -51.0% | ⚠ degradation |
| Holdout Fired | 40 trades | ⚠ low sample |
| SELL Win Rate | 75.0% (4 trades) | ✓ |
| BUY Win Rate | 81.8% (11 trades) | ✓ |
| Sharpe (annualised) | 4.39 | ✓ |
| Max Drawdown | 800000000.00% | ✗ |
| Profit Factor | 2.12 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1966% | ✓ |
| Meta gate optimizer profile | present | ✓ |
| Optimizer-selected gate | EDGE_STRICT_VETO | ✓ |
| Optimizer threshold match | YES | ✓ |
| Meta gate summary count | 89 symbols | ✓ |
| Statistical Sig. | p=0.0114 (z=-2.53) | ✓ significant |

### Class Distribution
- HOLD: **70.3%** — ⚠ severe imbalance
- SELL: **17.8%**
- BUY:  **11.9%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 70.3% HOLD labels biases model toward neutrality.
- **WARNING** — Only 40 holdout signals. 95% CI spans 27.4%. High variance in precision estimate.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 388 | 208 | 180 | 53.8% | +266.98% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0549 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.8491 | ~1.0 | ✓ |
...
| 70-80% | 30.0% | -0.45 | ⚠ overconfident |
| 80-90% | 40.0% | -0.45 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 58 dev samples):** `temperature`
...
| Confidence Drift | 🟢 OK | T=0.849 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +51.03pp |
...
**3 root causes identified.**  Combined top-5 impact score: **152/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🔴 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 70/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=70.3% of labels. Meta model sees 60% zero-labels → calibration di… |
| 2 | 🟡 **HMM Regime Collapse** | HMM Failure | 62/100 | — 🔴 ACTIVE | Max state concentration=100.0%. HMM assigning most bars to one state.… |
| 3 | 🟢 **Low Holdout Sample Size** | Statistical Reliability | 20/100 | `scripts/retrain_model.py:1632 (train_token entry)` ✅ FIXED | Only 40 holdout signals. 95% CI=[18.1%, 45.4%] (width=27.4%).… |
...
**Base precision:** 30.0%  →  **Expected precision (all fixes):** 35.8%  (+5.8pp)
...
| Selected precision | 30.0% |
| Rejected precision | 19.0% |
| Meta gate lift (precision) | +11.0% |
| Selected expectancy | +0.197% |
| Rejected expectancy | +0.151% |
| Selected Sharpe | +22.96 |
| Rejected Sharpe | +10.04 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 48.7 | 48.6 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep CHZ/USDT vs BTC/ETH Comparison
...
| Actual Precision | 30.0% |
| Gap | -27.0% |
| Coverage | 3.9% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 61/100 |
