...
**Symbol:** JUP/USDT  |  **Generated:** 2026-06-07 16:39:03
...
> Measured precision is 36.6% — estimated 6.8pp below achievable ceiling of ~43.4%. Primary contributors: HOLD over-representation in labels (−2.4pp), Barrier skew suppresses BUY labels (−2.0pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  36.6%
...
Achievable precision       :  ~43.5%  (+6.8pp precision)
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 271 BUY proposals / 308 total directional
   - PASS — primary fires BUY on some bars.

✅ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(216)=75 ≥ min_fires=35`
   - Value: 75 max fires vs 35 required
   - PASS — 75 ≥ 35.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 36 holdout BUY signals fired
   - PASS — 36 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=36, buy_win_rate=91.7%
   - PASS — 36 trades, 91.7% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -17.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 36.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -17.2pp |
| OOF → Holdout gap | +49.5pp |
| thr_buy / thr_sell | 60.417 / 60.051 |
| Blocked signals | 0 (78 would-win / 68 would-lose) |

> ⚠ Gate is DESTROYING 17.2pp of precision. Blocked signals (53.8%) would have beaten gated (36.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (86.2%) exceeds holdout (36.6%) by 49.5pp.
...
**Symbol:** JUP/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=36.6%  Sharpe=27.04
**Expected after fixes:** Precision≈42.4%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 66/100
   > HOLD=66.0% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 59.9% | ⚠ |
| Dev OOF Precision | 86.2% | ✓ |
| Holdout Precision | 36.6% | ✗ |
| Holdout Coverage | 33.1% | ✓ |
| 95% CI Precision | [32.3%, 41.2%] | — |
| OOF→Holdout Gap | -49.5% | ⚠ degradation |
| Holdout Fired | 442 trades | ✓ |
| SELL Win Rate | 89.0% (145 trades) | ✓ |
| BUY Win Rate | 91.7% (36 trades) | ✓ |
| Sharpe (annualised) | 27.04 | ✓ |
| Max Drawdown | 200000000.00% | ✗ |
| Profit Factor | 8.50 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +1.4679% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.61) | ✓ significant |

### Class Distribution
- HOLD: **66.0%** — ⚠ severe imbalance
- SELL: **20.8%**
- BUY:  **13.2%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 66.0% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       308  (100%)
Blocked by Meta Gate:      -  146  (47%)
Blocked by Quality (<55):  -   24
Blocked by HMM:            -   15
Blocked by Confluence:     -   12
Blocked by Fake Breakout:  -    9
Blocked by Portfolio Guard:-    6
Blocked by Safe Mode:      -    3
Blocked by Drift:          -    6
Blocked by Cooldown:       -    3
─────────────────────────────────────
Estimated Executed:            84  (27.3%)
```
...
Average hold-signal realized return: **2.0331%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 146 | 78 | 68 | 53.8% | +296.84% |
| quality | 24 | 14 | 10 | 61.5% | +48.80% ⚠ |
| hmm | 15 | 8 | 7 | 57.6% | +30.50% ⚠ |
| confluence | 12 | 6 | 6 | 54.5% | +24.40% |
| fake_breakout | 9 | 5 | 4 | 60.0% | +18.30% ⚠ |
| portfolio | 6 | 3 | 3 | 57.6% | +12.20% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0537 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1883 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 36.7% | -0.38 | ⚠ overconfident |
| 80-90% | 46.7% | -0.38 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 297 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.188 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +49.54pp |
...
| Holdout Sharpe | 27.04 | ✓ |
| Holdout Max DD | 200000000.00% | ✗ |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 66/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=66.0% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 36.6%  →  **Expected precision (all fixes):** 42.4%  (+5.8pp)
...
| Rejected signals | 892 |
| Selected precision | 36.6% |
| Rejected precision | 22.5% |
| Meta gate lift (precision) | +14.1% |
| Selected expectancy | +1.468% |
| Rejected expectancy | +0.401% |
| Selected Sharpe | +42.57 |
| Rejected Sharpe | +15.33 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 60.4 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep JUP/USDT vs BTC/ETH Comparison
