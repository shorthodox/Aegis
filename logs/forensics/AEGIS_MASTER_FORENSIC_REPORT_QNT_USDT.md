...
**Symbol:** QNT/USDT  |  **Generated:** 2026-06-07 16:39:32
...
> Measured precision is 36.4% — estimated 4.4pp below achievable ceiling of ~40.8%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Temperature T=1.206 — overconfidence (−1.0pp)

#### Precision Waterfall

```
Measured holdout precision :  36.4%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −1.0pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.206 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~40.8%  (+4.4pp precision)
...
| Temperature T=1.206 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.206 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 25 holdout BUY signals fired
   - PASS — 25 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=25, buy_win_rate=92.0%
   - PASS — 25 trades, 92.0% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -17.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 36.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -17.5pp |
| OOF → Holdout gap | +59.5pp |
| thr_buy / thr_sell | 80.206 / 80.137 |
| Blocked signals | 0 (163 would-win / 140 would-lose) |

> ⚠ Gate is DESTROYING 17.5pp of precision. Blocked signals (53.8%) would have beaten gated (36.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (95.9%) exceeds holdout (36.4%) by 59.5pp.
...
**Symbol:** QNT/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=36.4%  Sharpe=19.41
**Expected after fixes:** Precision≈42.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
   > HOLD=57.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 95.9% | ✓ |
| Holdout Precision | 36.4% | ✗ |
| Holdout Coverage | 25.3% | ✓ |
| 95% CI Precision | [31.4%, 41.6%] | — |
| OOF→Holdout Gap | -59.5% | ⚠ degradation |
| Holdout Fired | 338 trades | ✓ |
| SELL Win Rate | 84.8% (118 trades) | ✓ |
| BUY Win Rate | 92.0% (25 trades) | ✓ |
| Sharpe (annualised) | 19.41 | ✓ |
| Max Drawdown | 2248362977.15% | ✗ |
| Profit Factor | 3.80 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4671% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.00) | ✓ significant |

### Class Distribution
- HOLD: **57.4%** — ⚠ severe imbalance
- SELL: **26.0%**
- BUY:  **16.7%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 57.4% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 303 | 163 | 140 | 53.8% | +208.49% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0867 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2058 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 36.4% | -0.39 | ⚠ overconfident |
| 80-90% | 46.4% | -0.39 | ⚠ overconfident |
...
**States:** 7  |  **Global precision:** 66.0%  |  **Max state concentration:** 44.6%
...
| Confidence Drift | 🟡 WARNING | T=1.206 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +59.50pp |
...
| Holdout Sharpe | 19.41 | ✓ |
| Holdout Max DD | 2248362977.15% | ✗ |
...
**1 root causes identified.**  Combined top-5 impact score: **57/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 36.4%  →  **Expected precision (all fixes):** 42.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 338 |
| Rejected signals | 996 |
| Selected precision | 36.4% |
| Rejected precision | 28.0% |
| Meta gate lift (precision) | +8.4% |
| Selected expectancy | +0.467% |
| Rejected expectancy | +0.061% |
| Selected Sharpe | +34.94 |
| Rejected Sharpe | +5.49 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 80.2 | 80.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep QNT/USDT vs BTC/ETH Comparison
...
| Actual Precision | 36.4% |
| Gap | -20.8% |
| Coverage | 10.4% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 58/100 |
