...
**Symbol:** COMP/USDT  |  **Generated:** 2026-06-07 16:35:55
...
> Measured precision is 29.8% — estimated 6.9pp below achievable ceiling of ~36.7%. Primary contributors: Barrier skew suppresses BUY labels (−2.1pp), HOLD over-representation in labels (−2.4pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  29.8%
...
Achievable precision       :  ~36.7%  (+6.9pp precision)
Recall deficit (BUY side)  :  −3.1pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +5.4pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 71 holdout BUY signals fired
   - PASS — 71 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=71, buy_win_rate=71.8%
   - PASS — 71 trades, 71.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -24.0pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 29.8% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -24.0pp |
| OOF → Holdout gap | +54.1pp |
| thr_buy / thr_sell | 46.699 / 46.699 |
| Blocked signals | 0 (77 would-win / 66 would-lose) |

> ⚠ Gate is DESTROYING 24.0pp of precision. Blocked signals (53.8%) would have beaten gated (29.8%). Meta model is anti-selective. | OOF overfit warning: dev_prec (83.9%) exceeds holdout (29.8%) by 54.1pp.
...
**Symbol:** COMP/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=29.8%  Sharpe=17.89
**Expected after fixes:** Precision≈35.6%  (+5.8pp)
...
| Dev OOF Precision | 83.9% | ✓ |
| Holdout Precision | 29.8% | ✗ |
| Holdout Coverage | 57.8% | ✓ |
| 95% CI Precision | [26.7%, 33.1%] | — |
| OOF→Holdout Gap | -54.1% | ⚠ degradation |
| Holdout Fired | 775 trades | ✓ |
| SELL Win Rate | 79.3% (227 trades) | ✓ |
| BUY Win Rate | 71.8% (71 trades) | ✓ |
| Sharpe (annualised) | 17.89 | ✓ |
| Max Drawdown | 11472516389.41% | ✗ |
| Profit Factor | 2.24 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.3186% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-11.24) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 143 | 77 | 66 | 53.8% | +98.40% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.1072 | <0.10 | ✗ overcalibrated |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.8784 | ~1.0 | ✓ |
...
| 70-80% | 29.8% | -0.45 | ⚠ overconfident |
| 80-90% | 39.8% | -0.45 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 366 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=0.878 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +54.07pp |
...
| Holdout Sharpe | 17.89 | ✓ |
| Holdout Max DD | 11472516389.41% | ✗ |
...
**Base precision:** 29.8%  →  **Expected precision (all fixes):** 35.6%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 775 |
| Rejected signals | 565 |
| Selected precision | 29.8% |
| Rejected precision | 29.9% |
| Meta gate lift (precision) | -0.1% |
| Selected expectancy | +0.319% |
| Rejected expectancy | +0.181% |
| Selected Sharpe | +21.27 |
| Rejected Sharpe | +19.17 |
...
## Section 19 — Deep COMP/USDT vs BTC/ETH Comparison
