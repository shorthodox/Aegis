...
**Symbol:** AXS/USDT  |  **Generated:** 2026-06-07 16:35:51
...
> Measured precision is 40.2% — estimated 4.1pp below achievable ceiling of ~44.3%. Primary contributors: Brier score above target (−2.0pp), Temperature T=1.210 — overconfidence (−1.0pp), Barrier skew suppresses BUY labels (−0.7pp)

#### Precision Waterfall

```
Measured holdout precision :  40.2%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.210 — overconfidence
  [✅ FIXED       ]  −0.7pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~44.3%  (+4.1pp precision)
...
| Temperature T=1.210 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.210 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 77 holdout BUY signals fired
   - PASS — 77 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=77, buy_win_rate=94.8%
   - PASS — 77 trades, 94.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -13.7pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 40.2% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -13.7pp |
| OOF → Holdout gap | +49.2pp |
| thr_buy / thr_sell | 70.172 / 70.149 |
| Blocked signals | 0 (150 would-win / 129 would-lose) |

> ⚠ Gate is DESTROYING 13.7pp of precision. Blocked signals (53.8%) would have beaten gated (40.2%). Meta model is anti-selective. | OOF overfit warning: dev_prec (89.4%) exceeds holdout (40.2%) by 49.2pp.
...
**Symbol:** AXS/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=40.2%  Sharpe=26.74
**Expected after fixes:** Precision≈45.9%  (+5.8pp)
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 63.3% | ⚠ |
| Dev OOF Precision | 89.4% | ✓ |
| Holdout Precision | 40.2% | ✗ |
| Holdout Coverage | 32.1% | ✓ |
| 95% CI Precision | [35.6%, 44.9%] | — |
| OOF→Holdout Gap | -49.2% | ⚠ degradation |
| Holdout Fired | 428 trades | ✓ |
| SELL Win Rate | 87.6% (113 trades) | ✓ |
| BUY Win Rate | 94.8% (77 trades) | ✓ |
| Sharpe (annualised) | 26.74 | ✓ |
| Max Drawdown | 78.65% | ✗ |
| Profit Factor | 5.66 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.6126% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-4.06) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 279 | 150 | 129 | 53.8% | +191.98% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0767 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2103 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 40.2% | -0.35 | ⚠ overconfident |
| 80-90% | 50.2% | -0.35 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 273 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.210 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +49.19pp |
...
| Holdout Sharpe | 26.74 | ✓ |
| Holdout Max DD | 78.65% | ✗ |
...
**Base precision:** 40.2%  →  **Expected precision (all fixes):** 45.9%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 428 |
| Rejected signals | 906 |
| Selected precision | 40.2% |
| Rejected precision | 26.2% |
| Meta gate lift (precision) | +14.0% |
| Selected expectancy | +0.613% |
| Rejected expectancy | +0.167% |
| Selected Sharpe | +42.77 |
| Rejected Sharpe | +11.44 |
...
## Section 19 — Deep AXS/USDT vs BTC/ETH Comparison
