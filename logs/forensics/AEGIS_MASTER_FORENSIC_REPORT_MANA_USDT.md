...
**Symbol:** MANA/USDT  |  **Generated:** 2026-06-07 16:39:27
...
> Measured precision is 29.3% — estimated 5.6pp below achievable ceiling of ~34.9%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.5pp), HOLD over-representation in labels (−1.7pp)
...
Achievable precision       :  ~34.9%  (+5.6pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=57, buy_win_rate=66.7%
   - PASS — 57 trades, 66.7% WR.
...
| OOF → Holdout gap | +51.7pp |
| thr_buy / thr_sell | 46.667 / 46.667 |
| Blocked signals | 0 (151 would-win / 131 would-lose) |

> ⚠ Gate is DESTROYING 24.6pp of precision. Blocked signals (53.8%) would have beaten gated (29.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (81.0%) exceeds holdout (29.3%) by 51.7pp.
...
**Symbol:** MANA/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=29.3%  Sharpe=9.66
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 61/100
   > HOLD=61.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 58.7% | ⚠ |
| Dev OOF Precision | 81.0% | ✓ |
| Holdout Precision | 29.3% | ✗ |
| Holdout Coverage | 34.4% | ✓ |
| 95% CI Precision | [25.3%, 33.6%] | — |
| OOF→Holdout Gap | -51.7% | ⚠ degradation |
| Holdout Fired | 461 trades | ✓ |
| SELL Win Rate | 74.6% (130 trades) | ✓ |
| BUY Win Rate | 66.7% (57 trades) | ✓ |
| Sharpe (annualised) | 9.66 | ✓ |
| Max Drawdown | 492.30% | ✗ |
| Profit Factor | 1.61 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1337% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-8.90) | ✓ significant |

### Class Distribution
- HOLD: **61.4%** — ⚠ severe imbalance
- SELL: **23.7%**
- BUY:  **14.9%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 61.4% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 282 | 151 | 131 | 53.8% | +194.04% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0655 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0033 | ~1.0 | ✓ |
...
**Recommended calibrator (for 231 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.003 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +51.67pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 61/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=61.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
| Metric | Value |
|--------|-------|
| Selected signals | 461 |
| Rejected signals | 878 |
| Selected precision | 29.3% |
| Rejected precision | 22.7% |
| Meta gate lift (precision) | +6.6% |
| Selected expectancy | +0.134% |
| Rejected expectancy | +0.193% |
| Selected Sharpe | +14.89 |
| Rejected Sharpe | +14.61 |
...
## Section 19 — Deep MANA/USDT vs BTC/ETH Comparison
