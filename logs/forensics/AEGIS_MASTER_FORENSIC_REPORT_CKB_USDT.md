...
**Symbol:** CKB/USDT  |  **Generated:** 2026-06-07 16:35:55
...
> Measured precision is 30.9% — estimated 3.2pp below achievable ceiling of ~34.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.8pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  30.9%
...
Achievable precision       :  ~34.1%  (+3.2pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=15, buy_win_rate=86.7%
   - PASS — 15 trades, 86.7% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -22.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 30.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -22.9pp |
| OOF → Holdout gap | +69.0pp |
| thr_buy / thr_sell | 85.086 / 85.086 |
| Blocked signals | 0 (235 would-win / 203 would-lose) |

> ⚠ Gate is DESTROYING 22.9pp of precision. Blocked signals (53.8%) would have beaten gated (30.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (100.0%) exceeds holdout (30.9%) by 69.0pp.
...
**Symbol:** CKB/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.9%  Sharpe=5.59
**Expected after fixes:** Precision≈36.7%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 56/100
   > HOLD=56.8% of labels. Meta model sees 60% zero-labels → calibration distorted.

3. 🟡 **Low Holdout Sample Size** — Score: 18/100
   > Only 42 holdout signals. 95% CI=[19.1%, 46.0%] (width=27.0%).
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 54.8% | ⚠ |
| Dev OOF Precision | 30.9% | ✗ |
| Holdout Precision | 30.9% | ✗ |
| Holdout Coverage | 3.1% | ✓ |
| 95% CI Precision | [19.1%, 46.0%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 42 trades | ⚠ low sample |
| SELL Win Rate | 0.0% (0 trades) | ✗ |
| BUY Win Rate | 86.7% (15 trades) | ✓ |
| Sharpe (annualised) | 5.59 | ✓ |
| Max Drawdown | 32.42% | ✗ |
| Profit Factor | 2.60 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2219% | ✓ |
...
| Statistical Sig. | p=0.0136 (z=-2.47) | ✓ significant |

### Class Distribution
- HOLD: **56.8%** — ⚠ severe imbalance
- SELL: **25.9%**
- BUY:  **17.3%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 56.8% HOLD labels biases model toward neutrality.
- **WARNING** — Only 42 holdout signals. 95% CI spans 27.0%. High variance in precision estimate.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 438 | 235 | 203 | 53.8% | +301.38% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0625 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0107 | ~1.0 | ✓ |
...
| 70-80% | 31.0% | -0.44 | ⚠ overconfident |
| 80-90% | 41.0% | -0.44 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 2 dev samples):** `temperature`
...
| Confidence Drift | 🟢 OK | T=1.011 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +69.05pp |
...
**3 root causes identified.**  Combined top-5 impact score: **136/500**
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 56/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=56.8% of labels. Meta model sees 60% zero-labels → calibration di… |
| 3 | 🟢 **Low Holdout Sample Size** | Statistical Reliability | 18/100 | `scripts/retrain_model.py:1632 (train_token entry)` ✅ FIXED | Only 42 holdout signals. 95% CI=[19.1%, 46.0%] (width=27.0%).… |
...
**3. Low Holdout Sample Size**
> 📍 `scripts/retrain_model.py:1632 (train_token entry)` — `train_token() — no random.seed / np.random.seed at`
> Use 8760h of data. N_SPLITS_CV→15. Walk-forward validation.
...
**Base precision:** 30.9%  →  **Expected precision (all fixes):** 36.7%  (+5.8pp)
...
|--------|-------|
| Selected signals | 42 |
| Rejected signals | 1292 |
| Selected precision | 30.9% |
| Rejected precision | 34.8% |
| Meta gate lift (precision) | -3.8% |
| Selected expectancy | +0.222% |
| Rejected expectancy | +0.426% |
| Selected Sharpe | +28.52 |
| Rejected Sharpe | +30.99 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 85.1 | 85.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep CKB/USDT vs BTC/ETH Comparison
