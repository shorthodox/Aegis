...
**Symbol:** HBAR/USDT  |  **Generated:** 2026-06-07 16:36:04
...
> Measured precision is 28.5% — estimated 3.6pp below achievable ceiling of ~32.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.2pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  28.5%
...
Achievable precision       :  ~32.1%  (+3.6pp precision)
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 23 BUY proposals / 68 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(18)=6 ≥ min_fires=35`
   - Value: 6 max fires vs 35 required
   - FAIL (FIXED) — 6 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=38, buy_win_rate=55.3%
   - PASS — 38 trades, 55.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -25.4pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 28.5% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -25.4pp |
| OOF → Holdout gap | +55.5pp |
| thr_buy / thr_sell | 46.667 / 46.667 |
| Blocked signals | 0 (26 would-win / 23 would-lose) |

> ⚠ Gate is DESTROYING 25.4pp of precision. Blocked signals (53.8%) would have beaten gated (28.5%). Meta model is anti-selective. | OOF overfit warning: dev_prec (84.0%) exceeds holdout (28.5%) by 55.5pp.
...
**Symbol:** HBAR/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=28.5%  Sharpe=1.04
**Expected after fixes:** Precision≈34.2%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 58/100
   > HOLD=58.8% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 60.6% | ⚠ |
| Dev OOF Precision | 84.0% | ✓ |
| Holdout Precision | 28.5% | ✗ |
| Holdout Coverage | 44.6% | ✓ |
| 95% CI Precision | [25.0%, 32.2%] | — |
| OOF→Holdout Gap | -55.5% | ⚠ degradation |
| Holdout Fired | 597 trades | ✓ |
| SELL Win Rate | 63.9% (233 trades) | ✓ |
| BUY Win Rate | 55.3% (38 trades) | ✓ |
| Sharpe (annualised) | 1.04 | ✓ |
| Max Drawdown | 1313.77% | ✗ |
| Profit Factor | 1.04 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0096% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-10.52) | ✓ significant |

### Class Distribution
- HOLD: **58.8%** — ⚠ severe imbalance
- SELL: **25.1%**
- BUY:  **16.1%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 58.8% HOLD labels biases model toward neutrality.
...
```
Generated (directional):        68  (100%)
Blocked by Meta Gate:      -   49  (72%)
...
Average hold-signal realized return: **1.6373%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 49 | 26 | 23 | 53.8% | +80.23% |
| quality | 5 | 3 | 2 | 61.5% | +8.19% ⚠ |
| hmm | 3 | 1 | 2 | 57.6% | +4.91% ⚠ |
| confluence | 2 | 1 | 1 | 54.5% | +3.27% |
| fake_breakout | 2 | 1 | 1 | 60.0% | +3.27% ⚠ |
| portfolio | 1 | 0 | 1 | 57.6% | +1.64% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0714 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1695 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 28.5% | -0.47 | ⚠ overconfident |
| 80-90% | 38.5% | -0.47 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 187 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.169 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +55.48pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 58/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=58.8% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 28.5%  →  **Expected precision (all fixes):** 34.2%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 597 |
| Rejected signals | 742 |
| Selected precision | 28.5% |
| Rejected precision | 29.0% |
| Meta gate lift (precision) | -0.5% |
| Selected expectancy | +0.010% |
| Rejected expectancy | +0.246% |
| Selected Sharpe | +1.41 |
| Rejected Sharpe | +21.98 |
...
## Section 19 — Deep HBAR/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 57.5% |
| Actual Precision | 28.5% |
| Gap | -29.0% |
| Coverage | 9.2% |
