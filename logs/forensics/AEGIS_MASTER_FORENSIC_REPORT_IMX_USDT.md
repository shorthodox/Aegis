...
**Symbol:** IMX/USDT  |  **Generated:** 2026-06-07 16:36:25
...
> Measured precision is 39.2% — estimated 3.1pp below achievable ceiling of ~42.3%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.7pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  39.2%
...
Achievable precision       :  ~42.3%  (+3.1pp precision)
Recall deficit (BUY side)  :  −0.7pp  (signals not firing)
```
...
✅ **1 issues already applied** in the codebase (expected gain: +0.9pp once retrained).
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=27, buy_win_rate=70.4%
   - PASS — 27 trades, 70.4% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -14.6pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 39.2% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -14.6pp |
| OOF → Holdout gap | +56.5pp |
| thr_buy / thr_sell | 75.224 / 75.068 |
| Blocked signals | 0 (134 would-win / 115 would-lose) |

> ⚠ Gate is DESTROYING 14.6pp of precision. Blocked signals (53.8%) would have beaten gated (39.2%). Meta model is anti-selective. | OOF overfit warning: dev_prec (95.7%) exceeds holdout (39.2%) by 56.5pp.
...
**Symbol:** IMX/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=39.2%  Sharpe=27.61
**Expected after fixes:** Precision≈45.0%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
   > HOLD=57.6% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 57.0% | ⚠ |
| Dev OOF Precision | 95.7% | ✓ |
| Holdout Precision | 39.2% | ✗ |
| Holdout Coverage | 34.7% | ✓ |
| 95% CI Precision | [34.9%, 43.7%] | — |
| OOF→Holdout Gap | -56.5% | ⚠ degradation |
| Holdout Fired | 464 trades | ✓ |
| SELL Win Rate | 89.6% (182 trades) | ✓ |
| BUY Win Rate | 70.4% (27 trades) | ✓ |
| Sharpe (annualised) | 27.61 | ✓ |
| Max Drawdown | 26.54% | ✗ |
| Profit Factor | 5.19 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.7718% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-4.64) | ✓ significant |

### Class Distribution
- HOLD: **57.6%** — ⚠ severe imbalance
- SELL: **24.8%**
- BUY:  **17.6%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 57.6% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 249 | 134 | 115 | 53.8% | +171.33% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0536 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1605 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 39.2% | -0.36 | ⚠ overconfident |
| 80-90% | 49.2% | -0.36 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 305 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.161 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +56.51pp |
...
| Holdout Sharpe | 27.61 | ✓ |
| Holdout Max DD | 26.54% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.6% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 39.2%  →  **Expected precision (all fixes):** 45.0%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 464 |
| Rejected signals | 874 |
| Selected precision | 39.2% |
| Rejected precision | 26.2% |
| Meta gate lift (precision) | +13.0% |
| Selected expectancy | +0.772% |
| Rejected expectancy | +0.119% |
| Selected Sharpe | +42.42 |
| Rejected Sharpe | +9.57 |
...
## Section 19 — Deep IMX/USDT vs BTC/ETH Comparison
...
| Actual Precision | 39.2% |
| Gap | -17.6% |
| Coverage | 14.4% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 63/100 |
