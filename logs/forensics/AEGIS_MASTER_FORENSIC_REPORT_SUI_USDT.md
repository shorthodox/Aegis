...
**Symbol:** SUI/USDT  |  **Generated:** 2026-06-07 16:39:47
...
> Measured precision is 30.1% — estimated 5.1pp below achievable ceiling of ~35.2%. Primary contributors: Brier score above target (−2.0pp), HOLD over-representation in labels (−1.5pp), Barrier skew suppresses BUY labels (−1.2pp)

#### Precision Waterfall

```
Measured holdout precision :  30.1%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −1.5pp prec    HOLD over-representation in labels
  [✅ FIXED       ]  −1.2pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~35.2%  (+5.1pp precision)
Recall deficit (BUY side)  :  −1.8pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +3.2pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 103 BUY proposals / 264 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(82)=28 ≥ min_fires=35`
   - Value: 28 max fires vs 35 required
   - FAIL (FIXED) — 28 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 53 holdout BUY signals fired
   - PASS — 53 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=53, buy_win_rate=58.5%
   - PASS — 53 trades, 58.5% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -23.8pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 30.1% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.8pp |
| OOF → Holdout gap | +57.4pp |
| thr_buy / thr_sell | 60.209 / 60.116 |
| Blocked signals | 0 (129 would-win / 111 would-lose) |

> ⚠ Gate is DESTROYING 23.8pp of precision. Blocked signals (53.8%) would have beaten gated (30.1%). Meta model is anti-selective. | OOF overfit warning: dev_prec (87.5%) exceeds holdout (30.1%) by 57.4pp.
...
**Symbol:** SUI/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.1%  Sharpe=1.47
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 60/100
   > HOLD=60.3% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 87.5% | ✓ |
| Holdout Precision | 30.1% | ✗ |
| Holdout Coverage | 12.2% | ✓ |
| 95% CI Precision | [23.5%, 37.5%] | — |
| OOF→Holdout Gap | -57.4% | ⚠ degradation |
| Holdout Fired | 163 trades | ✓ |
| SELL Win Rate | 81.8% (22 trades) | ✓ |
| BUY Win Rate | 58.5% (53 trades) | ✓ |
| Sharpe (annualised) | 1.47 | ✓ |
| Max Drawdown | 240.29% | ✗ |
| Profit Factor | 1.13 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0320% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.09) | ✓ significant |

### Class Distribution
- HOLD: **60.3%** — ⚠ severe imbalance
- SELL: **23.7%**
- BUY:  **16.0%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 60.3% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       264  (100%)
Blocked by Meta Gate:      -  240  (91%)
Blocked by Quality (<55):  -   21
Blocked by HMM:            -   13
...
Average hold-signal realized return: **2.0390%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 240 | 129 | 111 | 53.8% | +489.37% |
| quality | 21 | 12 | 9 | 61.5% | +42.82% ⚠ |
| hmm | 13 | 7 | 6 | 57.6% | +26.51% ⚠ |
| confluence | 10 | 5 | 5 | 54.5% | +20.39% |
| fake_breakout | 7 | 4 | 3 | 60.0% | +14.27% ⚠ |
| portfolio | 5 | 2 | 3 | 57.6% | +10.20% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0545 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0446 | ~1.0 | ✓ |
...
| 70-80% | 30.1% | -0.45 | ⚠ overconfident |
| 80-90% | 40.1% | -0.45 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 56 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.045 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +57.44pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 60/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=60.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 30.1%  →  **Expected precision (all fixes):** 35.8%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 163 |
| Rejected signals | 1176 |
| Selected precision | 30.1% |
| Rejected precision | 27.7% |
| Meta gate lift (precision) | +2.3% |
| Selected expectancy | +0.032% |
| Rejected expectancy | +0.258% |
| Selected Sharpe | +3.82 |
| Rejected Sharpe | +17.03 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 60.2 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep SUI/USDT vs BTC/ETH Comparison
