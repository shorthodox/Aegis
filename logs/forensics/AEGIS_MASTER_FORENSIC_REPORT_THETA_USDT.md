...
**Symbol:** THETA/USDT  |  **Generated:** 2026-06-07 16:39:54
...
> Measured precision is 33.9% — estimated 4.4pp below achievable ceiling of ~38.3%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Temperature T=0.737 — overconfidence (−1.0pp)

#### Precision Waterfall

```
Measured holdout precision :  33.9%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −1.0pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=0.737 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~38.3%  (+4.4pp precision)
...
| Temperature T=0.737 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=0.737 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
❌ **1. Primary model generates BUY labels**
   ...
   - Value: 0 BUY proposals / 22 total directional
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 147 holdout BUY signals fired
   - PASS — 147 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=147, buy_win_rate=70.8%
   - PASS — 147 trades, 70.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -19.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 33.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -19.9pp |
| OOF → Holdout gap | +54.0pp |
| thr_buy / thr_sell | 46.475 / 46.475 |
| Blocked signals | 0 (4 would-win / 4 would-lose) |

> ⚠ Gate is DESTROYING 19.9pp of precision. Blocked signals (53.8%) would have beaten gated (33.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (87.9%) exceeds holdout (33.9%) by 54.0pp.
...
**Symbol:** THETA/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=33.9%  Sharpe=13.76
**Expected after fixes:** Precision≈39.7%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 55/100
   > HOLD=55.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 87.9% | ✓ |
| Holdout Precision | 33.9% | ✗ |
| Holdout Coverage | 49.5% | ✓ |
| 95% CI Precision | [30.4%, 37.6%] | — |
| OOF→Holdout Gap | -54.0% | ⚠ degradation |
| Holdout Fired | 660 trades | ✓ |
| SELL Win Rate | 69.8% (172 trades) | ✓ |
| BUY Win Rate | 70.8% (147 trades) | ✓ |
| Sharpe (annualised) | 13.76 | ✓ |
| Max Drawdown | 332.57% | ✗ |
| Profit Factor | 1.82 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2827% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-8.25) | ✓ significant |

### Class Distribution
- HOLD: **55.4%** — ⚠ severe imbalance
- SELL: **27.9%**
- BUY:  **16.8%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 55.4% HOLD labels biases model toward neutrality.
...
```
Generated (directional):        22  (100%)
Blocked by Meta Gate:      -    8  (36%)
Blocked by Quality (<55):  -    1
Blocked by HMM:            -    1
...
Average hold-signal realized return: **2.0575%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 8 | 4 | 4 | 53.8% | +16.46% |
| quality | 1 | 0 | 1 | 61.5% | +2.06% ⚠ |
| hmm | 1 | 0 | 1 | 57.6% | +2.06% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0377 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.7368 | ~1.0 | ✓ |
...
| 70-80% | 33.9% | -0.41 | ⚠ overconfident |
| 80-90% | 43.9% | -0.41 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 464 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=0.737 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +53.99pp |
...
| Holdout Sharpe | 13.76 | ✓ |
| Holdout Max DD | 332.57% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 55/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=55.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 33.9%  →  **Expected precision (all fixes):** 39.7%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 660 |
| Rejected signals | 674 |
| Selected precision | 33.9% |
| Rejected precision | 25.8% |
| Meta gate lift (precision) | +8.1% |
| Selected expectancy | +0.283% |
| Rejected expectancy | +0.497% |
| Selected Sharpe | +17.72 |
| Rejected Sharpe | +25.24 |
...
## Section 19 — Deep THETA/USDT vs BTC/ETH Comparison
