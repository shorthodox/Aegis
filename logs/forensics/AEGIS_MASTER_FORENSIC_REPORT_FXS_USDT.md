...
**Symbol:** FXS/USDT  |  **Generated:** 2026-06-07 16:36:02
...
> Measured precision is 30.4% — estimated 6.4pp below achievable ceiling of ~36.8%. Primary contributors: Barrier skew suppresses BUY labels (−2.3pp), Brier score above target (−2.0pp), HOLD over-representation in labels (−1.7pp)

#### Precision Waterfall

```
Measured holdout precision :  30.4%
...
Achievable precision       :  ~36.8%  (+6.4pp precision)
...
✅ **2 issues already applied** in the codebase (expected gain: +4.9pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 13 holdout BUY signals fired
   - PASS — 13 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=13, buy_win_rate=100.0%
   - PASS — 13 trades, 100.0% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -23.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 30.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.5pp |
| OOF → Holdout gap | +37.0pp |
| thr_buy / thr_sell | 97.833 / 97.147 |
| Blocked signals | 0 (166 would-win / 143 would-lose) |

> ⚠ Gate is DESTROYING 23.5pp of precision. Blocked signals (53.8%) would have beaten gated (30.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (67.4%) exceeds holdout (30.4%) by 37.0pp.
...
**Symbol:** FXS/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.4%  Sharpe=12.21
**Expected after fixes:** Precision≈36.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 61/100
   > HOLD=61.3% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 63.8% | ⚠ |
| Dev OOF Precision | 67.4% | ✓ |
| Holdout Precision | 30.4% | ✗ |
| Holdout Coverage | 12.3% | ✓ |
| 95% CI Precision | [21.3%, 41.2%] | — |
| OOF→Holdout Gap | -37.0% | ⚠ degradation |
| Holdout Fired | 79 trades | ✓ |
| SELL Win Rate | 100.0% (11 trades) | ✓ |
| BUY Win Rate | 100.0% (13 trades) | ✓ |
| Sharpe (annualised) | 12.21 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 15.07 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.9795% | ✓ |
| Meta gate optimizer profile | missing | ✗ |
| Optimizer-selected gate | None | ✗ |
| Optimizer threshold match | NO | ⚠ |
| Meta gate summary count | 89 symbols | ✓ |
| Statistical Sig. | p=0.0005 (z=-3.49) | ✓ significant |

### Class Distribution
- HOLD: **61.3%** — ⚠ severe imbalance
- SELL: **26.3%**
- BUY:  **12.4%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 61.3% HOLD labels biases model toward neutrality.
- **WARNING** — Meta gate optimizer profile is missing. Run `scripts/meta_gate_optimizer.py` to generate `data/meta_gate_profiles/<symbol>_gate.json`.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 309 | 166 | 143 | 53.8% | +212.62% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.1372 | <0.10 | ✗ overcalibrated |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0499 | ~1.0 | ✓ |
...
| 70-80% | 30.4% | -0.45 | ⚠ overconfident |
| 80-90% | 40.4% | -0.45 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 95 dev samples):** `platt`
...
**States:** 9  |  **Global precision:** 66.0%  |  **Max state concentration:** 44.6%
...
| Confidence Drift | 🟢 OK | T=1.050 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +36.99pp |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 61/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=61.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 30.4%  →  **Expected precision (all fixes):** 36.1%  (+5.8pp)
...
|--------|-------|
| Selected signals | 79 |
| Rejected signals | 562 |
| Selected precision | 30.4% |
| Rejected precision | 23.3% |
| Meta gate lift (precision) | +7.1% |
| Selected expectancy | +0.980% |
| Rejected expectancy | +0.167% |
| Selected Sharpe | +45.46 |
| Rejected Sharpe | +14.43 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 98.5 | 97.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep FXS/USDT vs BTC/ETH Comparison
