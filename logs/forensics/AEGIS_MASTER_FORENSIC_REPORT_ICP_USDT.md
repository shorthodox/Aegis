...
**Symbol:** ICP/USDT  |  **Generated:** 2026-06-07 16:36:10
...
> Measured precision is 42.6% — estimated 3.6pp below achievable ceiling of ~46.2%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.2pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  42.6%
...
Achievable precision       :  ~46.2%  (+3.6pp precision)
Recall deficit (BUY side)  :  −1.2pp  (signals not firing)
```
...
✅ **1 issues already applied** in the codebase (expected gain: +1.6pp once retrained).
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=11, buy_win_rate=90.9%
   - PASS — 11 trades, 90.9% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -11.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 42.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -11.2pp |
| OOF → Holdout gap | +54.6pp |
| thr_buy / thr_sell | 83.225 / 83.077 |
| Blocked signals | 0 (133 would-win / 114 would-lose) |

> ⚠ Gate is DESTROYING 11.2pp of precision. Blocked signals (53.8%) would have beaten gated (42.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (97.3%) exceeds holdout (42.6%) by 54.6pp.
...
**Symbol:** ICP/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=42.6%  Sharpe=17.00
**Expected after fixes:** Precision≈48.4%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=59.3% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 97.3% | ✓ |
| Holdout Precision | 42.6% | ✗ |
| Holdout Coverage | 13.7% | ✓ |
| 95% CI Precision | [35.7%, 49.9%] | — |
| OOF→Holdout Gap | -54.6% | ⚠ degradation |
| Holdout Fired | 183 trades | ✓ |
| SELL Win Rate | 93.2% (73 trades) | ✓ |
| BUY Win Rate | 90.9% (11 trades) | ✓ |
| Sharpe (annualised) | 17.00 | ✓ |
| Max Drawdown | 24.56% | ✗ |
| Profit Factor | 8.35 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +1.0850% | ✓ |
...
| Statistical Sig. | p=0.0459 (z=-2.00) | ✓ significant |

### Class Distribution
- HOLD: **59.3%** — ⚠ severe imbalance
- SELL: **24.6%**
- BUY:  **16.1%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 59.3% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       278  (100%)
Blocked by Meta Gate:      -  247  (89%)
...
Average hold-signal realized return: **2.0889%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 247 | 133 | 114 | 53.8% | +515.95% |
| quality | 22 | 13 | 9 | 61.5% | +45.95% ⚠ |
| hmm | 13 | 7 | 6 | 57.6% | +27.16% ⚠ |
| confluence | 11 | 6 | 5 | 54.5% | +22.98% |
| fake_breakout | 8 | 4 | 4 | 60.0% | +16.71% ⚠ |
| portfolio | 5 | 2 | 3 | 57.6% | +10.44% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0643 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1072 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 42.6% | -0.32 | ⚠ overconfident |
| 80-90% | 52.6% | -0.32 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 73 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.107 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +54.64pp |
...
| Holdout Sharpe | 17.00 | ✓ |
| Holdout Max DD | 24.56% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=59.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 42.6%  →  **Expected precision (all fixes):** 48.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 183 |
| Rejected signals | 1154 |
| Selected precision | 42.6% |
| Rejected precision | 25.7% |
| Meta gate lift (precision) | +16.9% |
| Selected expectancy | +1.085% |
| Rejected expectancy | +0.196% |
| Selected Sharpe | +41.60 |
| Rejected Sharpe | +10.22 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 83.2 | 83.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ICP/USDT vs BTC/ETH Comparison
...
| Actual Precision | 42.6% |
| Gap | -14.4% |
| Coverage | 3.6% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 66/100 |
