...
**Symbol:** STRK/USDT  |  **Generated:** 2026-06-07 16:39:39
...
> Measured precision is 33.6% — estimated 3.0pp below achievable ceiling of ~36.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.6pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  33.5%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −0.6pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~36.5%  (+3.0pp precision)
Recall deficit (BUY side)  :  −0.6pp  (signals not firing)
```
...
✅ **1 issues already applied** in the codebase (expected gain: +0.8pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 16 holdout BUY signals fired
   - PASS — 16 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=16, buy_win_rate=62.5%
   - PASS — 16 trades, 62.5% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -20.3pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 33.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -20.3pp |
| OOF → Holdout gap | +50.1pp |
| thr_buy / thr_sell | 50.670 / 50.575 |
| Blocked signals | 0 (63 would-win / 55 would-lose) |

> ⚠ Gate is DESTROYING 20.3pp of precision. Blocked signals (53.8%) would have beaten gated (33.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (83.7%) exceeds holdout (33.6%) by 50.1pp.
...
**Symbol:** STRK/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=33.6%  Sharpe=17.77
**Expected after fixes:** Precision≈39.3%  (+5.8pp)
...
| Dev OOF Precision | 83.7% | ✓ |
| Holdout Precision | 33.6% | ✗ |
| Holdout Coverage | 46.5% | ✓ |
| 95% CI Precision | [29.9%, 37.4%] | — |
| OOF→Holdout Gap | -50.1% | ⚠ degradation |
| Holdout Fired | 620 trades | ✓ |
| SELL Win Rate | 75.0% (264 trades) | ✓ |
| BUY Win Rate | 62.5% (16 trades) | ✓ |
| Sharpe (annualised) | 17.77 | ✓ |
| Max Drawdown | 154.91% | ✗ |
| Profit Factor | 2.29 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4829% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-8.19) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 118 | 63 | 55 | 53.8% | +81.19% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0692 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0984 | ~1.0 | ✓ |
...
| 70-80% | 33.5% | -0.41 | ⚠ overconfident |
| 80-90% | 43.5% | -0.41 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 558 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.098 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +50.14pp |
...
| Holdout Sharpe | 17.77 | ✓ |
| Holdout Max DD | 154.91% | ✗ |
...
**Base precision:** 33.6%  →  **Expected precision (all fixes):** 39.3%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 620 |
| Rejected signals | 714 |
| Selected precision | 33.6% |
| Rejected precision | 28.4% |
| Meta gate lift (precision) | +5.1% |
| Selected expectancy | +0.483% |
| Rejected expectancy | +0.392% |
| Selected Sharpe | +23.62 |
| Rejected Sharpe | +21.16 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 50.7 | 50.6 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep STRK/USDT vs BTC/ETH Comparison
