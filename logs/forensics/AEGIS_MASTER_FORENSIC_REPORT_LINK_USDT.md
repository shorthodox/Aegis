...
**Symbol:** LINK/USDT  |  **Generated:** 2026-06-14 02:00:53
...
> Measured precision is 49.4% — estimated 4.2pp below achievable ceiling of ~53.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Temperature T=1.971 — overconfidence (−1.0pp)

#### Precision Waterfall

```
Measured holdout precision :  49.4%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −0.9pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.971 — overconfidence
  [✅ FIXED       ]  −0.3pp prec    Anti-selective gate — precision below chance

Achievable precision       :  ~53.6%  (+4.2pp precision)
Recall deficit (BUY side)  :  −0.9pp  (signals not firing)
```
...
| Temperature T=1.971 — overconfidence | `scripts/retrain_model.py` | meta gate block (removed) | `meta_full LR gate removed — primary` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.971 — overconfidence**
> 📍 `scripts/retrain_model.py:meta gate block (removed)` — `meta_full LR gate removed — primary-only calibrated confidence gate`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). The entire 663-line LR OOF/edge engine/regime bloc…

### Q3 — What Is The Best Fix Right Now?

✅ **2 issues already applied** in the codebase (expected gain: +1.5pp once retrained).
...
❌ **3. Holdout signal precision ≥ token breakeven**
   ...
   - Value: signal_prec=49.4% vs breakeven=52.3% (796 fired, dir_prec≈79.4%)
   - FAIL — signal_prec=49.4% < breakeven=52.3% (gap=3.0pp). Primary model needs stronger directional skill.
...
✅ **4. tradeable_final (all criteria: signal_prec, dir_prec ≥55%, cov ≥5%)**
   ...
   - Value: tradeable=True, coverage=47.7%
   - PASS — token ENABLED. Signal gate cleared all criteria.

#### Primary Confidence Gate Audit

✅ **Gate status: HELPING**  (lift: +14.2pp)  ❌ signal_prec < breakeven

| Metric | Value |
|--------|-------|
| Selected signal precision | 49.4% |
| Rejected signal precision | 35.2% |
| Gate lift (precision) | +14.2pp |
| Primary conf. threshold | 0.520 |
| Token breakeven | 52.3% |
| Selected signals | 796 |
| Rejected signals | 872 |

> Primary gate adds 14.2pp of precision. Selected signals (49.4%) beat rejected (35.2%). Threshold=0.520. | DISABLED: selected signal_prec=49.4% < breakeven=52.3%.
...
**Symbol:** LINK/USDT  |  **Audit:** 2026-06-14 02:00  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=49.4%  Sharpe=28.94
**Expected after fixes:** Precision≈62.1%  (+12.7pp)
...
2. 🔴 **Anti-Selective Gate (precision < 50%)** — Score: 90/100
   > holdout_prec=49.4% — gate is selecting WRONG signals. Worse than random for directional trading.

3. 🟡 **HMM Regime Collapse** — Score: 62/100
   > Max state concentration=100.0%. HMM assigning most bars to one state.

4. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
   > HOLD=57.5% of labels. Meta model sees 60% zero-labels → calibration distorted.

5. 🟡 **Confidence Inflation** — Score: 55/100
   > T=1.971>1.0. Model overestimates confidence.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 66.5% | ✓ |
| Dev OOF Precision | 34.3% | ✗ |
| Holdout Precision | 49.4% | ✗ |
| Holdout Coverage | 47.7% | ✓ |
| 95% CI Precision | [45.9%, 52.8%] | — |
| OOF→Holdout Gap | +15.1% | ✓ holdout beat OOF |
| Holdout Fired | 796 trades | ✓ |
| SELL Win Rate | 74.4% (320 trades) | ✓ |
| BUY Win Rate | 88.6% (175 trades) | ✓ |
| Sharpe (annualised) | 28.94 | ✓ |
| Max Drawdown | 2.41% | ✓ |
| Profit Factor | 2.62 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1880% | ✓ |
| Gate mode | PRIMARY-ONLY (calibrated) | ✓ |
| Primary conf. threshold | 0.520 | ✓ |
| Primary calibrator | present (primary_only) | ✓ |
| Signal prec vs breakeven | -3.0pp (be=52.3%) | ✗ below breakeven |
| Statistical Sig. | p=0.7230 (z=-0.35) | ⚠ insufficient data |

### Class Distribution
- HOLD: **57.5%** — ⚠ severe imbalance
- SELL: **25.7%**
- BUY:  **16.8%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 57.5% HOLD labels biases model toward neutrality.
- **CRITICAL** — Anti-selective gate: holdout precision=49.4% < 50%. Gate is selecting the worst signals. Rebuild with fixed local model.
- **CRITICAL** — Precision below random chance (49.4%). Model is directionally anti-predictive on holdout data.
...
```
Generated (directional):       556  (100%)
Below Primary Conf. Thr:   -    0  (0%)
Blocked by Quality (<55):  -   44
Blocked by HMM:            -   27
Blocked by Confluence:     -   22
Blocked by Fake Breakout:  -   16
Blocked by Portfolio Guard:-   11
Blocked by Safe Mode:      -    5
Blocked by Drift:          -   11
Blocked by Cooldown:       -    5
─────────────────────────────────────
Estimated Executed:           415  (74.6%)
```
...
Average hold-signal realized return: **1.6655%/bar**
...
| quality | 44 | 27 | 17 | 61.5% | +73.28% ⚠ |
| hmm | 27 | 15 | 12 | 57.6% | +44.97% ⚠ |
| confluence | 22 | 12 | 10 | 54.5% | +36.64% |
| fake_breakout | 16 | 9 | 7 | 60.0% | +26.65% ⚠ |
| portfolio | 11 | 6 | 5 | 57.6% | +18.32% ⚠ |
...
| Cal. Temperature | 1.9708 | ~1.0 | ⚠ model overconfident |
| Calibration Type | temperature (T=1.971) | isotonic | — |
...
| 70-80% | 49.4% | -0.26 | ⚠ overconfident |
| 80-90% | 59.4% | -0.26 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** YES — model claims higher confidence than earned
**Recommended calibrator (for 796 dev samples):** `isotonic`
...
| Confidence Drift | 🔴 CRITICAL | T=1.971 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +49.37pp |
...
**5 root causes identified.**  Combined top-5 impact score: **363/500**
...
| 2 | 🔴 **Anti-Selective Gate (precision < 50%)** | Gate Failure | 90/100 | `scripts/retrain_model.py:meta gate block (removed)` ✅ FIXED | holdout_prec=49.4% — gate is selecting WRONG signals. Worse than rando… |
| 3 | 🟡 **HMM Regime Collapse** | HMM Failure | 62/100 | — 🔴 ACTIVE | Max state concentration=100.0%. HMM assigning most bars to one state.… |
| 4 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.5% of labels. Meta model sees 60% zero-labels → calibration di… |
| 5 | 🟡 **Confidence Inflation** | Calibration | 55/100 | `scripts/retrain_model.py:meta gate block (removed)` ✅ FIXED | T=1.971>1.0. Model overestimates confidence.… |
...
**Base precision:** 49.4%  →  **Expected precision (all fixes):** 62.1%  (+12.7pp)
...
| 4 | Full retrain with all pipeline fixes applied | +5.8pp | +2.0pp | +4.9pp | HIGH | HIGH |
...
| Selected signals | 796 |
| Rejected signals | 872 |
| Selected precision | 49.4% |
| Rejected precision | 35.2% |
| Meta gate lift (precision) | +14.2% |
| Selected expectancy | +0.188% |
| Rejected expectancy | -0.023% |
| Selected Sharpe | +33.95 |
| Rejected Sharpe | -3.17 |
...
## Section 19 — Deep LINK/USDT vs BTC/ETH Comparison
