...
**Symbol:** LINK/USDT  |  **Generated:** 2026-06-07 16:39:24
...
> Measured precision is 34.3% — estimated 3.3pp below achievable ceiling of ~37.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  34.3%
...
Achievable precision       :  ~37.6%  (+3.3pp precision)
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 289 BUY proposals / 556 total directional
   - PASS — primary fires BUY on some bars.

✅ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(231)=80 ≥ min_fires=35`
   - Value: 80 max fires vs 35 required
   - PASS — 80 ≥ 35.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 142 holdout BUY signals fired
   - PASS — 142 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=142, buy_win_rate=67.6%
   - PASS — 142 trades, 67.6% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -19.6pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 34.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -19.6pp |
| OOF → Holdout gap | +42.0pp |
| thr_buy / thr_sell | 46.589 / 46.589 |
| Blocked signals | 0 (241 would-win / 208 would-lose) |

> ⚠ Gate is DESTROYING 19.6pp of precision. Blocked signals (53.8%) would have beaten gated (34.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (76.3%) exceeds holdout (34.3%) by 42.0pp.
...
**Symbol:** LINK/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=34.3%  Sharpe=7.54
**Expected after fixes:** Precision≈40.0%  (+5.8pp)
...
| Dev OOF Precision | 76.3% | ✓ |
| Holdout Precision | 34.3% | ✗ |
| Holdout Coverage | 39.4% | ✓ |
| 95% CI Precision | [30.3%, 38.5%] | — |
| OOF→Holdout Gap | -42.0% | ⚠ degradation |
| Holdout Fired | 525 trades | ✓ |
| SELL Win Rate | 60.9% (138 trades) | ✓ |
| BUY Win Rate | 67.6% (142 trades) | ✓ |
| Sharpe (annualised) | 7.54 | ✓ |
| Max Drawdown | 23344078607.63% | ✗ |
| Profit Factor | 1.37 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0949% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-7.20) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 449 | 241 | 208 | 53.8% | +747.79% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0911 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1802 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 34.3% | -0.41 | ⚠ overconfident |
| 80-90% | 44.3% | -0.41 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 131 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.180 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +42.05pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.5% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 34.3%  →  **Expected precision (all fixes):** 40.0%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 525 |
| Rejected signals | 809 |
| Selected precision | 34.3% |
| Rejected precision | 28.5% |
| Meta gate lift (precision) | +5.7% |
| Selected expectancy | +0.095% |
| Rejected expectancy | +0.213% |
| Selected Sharpe | +10.88 |
| Rejected Sharpe | +17.33 |
