...
**Symbol:** ENJ/USDT  |  **Generated:** 2026-06-07 16:35:58
...
> Measured precision is 30.3% — estimated 7.9pp below achievable ceiling of ~38.2%. Primary contributors: Barrier skew suppresses BUY labels (−2.6pp), HOLD over-representation in labels (−2.9pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  30.3%
...
Achievable precision       :  ~38.2%  (+7.9pp precision)
Recall deficit (BUY side)  :  −3.8pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +6.6pp once retrained).
...
| Metric | Value |
|--------|-------|
| Gated-in precision | 30.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.5pp |
| OOF → Holdout gap | +53.8pp |
| thr_buy / thr_sell | 60.345 / 60.126 |
| Blocked signals | 0 (132 would-win / 114 would-lose) |

> ⚠ Gate is DESTROYING 23.5pp of precision. Blocked signals (53.8%) would have beaten gated (30.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (84.1%) exceeds holdout (30.3%) by 53.8pp.
...
**Symbol:** ENJ/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.3%  Sharpe=18.09
**Expected after fixes:** Precision≈36.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 69/100
   > HOLD=69.0% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 62.0% | ⚠ |
| Dev OOF Precision | 84.1% | ✓ |
| Holdout Precision | 30.3% | ✗ |
| Holdout Coverage | 30.7% | ✓ |
| 95% CI Precision | [26.1%, 34.9%] | — |
| OOF→Holdout Gap | -53.8% | ⚠ degradation |
| Holdout Fired | 409 trades | ✓ |
| SELL Win Rate | 82.5% (149 trades) | ✓ |
| BUY Win Rate | 100.0% (1 trades) | ✓ |
| Sharpe (annualised) | 18.09 | ✓ |
| Max Drawdown | 38782330026.24% | ✗ |
| Profit Factor | 3.06 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.6416% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-7.96) | ✓ significant |

### Class Distribution
- HOLD: **69.0%** — ⚠ severe imbalance
- SELL: **19.6%**
- BUY:  **11.4%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 69.0% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 246 | 132 | 114 | 53.8% | +169.27% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0339 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9626 | ~1.0 | ✓ |
...
| 70-80% | 30.3% | -0.45 | ⚠ overconfident |
| 80-90% | 40.3% | -0.45 | ⚠ overconfident |
...
| Confidence Drift | 🟢 OK | T=0.963 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +53.82pp |
...
| Holdout Sharpe | 18.09 | ✓ |
| Holdout Max DD | 38782330026.24% | ✗ |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 69/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=69.0% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 30.3%  →  **Expected precision (all fixes):** 36.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 409 |
| Rejected signals | 925 |
| Selected precision | 30.3% |
| Rejected precision | 19.6% |
| Meta gate lift (precision) | +10.8% |
| Selected expectancy | +0.642% |
| Rejected expectancy | +1.254% |
| Selected Sharpe | +29.61 |
| Rejected Sharpe | +15.23 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 60.3 | 60.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ENJ/USDT vs BTC/ETH Comparison
