...
**Symbol:** APT/USDT  |  **Generated:** 2026-06-07 16:35:47
...
> Measured precision is 33.4% — estimated 7.5pp below achievable ceiling of ~40.9%. Primary contributors: Barrier skew suppresses BUY labels (−2.4pp), HOLD over-representation in labels (−2.7pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  33.4%

  [✅ FIXED       ]  −2.4pp prec    Barrier skew suppresses BUY labels
  [✅ FIXED       ]  −2.7pp prec    HOLD over-representation in labels
  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~40.9%  (+7.5pp precision)
...
✅ **2 issues already applied** in the codebase (expected gain: +6.1pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 46 holdout BUY signals fired
   - PASS — 46 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=46, buy_win_rate=87.0%
   - PASS — 46 trades, 87.0% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -20.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 33.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -20.5pp |
| OOF → Holdout gap | +58.4pp |
| thr_buy / thr_sell | 70.690 / 70.094 |
| Blocked signals | 0 (41 would-win / 36 would-lose) |

> ⚠ Gate is DESTROYING 20.5pp of precision. Blocked signals (53.8%) would have beaten gated (33.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (91.8%) exceeds holdout (33.4%) by 58.4pp.
...
**Symbol:** APT/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=33.4%  Sharpe=29.54
**Expected after fixes:** Precision≈39.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 67/100
   > HOLD=67.9% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 63.2% | ⚠ |
| Dev OOF Precision | 91.8% | ✓ |
| Holdout Precision | 33.4% | ✗ |
| Holdout Coverage | 43.3% | ✓ |
| 95% CI Precision | [29.7%, 37.3%] | — |
| OOF→Holdout Gap | -58.4% | ⚠ degradation |
| Holdout Fired | 581 trades | ✓ |
| SELL Win Rate | 90.1% (171 trades) | ✓ |
| BUY Win Rate | 87.0% (46 trades) | ✓ |
| Sharpe (annualised) | 29.54 | ✓ |
| Max Drawdown | 122.07% | ✗ |
| Profit Factor | 5.57 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.5785% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-8.01) | ✓ significant |

### Class Distribution
- HOLD: **67.9%** — ⚠ severe imbalance
- SELL: **20.0%**
- BUY:  **12.1%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 67.9% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       134  (100%)
Blocked by Meta Gate:      -   77  (57%)
Blocked by Quality (<55):  -   10
Blocked by HMM:            -    6
...
Average hold-signal realized return: **1.8263%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 77 | 41 | 36 | 53.8% | +140.63% |
| quality | 10 | 6 | 4 | 61.5% | +18.26% ⚠ |
| hmm | 6 | 3 | 3 | 57.6% | +10.96% ⚠ |
| confluence | 5 | 2 | 3 | 54.5% | +9.13% |
| fake_breakout | 4 | 2 | 2 | 60.0% | +7.31% ⚠ |
| portfolio | 2 | 1 | 1 | 57.6% | +3.65% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0816 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0986 | ~1.0 | ✓ |
...
| 70-80% | 33.4% | -0.42 | ⚠ overconfident |
| 80-90% | 43.4% | -0.42 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 219 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.099 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +58.39pp |
...
| Holdout Sharpe | 29.54 | ✓ |
| Holdout Max DD | 122.07% | ✗ |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 67/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=67.9% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 33.4%  →  **Expected precision (all fixes):** 39.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 581 |
| Rejected signals | 761 |
| Selected precision | 33.4% |
| Rejected precision | 18.5% |
| Meta gate lift (precision) | +14.9% |
| Selected expectancy | +0.579% |
| Rejected expectancy | +0.033% |
| Selected Sharpe | +40.56 |
| Rejected Sharpe | +3.12 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 70.7 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep APT/USDT vs BTC/ETH Comparison
