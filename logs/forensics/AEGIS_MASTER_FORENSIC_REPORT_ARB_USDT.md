...
**Symbol:** ARB/USDT  |  **Generated:** 2026-06-07 16:35:48
...
> Measured precision is 24.3% — estimated 3.1pp below achievable ceiling of ~27.4%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.7pp)

#### Precision Waterfall

```
Measured holdout precision :  24.3%

  [✅ FIXED       ]  −8.0pp recall  BUY side completely disabled (min_fires deadlock)
  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −0.7pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~27.4%  (+3.1pp precision)
Recall deficit (BUY side)  :  −8.7pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +3.3pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 29 BUY proposals / 127 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(23)=8 ≥ min_fires=35`
   - Value: 8 max fires vs 35 required
   - FAIL (FIXED) — 8 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
🔴 **Gate status: HURTING**  (lift: -29.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 24.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -29.5pp |
| OOF → Holdout gap | +63.9pp |
| thr_buy / thr_sell | 60.142 / 60.160 |
| Blocked signals | 0 (66 would-win / 58 would-lose) |

> ⚠ Gate is DESTROYING 29.5pp of precision. Blocked signals (53.8%) would have beaten gated (24.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (88.2%) exceeds holdout (24.3%) by 63.9pp.
...
**Symbol:** ARB/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=24.3%  Sharpe=4.25
**Expected after fixes:** Precision≈30.1%  (+5.8pp)
...
3. 🟡 **Low Holdout Sample Size** — Score: 23/100
   > Only 37 holdout signals. 95% CI=[13.4%, 40.1%] (width=26.8%).
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 64.5% | ⚠ |
| Dev OOF Precision | 24.3% | ✗ |
| Holdout Precision | 24.3% | ✗ |
| Holdout Coverage | 2.8% | ✓ |
| 95% CI Precision | [13.4%, 40.1%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 37 trades | ⚠ low sample |
| SELL Win Rate | 90.0% (10 trades) | ✓ |
| BUY Win Rate | 0.0% (0 trades) | ✗ disabled |
| Sharpe (annualised) | 4.25 | ✓ |
| Max Drawdown | 300000000.00% | ✗ |
| Profit Factor | 2.22 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1388% | ✓ |
...
| Statistical Sig. | p=0.0018 (z=-3.12) | ✓ significant |
...
- **WARNING** — Only 37 holdout signals. 95% CI spans 26.8%. High variance in precision estimate.
...
```
Generated (directional):       127  (100%)
Blocked by Meta Gate:      -  124  (98%)
...
Average hold-signal realized return: **2.0263%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 124 | 66 | 58 | 53.8% | +251.27% |
| quality | 10 | 6 | 4 | 61.5% | +20.26% ⚠ |
| hmm | 6 | 3 | 3 | 57.6% | +12.16% ⚠ |
| confluence | 5 | 2 | 3 | 54.5% | +10.13% |
| fake_breakout | 3 | 1 | 2 | 60.0% | +6.08% ⚠ |
| portfolio | 2 | 1 | 1 | 57.6% | +4.05% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0713 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9342 | ~1.0 | ✓ |
...
| 70-80% | 24.3% | -0.51 | ⚠ overconfident |
| 80-90% | 34.3% | -0.51 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 17 dev samples):** `temperature`
...
| Confidence Drift | 🟢 OK | T=0.934 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +63.91pp |
...
**3 root causes identified.**  Combined top-5 impact score: **173/500**
...
| 3 | 🟢 **Low Holdout Sample Size** | Statistical Reliability | 23/100 | `scripts/retrain_model.py:1632 (train_token entry)` ✅ FIXED | Only 37 holdout signals. 95% CI=[13.4%, 40.1%] (width=26.8%).… |
...
**Base precision:** 24.3%  →  **Expected precision (all fixes):** 30.1%  (+5.8pp)
...
|--------|-------|
| Selected signals | 37 |
| Rejected signals | 1297 |
| Selected precision | 24.3% |
| Rejected precision | 28.6% |
| Meta gate lift (precision) | -4.3% |
| Selected expectancy | +0.139% |
| Rejected expectancy | +0.241% |
| Selected Sharpe | +23.10 |
| Rejected Sharpe | +16.01 |
...
## Section 19 — Deep ARB/USDT vs BTC/ETH Comparison
