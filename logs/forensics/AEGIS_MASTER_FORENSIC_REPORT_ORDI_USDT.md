...
**Symbol:** ORDI/USDT  |  **Generated:** 2026-06-07 16:39:31
...
> Measured precision is 39.9% — estimated 3.2pp below achievable ceiling of ~43.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.8pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  39.9%
...
Achievable precision       :  ~43.1%  (+3.2pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 96 holdout BUY signals fired
   - PASS — 96 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=96, buy_win_rate=77.1%
   - PASS — 96 trades, 77.1% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -13.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 39.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -13.9pp |
| OOF → Holdout gap | +44.8pp |
| thr_buy / thr_sell | 46.932 / 46.837 |
| Blocked signals | 0 (92 would-win / 80 would-lose) |

> ⚠ Gate is DESTROYING 13.9pp of precision. Blocked signals (53.8%) would have beaten gated (39.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (84.7%) exceeds holdout (39.9%) by 44.8pp.
...
**Symbol:** ORDI/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=39.9%  Sharpe=28.09
**Expected after fixes:** Precision≈45.7%  (+5.8pp)

### Top 5 Problems

1. 🟡 **HMM Regime Collapse** — Score: 62/100
   > Max state concentration=55.0%. HMM assigning most bars to one state.

2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
   > HOLD=57.7% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 55.5% | ⚠ |
| Dev OOF Precision | 84.7% | ✓ |
| Holdout Precision | 39.9% | ✗ |
| Holdout Coverage | 59.0% | ✓ |
| 95% CI Precision | [36.6%, 43.4%] | — |
| OOF→Holdout Gap | -44.8% | ⚠ degradation |
| Holdout Fired | 789 trades | ✓ |
| SELL Win Rate | 79.8% (302 trades) | ✓ |
| BUY Win Rate | 77.1% (96 trades) | ✓ |
| Sharpe (annualised) | 28.09 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 5.30 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +1.5722% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.66) | ✓ significant |

### Class Distribution
- HOLD: **57.7%** — ⚠ severe imbalance
- SELL: **24.8%**
- BUY:  **17.5%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 57.7% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 172 | 92 | 80 | 53.8% | +118.35% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0801 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.8135 | ~1.0 | ✓ |
...
| 70-80% | 39.9% | -0.35 | ⚠ overconfident |
| 80-90% | 49.9% | -0.35 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 412 dev samples):** `isotonic`
...
**States:** 8  |  **Global precision:** 66.0%  |  **Max state concentration:** 55.0% ⚠ REGIME COLLAPSE
...
| Confidence Drift | 🟢 OK | T=0.814 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +44.78pp |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **HMM Regime Collapse** | HMM Failure | 62/100 | — 🔴 ACTIVE | Max state concentration=55.0%. HMM assigning most bars to one state.… |
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.7% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 39.9%  →  **Expected precision (all fixes):** 45.7%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 789 |
| Rejected signals | 548 |
| Selected precision | 39.9% |
| Rejected precision | 30.7% |
| Meta gate lift (precision) | +9.3% |
| Selected expectancy | +1.572% |
| Rejected expectancy | +0.160% |
| Selected Sharpe | +33.09 |
| Rejected Sharpe | +8.63 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 46.9 | 46.8 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ORDI/USDT vs BTC/ETH Comparison
