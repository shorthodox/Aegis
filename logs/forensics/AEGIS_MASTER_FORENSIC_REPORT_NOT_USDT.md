...
**Symbol:** NOT/USDT  |  **Generated:** 2026-06-07 16:39:28
...
> Measured precision is 39.4% — estimated 3.4pp below achievable ceiling of ~42.8%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  39.4%
...
Achievable precision       :  ~42.8%  (+3.4pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=14, buy_win_rate=92.9%
   - PASS — 14 trades, 92.9% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -14.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 39.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -14.5pp |
| OOF → Holdout gap | +60.6pp |
| thr_buy / thr_sell | 85.058 / 85.058 |
| Blocked signals | 0 (232 would-win / 200 would-lose) |

> ⚠ Gate is DESTROYING 14.5pp of precision. Blocked signals (53.8%) would have beaten gated (39.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (100.0%) exceeds holdout (39.4%) by 60.6pp.
...
**Symbol:** NOT/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=39.4%  Sharpe=8.91
**Expected after fixes:** Precision≈45.1%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 58/100
   > HOLD=58.1% of labels. Meta model sees 60% zero-labels → calibration distorted.

3. 🟡 **Low Holdout Sample Size** — Score: 27/100
   > Only 33 holdout signals. 95% CI=[24.7%, 56.3%] (width=31.6%).
...
| Dev OOF Precision | 39.4% | ✗ |
| Holdout Precision | 39.4% | ✗ |
| Holdout Coverage | 2.5% | ✓ |
| 95% CI Precision | [24.7%, 56.3%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 33 trades | ⚠ low sample |
| SELL Win Rate | 0.0% (0 trades) | ✗ |
| BUY Win Rate | 92.9% (14 trades) | ✓ |
| Sharpe (annualised) | 8.91 | ✓ |
| Max Drawdown | 14.15% | ✗ |
| Profit Factor | 5.14 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4050% | ✓ |
...
| Statistical Sig. | p=0.2230 (z=-1.22) | ⚠ insufficient data |

### Class Distribution
- HOLD: **58.1%** — ⚠ severe imbalance
- SELL: **25.1%**
- BUY:  **16.8%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 58.1% HOLD labels biases model toward neutrality.
- **WARNING** — Only 33 holdout signals. 95% CI spans 31.6%. High variance in precision estimate.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 432 | 232 | 200 | 53.8% | +297.25% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0698 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1112 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 39.4% | -0.36 | ⚠ overconfident |
| 80-90% | 49.4% | -0.36 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 12 dev samples):** `temperature`
...
| Confidence Drift | 🟡 WARNING | T=1.111 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +60.61pp |
...
**3 root causes identified.**  Combined top-5 impact score: **147/500**
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 58/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=58.1% of labels. Meta model sees 60% zero-labels → calibration di… |
| 3 | 🟢 **Low Holdout Sample Size** | Statistical Reliability | 27/100 | `scripts/retrain_model.py:1632 (train_token entry)` ✅ FIXED | Only 33 holdout signals. 95% CI=[24.7%, 56.3%] (width=31.6%).… |
...
**Base precision:** 39.4%  →  **Expected precision (all fixes):** 45.1%  (+5.8pp)
...
|--------|-------|
| Selected signals | 33 |
| Rejected signals | 1301 |
| Selected precision | 39.4% |
| Rejected precision | 28.7% |
| Meta gate lift (precision) | +10.7% |
| Selected expectancy | +0.405% |
| Rejected expectancy | +0.408% |
| Selected Sharpe | +51.30 |
| Rejected Sharpe | +15.80 |
...
## Section 19 — Deep NOT/USDT vs BTC/ETH Comparison
