...
**Symbol:** 1INCH/USDT  |  **Generated:** 2026-06-07 16:35:46
...
> Measured precision is 38.0% — estimated 3.6pp below achievable ceiling of ~41.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.2pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  38.0%
...
Achievable precision       :  ~41.6%  (+3.6pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=43, buy_win_rate=69.8%
   - PASS — 43 trades, 69.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -15.8pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 38.0% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -15.8pp |
| OOF → Holdout gap | +60.9pp |
| thr_buy / thr_sell | 70.121 / 70.183 |
| Blocked signals | 0 (206 would-win / 177 would-lose) |

> ⚠ Gate is DESTROYING 15.8pp of precision. Blocked signals (53.8%) would have beaten gated (38.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (98.9%) exceeds holdout (38.0%) by 60.9pp.
...
**Symbol:** 1INCH/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=38.0%  Sharpe=19.64
**Expected after fixes:** Precision≈43.8%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=59.7% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 57.8% | ⚠ |
| Dev OOF Precision | 98.9% | ✓ |
| Holdout Precision | 38.0% | ✗ |
| Holdout Coverage | 25.6% | ✓ |
| 95% CI Precision | [33.0%, 43.3%] | — |
| OOF→Holdout Gap | -60.9% | ⚠ degradation |
| Holdout Fired | 342 trades | ✓ |
| SELL Win Rate | 85.5% (117 trades) | ✓ |
| BUY Win Rate | 69.8% (43 trades) | ✓ |
| Sharpe (annualised) | 19.64 | ✓ |
| Max Drawdown | 1330708087.41% | ✗ |
| Profit Factor | 2.96 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2860% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-4.43) | ✓ significant |

### Class Distribution
- HOLD: **59.7%** — ⚠ severe imbalance
- SELL: **24.3%**
- BUY:  **16.0%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 59.7% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 383 | 206 | 177 | 53.8% | +263.54% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0914 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1535 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 38.0% | -0.37 | ⚠ overconfident |
| 80-90% | 48.0% | -0.37 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 88 dev samples):** `isotonic`
...
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +60.85pp |
...
| Holdout Sharpe | 19.64 | ✓ |
| Holdout Max DD | 1330708087.41% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=59.7% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 38.0%  →  **Expected precision (all fixes):** 43.8%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 342 |
| Rejected signals | 996 |
| Selected precision | 38.0% |
| Rejected precision | 21.1% |
| Meta gate lift (precision) | +16.9% |
| Selected expectancy | +0.286% |
| Rejected expectancy | +0.058% |
| Selected Sharpe | +35.14 |
| Rejected Sharpe | +5.73 |
...
## Section 19 — Deep 1INCH/USDT vs BTC/ETH Comparison
