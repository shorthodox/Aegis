...
**Symbol:** ENA/USDT  |  **Generated:** 2026-06-07 16:35:58
...
> Measured precision is 20.7% — estimated 6.8pp below achievable ceiling of ~27.5%. Primary contributors: Barrier skew suppresses BUY labels (−1.6pp), HOLD over-representation in labels (−1.8pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  20.7%
  ...
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.218 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~27.5%  (+6.8pp precision)
Recall deficit (BUY side)  :  −2.3pp  (signals not firing)
```
...
| Temperature T=1.218 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.218 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 84 BUY proposals / 261 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(67)=23 ≥ min_fires=35`
   - Value: 23 max fires vs 35 required
   - FAIL (FIXED) — 23 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
❌ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=15, buy_win_rate=40.0%
   - PASS — 15 trades, 40.0% WR.
...
| Metric | Value |
|--------|-------|
| Gated-in precision | 20.7% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -33.2pp |
| OOF → Holdout gap | +43.2pp |
| thr_buy / thr_sell | 46.504 / 46.504 |
| Blocked signals | 0 (122 would-win / 106 would-lose) |

> ⚠ Gate is DESTROYING 33.2pp of precision. Blocked signals (53.8%) would have beaten gated (20.7%). Meta model is anti-selective. | OOF overfit warning: dev_prec (63.9%) exceeds holdout (20.7%) by 43.2pp.
...
**Symbol:** ENA/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=20.7%  Sharpe=-2.98
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 61/100
   > HOLD=61.9% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 63.9% | ✓ |
| Holdout Precision | 20.7% | ✗ |
| Holdout Coverage | 15.5% | ✓ |
| 95% CI Precision | [15.7%, 26.7%] | — |
| OOF→Holdout Gap | -43.2% | ⚠ degradation |
| Holdout Fired | 208 trades | ✓ |
| SELL Win Rate | 52.1% (71 trades) | ✗ |
| BUY Win Rate | 40.0% (15 trades) | ✓ |
| Sharpe (annualised) | -2.98 | ✗ |
| Max Drawdown | 1368.07% | ✗ |
| Profit Factor | 0.77 | ✗ |
| Kelly Fraction | 0.0% | — |
| Expectancy/Trade | -0.1534% | ✗ |
...
| Statistical Sig. | p=0.0000 (z=-8.46) | ✓ significant |

### Class Distribution
- HOLD: **61.9%** — ⚠ severe imbalance
- SELL: **23.4%**
- BUY:  **14.8%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 61.9% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       261  (100%)
Blocked by Meta Gate:      -  228  (87%)
...
Average hold-signal realized return: **2.4179%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 228 | 122 | 106 | 53.8% | +551.29% |
| quality | 20 | 12 | 8 | 61.5% | +48.36% ⚠ |
| hmm | 13 | 7 | 6 | 57.6% | +31.43% ⚠ |
| confluence | 10 | 5 | 5 | 54.5% | +24.18% |
| fake_breakout | 7 | 4 | 3 | 60.0% | +16.93% ⚠ |
| portfolio | 5 | 2 | 3 | 57.6% | +12.09% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0380 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2180 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 20.7% | -0.54 | ⚠ overconfident |
| 80-90% | 30.7% | -0.54 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 83 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.218 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +43.18pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 61/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=61.9% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 20.7%  →  **Expected precision (all fixes):** 26.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 208 |
| Rejected signals | 1132 |
| Selected precision | 20.7% |
| Rejected precision | 24.2% |
| Meta gate lift (precision) | -3.5% |
| Selected expectancy | -0.153% |
| Rejected expectancy | +0.444% |
| Selected Sharpe | -6.84 |
| Rejected Sharpe | +24.10 |
...
## Section 19 — Deep ENA/USDT vs BTC/ETH Comparison
