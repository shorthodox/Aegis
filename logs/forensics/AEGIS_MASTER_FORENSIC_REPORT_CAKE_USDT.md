...
**Symbol:** CAKE/USDT  |  **Generated:** 2026-06-07 16:35:54
...
> Measured precision is 30.7% — estimated 2.4pp below achievable ceiling of ~33.1%. Primary contributors: Brier score above target (−2.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  30.7%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~33.1%  (+2.4pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 30 holdout BUY signals fired
   - PASS — 30 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=30, buy_win_rate=66.7%
   - PASS — 30 trades, 66.7% WR.
...
| Metric | Value |
|--------|-------|
| Gated-in precision | 30.7% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -23.2pp |
| OOF → Holdout gap | +54.1pp |
| thr_buy / thr_sell | 46.438 / 46.438 |
| Blocked signals | 0 (183 would-win / 157 would-lose) |

> ⚠ Gate is DESTROYING 23.2pp of precision. Blocked signals (53.8%) would have beaten gated (30.7%). Meta model is anti-selective. | OOF overfit warning: dev_prec (84.8%) exceeds holdout (30.7%) by 54.1pp.
...
**Symbol:** CAKE/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=30.7%  Sharpe=5.25
...
| Dev OOF Precision | 84.8% | ✓ |
| Holdout Precision | 30.7% | ✗ |
| Holdout Coverage | 7.6% | ✓ |
| 95% CI Precision | [22.5%, 40.3%] | — |
| OOF→Holdout Gap | -54.0% | ⚠ degradation |
| Holdout Fired | 101 trades | ✓ |
| SELL Win Rate | 78.6% (14 trades) | ✓ |
| BUY Win Rate | 66.7% (30 trades) | ✓ |
| Sharpe (annualised) | 5.25 | ✓ |
| Max Drawdown | 478.75% | ✗ |
| Profit Factor | 1.70 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1488% | ✓ |
...
| Statistical Sig. | p=0.0001 (z=-3.88) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 340 | 183 | 157 | 53.8% | +233.95% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0718 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0351 | ~1.0 | ✓ |
...
**Recommended calibrator (for 177 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.035 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +54.05pp |
...
**Base precision:** 30.7%  →  **Expected precision (all fixes):** 36.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 101 |
| Rejected signals | 1233 |
| Selected precision | 30.7% |
| Rejected precision | 34.5% |
| Meta gate lift (precision) | -3.8% |
| Selected expectancy | +0.149% |
| Rejected expectancy | +0.177% |
| Selected Sharpe | +17.28 |
| Rejected Sharpe | +16.25 |
...
## Section 19 — Deep CAKE/USDT vs BTC/ETH Comparison
