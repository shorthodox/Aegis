...
**Symbol:** ENS/USDT  |  **Generated:** 2026-06-07 16:35:59
...
> Measured precision is 29.6% — estimated 5.3pp below achievable ceiling of ~34.9%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.4pp), HOLD over-representation in labels (−1.5pp)
...
Achievable precision       :  ~34.9%  (+5.3pp precision)
...
✅ **2 issues already applied** in the codebase (expected gain: +3.5pp once retrained).
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=73, buy_win_rate=69.9%
   - PASS — 73 trades, 69.9% WR.
...
| thr_buy / thr_sell | 60.131 / 60.164 |
| Blocked signals | 0 (89 would-win / 77 would-lose) |

> ⚠ Gate is DESTROYING 24.2pp of precision. Blocked signals (53.8%) would have beaten gated (29.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (92.7%) exceeds holdout (29.6%) by 63.1pp.
...
**Symbol:** ENS/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=29.6%  Sharpe=16.91
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 60/100
   > HOLD=60.1% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 64.3% | ⚠ |
| Dev OOF Precision | 92.7% | ✓ |
| Holdout Precision | 29.6% | ✗ |
| Holdout Coverage | 48.0% | ✓ |
| 95% CI Precision | [26.2%, 33.3%] | — |
| OOF→Holdout Gap | -63.1% | ⚠ degradation |
| Holdout Fired | 641 trades | ✓ |
| SELL Win Rate | 73.9% (188 trades) | ✓ |
| BUY Win Rate | 69.9% (73 trades) | ✓ |
| Sharpe (annualised) | 16.91 | ✓ |
| Max Drawdown | 26291905313.83% | ✗ |
| Profit Factor | 2.24 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.3540% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-10.31) | ✓ significant |

### Class Distribution
- HOLD: **60.1%** — ⚠ severe imbalance
- SELL: **24.6%**
- BUY:  **15.2%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 60.1% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 166 | 89 | 77 | 53.8% | +114.22% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0937 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0383 | ~1.0 | ✓ |
...
**Recommended calibrator (for 398 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.038 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +63.07pp |
...
| Holdout Sharpe | 16.91 | ✓ |
| Holdout Max DD | 26291905313.83% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 60/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=60.1% of labels. Meta model sees 60% zero-labels → calibration di… |
...
| Metric | Value |
|--------|-------|
| Selected signals | 641 |
| Rejected signals | 693 |
| Selected precision | 29.6% |
| Rejected precision | 28.6% |
| Meta gate lift (precision) | +1.1% |
| Selected expectancy | +0.354% |
| Rejected expectancy | +0.279% |
| Selected Sharpe | +22.10 |
...
## Section 19 — Deep ENS/USDT vs BTC/ETH Comparison
