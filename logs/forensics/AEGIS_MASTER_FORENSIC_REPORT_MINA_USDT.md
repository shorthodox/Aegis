...
**Symbol:** MINA/USDT  |  **Generated:** 2026-06-07 16:39:28
...
> Measured precision is 36.8% — estimated 3.8pp below achievable ceiling of ~40.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.4pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  36.8%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −1.4pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~40.6%  (+3.8pp precision)
Recall deficit (BUY side)  :  −1.4pp  (signals not firing)
```
...
✅ **1 issues already applied** in the codebase (expected gain: +1.8pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 48 holdout BUY signals fired
   - PASS — 48 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=48, buy_win_rate=64.6%
   - PASS — 48 trades, 64.6% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -17.1pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 36.8% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -17.1pp |
| OOF → Holdout gap | +59.5pp |
| thr_buy / thr_sell | 70.485 / 70.071 |
| Blocked signals | 0 (218 would-win / 187 would-lose) |

> ⚠ Gate is DESTROYING 17.1pp of precision. Blocked signals (53.8%) would have beaten gated (36.8%). Meta model is anti-selective. | OOF overfit warning: dev_prec (96.2%) exceeds holdout (36.8%) by 59.5pp.
...
**Symbol:** MINA/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=36.8%  Sharpe=11.56
**Expected after fixes:** Precision≈42.5%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=59.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 96.2% | ✓ |
| Holdout Precision | 36.8% | ✗ |
| Holdout Coverage | 30.5% | ✓ |
| 95% CI Precision | [32.2%, 41.5%] | — |
| OOF→Holdout Gap | -59.5% | ⚠ degradation |
| Holdout Fired | 408 trades | ✓ |
| SELL Win Rate | 81.0% (147 trades) | ✓ |
| BUY Win Rate | 64.6% (48 trades) | ✓ |
| Sharpe (annualised) | 11.56 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 1.86 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2144% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.35) | ✓ significant |

### Class Distribution
- HOLD: **59.4%** — ⚠ severe imbalance
- SELL: **25.1%**
- BUY:  **15.5%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 59.4% HOLD labels biases model toward neutrality.
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0888 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1439 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 36.8% | -0.38 | ⚠ overconfident |
| 80-90% | 46.8% | -0.38 | ⚠ overconfident |
...
**States:** 5  |  **Global precision:** 66.0%  |  **Max state concentration:** 45.2%
...
| Confidence Drift | 🟡 WARNING | T=1.144 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +59.46pp |
...
**1 root causes identified.**  Combined top-5 impact score: **59/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=59.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 36.8%  →  **Expected precision (all fixes):** 42.5%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 408 |
| Rejected signals | 930 |
| Selected precision | 36.8% |
| Rejected precision | 25.5% |
| Meta gate lift (precision) | +11.3% |
| Selected expectancy | +0.214% |
| Rejected expectancy | +0.178% |
| Selected Sharpe | +18.95 |
| Rejected Sharpe | +13.68 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 70.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep MINA/USDT vs BTC/ETH Comparison
