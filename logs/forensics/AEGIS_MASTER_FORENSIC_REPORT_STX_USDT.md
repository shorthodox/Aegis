...
**Symbol:** STX/USDT  |  **Generated:** 2026-06-07 16:39:40
...
> Measured precision is 48.3% — estimated 3.3pp below achievable ceiling of ~51.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  48.3%
...
Achievable precision       :  ~51.6%  (+3.3pp precision)
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 92 BUY proposals / 101 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(73)=25 ≥ min_fires=35`
   - Value: 25 max fires vs 35 required
   - FAIL (FIXED) — 25 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 38 holdout BUY signals fired
   - PASS — 38 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=38, buy_win_rate=86.8%
   - PASS — 38 trades, 86.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -5.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 48.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -5.5pp |
| OOF → Holdout gap | +46.6pp |
| thr_buy / thr_sell | 75.161 / 75.099 |
| Blocked signals | 0 (39 would-win / 34 would-lose) |

> ⚠ Gate is DESTROYING 5.5pp of precision. Blocked signals (53.8%) would have beaten gated (48.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (95.0%) exceeds holdout (48.3%) by 46.6pp.
...
**Symbol:** STX/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=48.3%  Sharpe=25.27
**Expected after fixes:** Precision≈54.1%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 55/100
   > HOLD=55.3% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 66.1% | ✓ |
| Dev OOF Precision | 95.0% | ✓ |
| Holdout Precision | 48.3% | ✗ |
| Holdout Coverage | 15.8% | ✓ |
| 95% CI Precision | [41.7%, 55.0%] | — |
| OOF→Holdout Gap | -46.6% | ⚠ degradation |
| Holdout Fired | 211 trades | ✓ |
| SELL Win Rate | 83.1% (83 trades) | ✓ |
| BUY Win Rate | 86.8% (38 trades) | ✓ |
| Sharpe (annualised) | 25.27 | ✓ |
| Max Drawdown | 7053997235.20% | ✗ |
| Profit Factor | 8.72 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +1.3627% | ✓ |
...
| Statistical Sig. | p=0.6299 (z=-0.48) | ⚠ insufficient data |

### Class Distribution
- HOLD: **55.3%** — ⚠ severe imbalance
- SELL: **27.6%**
- BUY:  **17.1%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 55.3% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       101  (100%)
Blocked by Meta Gate:      -   73  (72%)
...
Average hold-signal realized return: **1.7828%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 73 | 39 | 34 | 53.8% | +130.14% |
| quality | 8 | 4 | 4 | 61.5% | +14.26% ⚠ |
| hmm | 5 | 2 | 3 | 57.6% | +8.91% ⚠ |
| confluence | 4 | 2 | 2 | 54.5% | +7.13% |
| fake_breakout | 3 | 1 | 2 | 60.0% | +5.35% ⚠ |
| portfolio | 2 | 1 | 1 | 57.6% | +3.57% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0621 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1806 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 48.3% | -0.27 | ⚠ overconfident |
| 80-90% | 58.3% | -0.27 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 198 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.181 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +46.61pp |
...
| Holdout Sharpe | 25.27 | ✓ |
| Holdout Max DD | 7053997235.20% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 55/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=55.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 48.3%  →  **Expected precision (all fixes):** 54.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 211 |
| Rejected signals | 1123 |
| Selected precision | 48.3% |
| Rejected precision | 28.0% |
| Meta gate lift (precision) | +20.4% |
| Selected expectancy | +1.363% |
| Rejected expectancy | +0.169% |
| Selected Sharpe | +57.56 |
| Rejected Sharpe | +11.99 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 75.2 | 75.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep STX/USDT vs BTC/ETH Comparison
