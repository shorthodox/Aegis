...
**Symbol:** GALA/USDT  |  **Generated:** 2026-06-07 16:36:03
...
> Measured precision is 38.9% — estimated 3.9pp below achievable ceiling of ~42.8%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.5pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  38.9%
...
Achievable precision       :  ~42.8%  (+3.9pp precision)
Recall deficit (BUY side)  :  −1.5pp  (signals not firing)
```
...
✅ **1 issues already applied** in the codebase (expected gain: +1.9pp once retrained).
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 19 holdout BUY signals fired
   - PASS — 19 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=19, buy_win_rate=79.0%
   - PASS — 19 trades, 79.0% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -15.0pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 38.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -15.0pp |
| OOF → Holdout gap | +51.8pp |
| thr_buy / thr_sell | 60.165 / 60.142 |
| Blocked signals | 0 (192 would-win / 165 would-lose) |

> ⚠ Gate is DESTROYING 15.0pp of precision. Blocked signals (53.8%) would have beaten gated (38.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (90.7%) exceeds holdout (38.9%) by 51.8pp.
...
**Symbol:** GALA/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=38.9%  Sharpe=11.06
**Expected after fixes:** Precision≈44.6%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=59.2% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 67.9% | ✓ |
| Dev OOF Precision | 90.7% | ✓ |
| Holdout Precision | 38.9% | ✗ |
| Holdout Coverage | 10.8% | ✓ |
| 95% CI Precision | [31.3%, 47.0%] | — |
| OOF→Holdout Gap | -51.8% | ⚠ degradation |
| Holdout Fired | 144 trades | ✓ |
| SELL Win Rate | 80.4% (51 trades) | ✓ |
| BUY Win Rate | 79.0% (19 trades) | ✓ |
| Sharpe (annualised) | 11.06 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 2.51 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2903% | ✓ |
...
| Statistical Sig. | p=0.0077 (z=-2.67) | ✓ significant |

### Class Distribution
- HOLD: **59.2%** — ⚠ severe imbalance
- SELL: **25.8%**
- BUY:  **15.0%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 59.2% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 357 | 192 | 165 | 53.8% | +245.65% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0674 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1583 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 38.9% | -0.36 | ⚠ overconfident |
| 80-90% | 48.9% | -0.36 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 129 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.158 |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=59.2% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 38.9%  →  **Expected precision (all fixes):** 44.6%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 144 |
| Rejected signals | 1195 |
| Selected precision | 38.9% |
| Rejected precision | 28.7% |
| Meta gate lift (precision) | +10.2% |
| Selected expectancy | +0.290% |
| Rejected expectancy | +0.344% |
| Selected Sharpe | +30.50 |
| Rejected Sharpe | +21.64 |
...
## Section 19 — Deep GALA/USDT vs BTC/ETH Comparison
