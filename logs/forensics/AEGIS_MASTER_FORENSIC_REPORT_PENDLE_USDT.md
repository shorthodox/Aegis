...
**Symbol:** PENDLE/USDT  |  **Generated:** 2026-06-07 16:39:31
...
> Measured precision is 41.5% — estimated 3.6pp below achievable ceiling of ~45.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.2pp)

#### Precision Waterfall

```
Measured holdout precision :  41.5%
...
Achievable precision       :  ~45.1%  (+3.6pp precision)
Recall deficit (BUY side)  :  −9.2pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +4.0pp once retrained).
...
🔴 **Gate status: HURTING**  (lift: -12.3pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 41.5% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -12.3pp |
| OOF → Holdout gap | +40.9pp |
| thr_buy / thr_sell | 58.685 / 58.546 |
| Blocked signals | 0 (126 would-win / 108 would-lose) |

> ⚠ Gate is DESTROYING 12.3pp of precision. Blocked signals (53.8%) would have beaten gated (41.5%). Meta model is anti-selective. | OOF overfit warning: dev_prec (82.4%) exceeds holdout (41.5%) by 40.9pp.
...
**Symbol:** PENDLE/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=41.5%  Sharpe=26.77
**Expected after fixes:** Precision≈47.2%  (+5.8pp)
...
3. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
...
| Dev OOF Precision | 82.4% | ✓ |
| Holdout Precision | 41.5% | ✗ |
| Holdout Coverage | 30.9% | ✓ |
| 95% CI Precision | [36.9%, 46.3%] | — |
| OOF→Holdout Gap | -40.9% | ⚠ degradation |
| Holdout Fired | 412 trades | ✓ |
| SELL Win Rate | 77.7% (220 trades) | ✓ |
| BUY Win Rate | 0.0% (0 trades) | ✗ disabled |
| Sharpe (annualised) | 26.77 | ✓ |
| Max Drawdown | 5344023163.98% | ✗ |
| Profit Factor | 4.13 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +1.1568% | ✓ |
...
| Statistical Sig. | p=0.0006 (z=-3.45) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 234 | 126 | 108 | 53.8% | +161.01% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0515 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9527 | ~1.0 | ✓ |
...
| 70-80% | 41.5% | -0.33 | ⚠ overconfident |
| 80-90% | 51.5% | -0.33 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 329 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=0.953 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +40.87pp |
...
| Holdout Sharpe | 26.77 | ✓ |
| Holdout Max DD | 5344023163.98% | ✗ |
...
**3 root causes identified.**  Combined top-5 impact score: **207/500**
...
| 3 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 41.5%  →  **Expected precision (all fixes):** 47.2%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 412 |
| Rejected signals | 922 |
| Selected precision | 41.5% |
| Rejected precision | 23.0% |
| Meta gate lift (precision) | +18.5% |
| Selected expectancy | +1.157% |
| Rejected expectancy | +0.240% |
| Selected Sharpe | +43.64 |
| Rejected Sharpe | +9.66 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 58.8 | 58.5 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep PENDLE/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 56.3% |
| Actual Precision | 41.5% |
| Gap | -14.8% |
| Coverage | 15.6% |
