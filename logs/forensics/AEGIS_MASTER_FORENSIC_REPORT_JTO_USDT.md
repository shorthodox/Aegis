...
**Symbol:** JTO/USDT  |  **Generated:** 2026-06-07 16:38:32
...
> Measured precision is 31.6% — estimated 2.4pp below achievable ceiling of ~34.0%. Primary contributors: Brier score above target (−2.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  31.6%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~34.0%  (+2.4pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=53, buy_win_rate=98.1%
   - PASS — 53 trades, 98.1% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -22.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 31.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -22.2pp |
| OOF → Holdout gap | +47.5pp |
| thr_buy / thr_sell | 46.385 / 46.385 |
| Blocked signals | 0 (181 would-win / 156 would-lose) |

> ⚠ Gate is DESTROYING 22.2pp of precision. Blocked signals (53.8%) would have beaten gated (31.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (79.1%) exceeds holdout (31.6%) by 47.5pp.
...
**Symbol:** JTO/USDT  |  **Audit:** 2026-06-07 16:38  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=31.6%  Sharpe=10.97
**Expected after fixes:** Precision≈37.4%  (+5.8pp)
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 65.6% | ✓ |
| Dev OOF Precision | 79.1% | ✓ |
| Holdout Precision | 31.6% | ✗ |
| Holdout Coverage | 22.3% | ✓ |
| 95% CI Precision | [26.6%, 37.1%] | — |
| OOF→Holdout Gap | -47.5% | ⚠ degradation |
| Holdout Fired | 297 trades | ✓ |
| SELL Win Rate | 53.2% (79 trades) | ✗ |
| BUY Win Rate | 98.1% (53 trades) | ✓ |
| Sharpe (annualised) | 10.97 | ✓ |
| Max Drawdown | 700000000.00% | ✗ |
| Profit Factor | 1.92 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2578% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.33) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 337 | 181 | 156 | 53.8% | +231.88% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0588 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1595 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 31.6% | -0.43 | ⚠ overconfident |
| 80-90% | 41.6% | -0.43 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 163 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.159 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +47.49pp |
...
**Base precision:** 31.6%  →  **Expected precision (all fixes):** 37.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 297 |
| Rejected signals | 1037 |
| Selected precision | 31.6% |
| Rejected precision | 24.1% |
| Meta gate lift (precision) | +7.5% |
| Selected expectancy | +0.258% |
| Rejected expectancy | +0.312% |
| Selected Sharpe | +21.07 |
| Rejected Sharpe | +9.81 |
...
## Section 19 — Deep JTO/USDT vs BTC/ETH Comparison
