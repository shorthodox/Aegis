...
**Symbol:** ARKM/USDT  |  **Generated:** 2026-06-07 16:35:49
...
> Measured precision is 36.7% — estimated 3.4pp below achievable ceiling of ~40.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  36.7%
...
Achievable precision       :  ~40.1%  (+3.4pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 81 holdout BUY signals fired
   - PASS — 81 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=81, buy_win_rate=80.2%
   - PASS — 81 trades, 80.2% WR.
...
| Metric | Value |
|--------|-------|
| Gated-in precision | 36.7% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -17.1pp |
| OOF → Holdout gap | +62.8pp |
| thr_buy / thr_sell | 85.058 / 85.058 |
| Blocked signals | 0 (165 would-win / 143 would-lose) |

> ⚠ Gate is DESTROYING 17.1pp of precision. Blocked signals (53.8%) would have beaten gated (36.7%). Meta model is anti-selective. | OOF overfit warning: dev_prec (99.5%) exceeds holdout (36.7%) by 62.8pp.
...
**Symbol:** ARKM/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=36.7%  Sharpe=23.04
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 68.4% | ✓ |
| Dev OOF Precision | 99.5% | ✓ |
| Holdout Precision | 36.7% | ✗ |
| Holdout Coverage | 28.0% | ✓ |
| 95% CI Precision | [32.0%, 41.7%] | — |
| OOF→Holdout Gap | -62.8% | ⚠ degradation |
| Holdout Fired | 373 trades | ✓ |
| SELL Win Rate | 83.7% (86 trades) | ✓ |
| BUY Win Rate | 80.2% (81 trades) | ✓ |
| Sharpe (annualised) | 23.04 | ✓ |
| Max Drawdown | 5262362705.04% | ✗ |
| Profit Factor | 4.77 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.8766% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.13) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 308 | 165 | 143 | 53.8% | +211.93% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0818 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1461 | ~1.0 | ⚠ model overconfident |
...
**Recommended calibrator (for 218 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.146 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +62.81pp |
...
| Holdout Sharpe | 23.04 | ✓ |
| Holdout Max DD | 5262362705.04% | ✗ |
...
**Base precision:** 36.7%  →  **Expected precision (all fixes):** 42.5%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 373 |
| Rejected signals | 961 |
| Selected precision | 36.7% |
| Rejected precision | 27.3% |
| Meta gate lift (precision) | +9.5% |
| Selected expectancy | +0.877% |
| Rejected expectancy | +0.286% |
| Selected Sharpe | +39.48 |
...
## Section 19 — Deep ARKM/USDT vs BTC/ETH Comparison
