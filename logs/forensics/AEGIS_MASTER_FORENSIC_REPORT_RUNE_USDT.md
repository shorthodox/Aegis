...
**Symbol:** RUNE/USDT  |  **Generated:** 2026-06-07 16:39:33
...
> Measured precision is 29.6% — estimated 5.5pp below achievable ceiling of ~35.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.5pp), HOLD over-representation in labels (−1.6pp)

#### Precision Waterfall

```
Measured holdout precision :  29.6%
...
Achievable precision       :  ~35.1%  (+5.5pp precision)
Recall deficit (BUY side)  :  −2.1pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +3.7pp once retrained).
...
🔴 **Gate status: HURTING**  (lift: -24.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 29.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -24.2pp |
| OOF → Holdout gap | +53.7pp |
| thr_buy / thr_sell | 46.344 / 46.344 |
| Blocked signals | 0 (72 would-win / 63 would-lose) |

> ⚠ Gate is DESTROYING 24.2pp of precision. Blocked signals (53.8%) would have beaten gated (29.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (83.3%) exceeds holdout (29.6%) by 53.7pp.
...
**Symbol:** RUNE/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=29.6%  Sharpe=20.69
**Expected after fixes:** Precision≈35.4%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 60/100
   > HOLD=60.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 67.5% | ✓ |
| Dev OOF Precision | 83.3% | ✓ |
| Holdout Precision | 29.6% | ✗ |
| Holdout Coverage | 47.4% | ✓ |
| 95% CI Precision | [26.2%, 33.3%] | — |
| OOF→Holdout Gap | -53.7% | ⚠ degradation |
| Holdout Fired | 635 trades | ✓ |
| SELL Win Rate | 72.6% (226 trades) | ✓ |
| BUY Win Rate | 92.3% (26 trades) | ✓ |
| Sharpe (annualised) | 20.69 | ✓ |
| Max Drawdown | 188.82% | ✗ |
| Profit Factor | 3.22 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4899% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-10.28) | ✓ significant |

### Class Distribution
- HOLD: **60.4%** — ⚠ severe imbalance
- SELL: **24.6%**
- BUY:  **15.0%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 60.4% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 135 | 72 | 63 | 53.8% | +92.89% |
...
| 70-80% | 29.6% | -0.45 | ⚠ overconfident |
| 80-90% | 39.6% | -0.45 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 461 dev samples):** `isotonic`
...
**States:** 5  |  **Global precision:** 66.0%  |  **Max state concentration:** 42.9%
...
| Confidence Drift | 🟢 OK | T=1.094 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +53.69pp |
...
| Holdout Sharpe | 20.69 | ✓ |
| Holdout Max DD | 188.82% | ✗ |
...
**1 root causes identified.**  Combined top-5 impact score: **60/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 60/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=60.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 29.6%  →  **Expected precision (all fixes):** 35.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 635 |
| Rejected signals | 704 |
| Selected precision | 29.6% |
| Rejected precision | 25.4% |
| Meta gate lift (precision) | +4.2% |
| Selected expectancy | +0.490% |
| Rejected expectancy | +0.182% |
| Selected Sharpe | +27.17 |
| Rejected Sharpe | +14.51 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 46.3 | 46.3 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep RUNE/USDT vs BTC/ETH Comparison
