...
**Symbol:** CFX/USDT  |  **Generated:** 2026-06-07 16:35:54
...
> Measured precision is 31.9% — estimated 3.3pp below achievable ceiling of ~35.2%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  31.9%
...
Achievable precision       :  ~35.2%  (+3.3pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 9 holdout BUY signals fired
   - PASS — 9 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=9, buy_win_rate=88.9%
   - PASS — 9 trades, 88.9% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -21.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 31.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -21.9pp |
| OOF → Holdout gap | +54.2pp |
| thr_buy / thr_sell | 53.879 / 53.883 |
| Blocked signals | 0 (190 would-win / 164 would-lose) |

> ⚠ Gate is DESTROYING 21.9pp of precision. Blocked signals (53.8%) would have beaten gated (31.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (86.1%) exceeds holdout (31.9%) by 54.2pp.
...
**Symbol:** CFX/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=31.9%  Sharpe=10.06
**Expected after fixes:** Precision≈37.7%  (+5.8pp)
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 66.6% | ✓ |
| Dev OOF Precision | 86.1% | ✓ |
| Holdout Precision | 31.9% | ✗ |
| Holdout Coverage | 23.2% | ✓ |
| 95% CI Precision | [27.0%, 37.3%] | — |
| OOF→Holdout Gap | -54.2% | ⚠ degradation |
| Holdout Fired | 310 trades | ✓ |
| SELL Win Rate | 73.4% (124 trades) | ✓ |
| BUY Win Rate | 88.9% (9 trades) | ✓ |
| Sharpe (annualised) | 10.06 | ✓ |
| Max Drawdown | 282.67% | ✗ |
| Profit Factor | 1.81 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1875% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.36) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 354 | 190 | 164 | 53.8% | +243.58% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0965 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1909 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 31.9% | -0.43 | ⚠ overconfident |
| 80-90% | 41.9% | -0.43 | ⚠ overconfident |
...
**States:** 8  |  **Global precision:** 66.0%  |  **Max state concentration:** 43.5%
...
| Confidence Drift | 🟡 WARNING | T=1.191 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +54.20pp |
...
| Holdout Sharpe | 10.06 | ✓ |
| Holdout Max DD | 282.67% | ✗ |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 56/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=56.8% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 31.9%  →  **Expected precision (all fixes):** 37.7%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 310 |
| Rejected signals | 1024 |
| Selected precision | 31.9% |
| Rejected precision | 35.7% |
| Meta gate lift (precision) | -3.8% |
| Selected expectancy | +0.188% |
| Rejected expectancy | +0.541% |
| Selected Sharpe | +18.92 |
| Rejected Sharpe | +31.55 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 53.9 | 53.9 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep CFX/USDT vs BTC/ETH Comparison
