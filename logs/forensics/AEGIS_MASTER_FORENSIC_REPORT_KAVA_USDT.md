...
**Symbol:** KAVA/USDT  |  **Generated:** 2026-06-07 16:39:22
...
> Measured precision is 18.4% — estimated 8.8pp below achievable ceiling of ~27.2%. Primary contributors: Barrier skew suppresses BUY labels (−2.5pp), HOLD over-representation in labels (−2.9pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  18.4%
  ...
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.213 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~27.2%  (+8.8pp precision)
Recall deficit (BUY side)  :  −3.7pp  (signals not firing)
```
...
| Temperature T=1.213 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.213 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
❌ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=27, buy_win_rate=40.7%
   - PASS — 27 trades, 40.7% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -35.4pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 18.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -35.4pp |
| OOF → Holdout gap | +64.8pp |
| thr_buy / thr_sell | 46.701 / 46.701 |
| Blocked signals | 0 (163 would-win / 140 would-lose) |

> ⚠ Gate is DESTROYING 35.4pp of precision. Blocked signals (53.8%) would have beaten gated (18.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (83.2%) exceeds holdout (18.4%) by 64.8pp.
...
**Symbol:** KAVA/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=18.4%  Sharpe=-0.04
**Expected after fixes:** Precision≈24.2%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 69/100
   > HOLD=69.5% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 83.2% | ✓ |
| Holdout Precision | 18.4% | ✗ |
| Holdout Coverage | 30.7% | ✓ |
| 95% CI Precision | [15.0%, 22.5%] | — |
| OOF→Holdout Gap | -64.8% | ⚠ degradation |
| Holdout Fired | 412 trades | ✓ |
| SELL Win Rate | 67.0% (97 trades) | ✓ |
| BUY Win Rate | 40.7% (27 trades) | ✓ |
| Sharpe (annualised) | -0.04 | ✗ |
| Max Drawdown | 200000000.00% | ✗ |
| Profit Factor | 1.00 | ✗ |
| Kelly Fraction | 0.0% | — |
| Expectancy/Trade | -0.0006% | ✗ |
...
| Statistical Sig. | p=0.0000 (z=-12.81) | ✓ significant |

### Class Distribution
- HOLD: **69.5%** — ⚠ severe imbalance
- SELL: **18.8%**
- BUY:  **11.7%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 69.5% HOLD labels biases model toward neutrality.
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0586 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2130 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 18.4% | -0.57 | ⚠ overconfident |
| 80-90% | 28.4% | -0.57 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 161 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.213 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +64.78pp |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 69/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=69.5% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 18.4%  →  **Expected precision (all fixes):** 24.2%  (+5.8pp)
...
| Selected precision | 18.4% |
| Rejected precision | 22.0% |
| Meta gate lift (precision) | -3.6% |
| Selected expectancy | -0.001% |
| Rejected expectancy | +0.152% |
| Selected Sharpe | -0.07 |
| Rejected Sharpe | +13.57 |
...
## Section 19 — Deep KAVA/USDT vs BTC/ETH Comparison
