...
**Symbol:** FET/USDT  |  **Generated:** 2026-06-07 16:36:00
...
> Measured precision is 25.6% — estimated 3.3pp below achievable ceiling of ~28.9%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  25.6%
...
Achievable precision       :  ~28.9%  (+3.3pp precision)
...
✅ **1 issues already applied** in the codebase (expected gain: +1.2pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 181 BUY proposals / 336 total directional
   - PASS — primary fires BUY on some bars.

✅ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(144)=50 ≥ min_fires=35`
   - Value: 50 max fires vs 35 required
   - PASS — 50 ≥ 35.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 15 holdout BUY signals fired
   - PASS — 15 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=15, buy_win_rate=93.3%
   - PASS — 15 trades, 93.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -28.2pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 25.6% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -28.2pp |
| OOF → Holdout gap | +47.7pp |
| thr_buy / thr_sell | 85.084 / 46.423 |
| Blocked signals | 0 (154 would-win / 133 would-lose) |

> ⚠ Gate is DESTROYING 28.2pp of precision. Blocked signals (53.8%) would have beaten gated (25.6%). Meta model is anti-selective. | OOF overfit warning: dev_prec (73.3%) exceeds holdout (25.6%) by 47.7pp.
...
**Symbol:** FET/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=25.6%  Sharpe=1.82
**Expected after fixes:** Precision≈31.4%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 56/100
   > HOLD=56.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 64.9% | ⚠ |
| Dev OOF Precision | 73.3% | ✓ |
| Holdout Precision | 25.6% | ✗ |
| Holdout Coverage | 17.8% | ✓ |
| 95% CI Precision | [20.5%, 31.5%] | — |
| OOF→Holdout Gap | -47.7% | ⚠ degradation |
| Holdout Fired | 238 trades | ✓ |
| SELL Win Rate | 54.6% (86 trades) | ✗ |
| BUY Win Rate | 93.3% (15 trades) | ✓ |
| Sharpe (annualised) | 1.82 | ✓ |
| Max Drawdown | 1730927527.59% | ✗ |
| Profit Factor | 1.13 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0372% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-7.52) | ✓ significant |

### Class Distribution
- HOLD: **56.4%** — ⚠ severe imbalance
- SELL: **26.5%**
- BUY:  **17.1%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 56.4% HOLD labels biases model toward neutrality.
...
```
Generated (directional):       336  (100%)
Blocked by Meta Gate:      -  287  (85%)
Blocked by Quality (<55):  -   26
Blocked by HMM:            -   16
Blocked by Confluence:     -   13
Blocked by Fake Breakout:  -   10
Blocked by Portfolio Guard:-    6
...
Average hold-signal realized return: **2.2060%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 287 | 154 | 133 | 53.8% | +633.13% |
| quality | 26 | 16 | 10 | 61.5% | +57.36% ⚠ |
| hmm | 16 | 9 | 7 | 57.6% | +35.30% ⚠ |
| confluence | 13 | 7 | 6 | 54.5% | +28.68% |
| fake_breakout | 10 | 6 | 4 | 60.0% | +22.06% ⚠ |
| portfolio | 6 | 3 | 3 | 57.6% | +13.24% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0946 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1124 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 25.6% | -0.49 | ⚠ overconfident |
| 80-90% | 35.6% | -0.49 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 105 dev samples):** `isotonic`
...
**States:** 5  |  **Global precision:** 66.0%  |  **Max state concentration:** 48.6%
...
| Confidence Drift | 🟡 WARNING | T=1.112 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +47.70pp |
...
**1 root causes identified.**  Combined top-5 impact score: **56/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 56/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=56.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 25.6%  →  **Expected precision (all fixes):** 31.4%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 238 |
| Rejected signals | 1096 |
| Selected precision | 25.6% |
| Rejected precision | 28.6% |
| Meta gate lift (precision) | -3.0% |
| Selected expectancy | +0.037% |
| Rejected expectancy | +0.373% |
| Selected Sharpe | +3.91 |
| Rejected Sharpe | +19.10 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 85.1 | 46.4 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep FET/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 56.7% |
| Actual Precision | 25.6% |
| Gap | -31.1% |
| Coverage | 4.8% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 47/100 |
