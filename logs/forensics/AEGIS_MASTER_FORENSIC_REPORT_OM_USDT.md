...
**Symbol:** OM/USDT  |  **Generated:** 2026-06-07 16:39:29
...
> Measured precision is 42.5% — estimated 7.2pp below achievable ceiling of ~49.7%. Primary contributors: HOLD over-representation in labels (−2.6pp), Barrier skew suppresses BUY labels (−2.2pp)

#### Precision Waterfall

```
Measured holdout precision :  42.5%
...
Achievable precision       :  ~49.7%  (+7.2pp precision)
...
✅ **3 issues already applied** in the codebase (expected gain: +8.2pp once retrained).
...
🔴 **Gate status: HURTING**  (lift: -11.4pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 42.5% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -11.4pp |
| OOF → Holdout gap | +16.5pp |
| thr_buy / thr_sell | 0.000 / 0.000 |
| Blocked signals | 0 (204 would-win / 176 would-lose) |

> ⚠ Gate is DESTROYING 11.4pp of precision. Blocked signals (53.8%) would have beaten gated (42.5%). Meta model is anti-selective. | OOF overfit warning: dev_prec (59.0%) exceeds holdout (42.5%) by 16.5pp.
...
**Symbol:** OM/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=42.5%  Sharpe=8.66
**Expected after fixes:** Precision≈48.2%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 67/100
...
| Holdout Precision | 42.5% | ✗ |
| Holdout Coverage | 26.7% | ✓ |
| 95% CI Precision | [37.4%, 47.6%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 358 trades | ✓ |
| SELL Win Rate | 81.6% (38 trades) | ✓ |
| BUY Win Rate | 0.0% (0 trades) | ✗ disabled |
| Sharpe (annualised) | 8.66 | ✓ |
| Max Drawdown | 3.34% | ✓ |
| Profit Factor | 2.78 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2563% | ✓ |
...
| Statistical Sig. | p=0.0043 (z=-2.85) | ✓ significant |
...
**BUY side:** ✗ DISABLED  |  **SELL side:** ✓ ENABLED
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 380 | 204 | 176 | 53.8% | +261.47% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0811 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0000 | ~1.0 | ✓ |
...
| 70-80% | 66.0% | -0.09 | ⚠ overconfident |
| 80-90% | 76.0% | -0.09 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 81 dev samples):** `temperature`
...
**States:** 9  |  **Global precision:** 66.0%  |  **Max state concentration:** 34.4%
...
| Confidence Drift | 🟢 OK | T=1.000 |
| Prediction Drift | 🟡 WARNING | OOF vs holdout gap: +7.00pp |
...
**2 root causes identified.**  Combined top-5 impact score: **155/500**
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 67/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=67.1% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 42.5%  →  **Expected precision (all fixes):** 48.2%  (+5.8pp)
...
| Selected signals | 0 |
| Rejected signals | 0 |
| Selected precision | 66.0% |
| Rejected precision | 50.0% |
| Meta gate lift (precision) | +16.0% |
| Selected expectancy | +0.000% |
| Rejected expectancy | +0.000% |
| Selected Sharpe | +0.00 |
| Rejected Sharpe | +0.00 |
...
## Section 19 — Deep OM/USDT vs BTC/ETH Comparison
...
|--------|-------|
| Gate Lift (pp) | -12.6% |
| Selected signals | 155 |
| Rejected signals | 1726 |
| Gate coverage | 8.2% |
| Status | DEGRADED (-20pp to -10pp) |
...
| Metric | Value |
|--------|-------|
| Precision Target | 62.0% |
| Actual Precision | 37.4% |
| Gap | -24.6% |
| Coverage | 8.0% |
