...
**Symbol:** PYTH/USDT  |  **Generated:** 2026-06-07 16:39:32
...
> Measured precision is 41.0% — estimated 4.2pp below achievable ceiling of ~45.2%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.8pp), Temperature T=1.270 — overconfidence (−1.0pp)

#### Precision Waterfall

```
Measured holdout precision :  41.0%

  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [✅ FIXED       ]  −0.8pp prec    Barrier skew suppresses BUY labels
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.270 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~45.2%  (+4.2pp precision)
Recall deficit (BUY side)  :  −0.8pp  (signals not firing)
```
...
| Temperature T=1.270 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.270 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **1 issues already applied** in the codebase (expected gain: +1.0pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 59 BUY proposals / 64 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(47)=16 ≥ min_fires=35`
   - Value: 16 max fires vs 35 required
   - FAIL (FIXED) — 16 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 12 holdout BUY signals fired
   - PASS — 12 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=12, buy_win_rate=91.7%
   - PASS — 12 trades, 91.7% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -12.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 41.0% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -12.9pp |
| OOF → Holdout gap | +52.2pp |
| thr_buy / thr_sell | 83.856 / 83.806 |
| Blocked signals | 0 (26 would-win / 24 would-lose) |

> ⚠ Gate is DESTROYING 12.9pp of precision. Blocked signals (53.8%) would have beaten gated (41.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (93.2%) exceeds holdout (41.0%) by 52.2pp.
...
**Symbol:** PYTH/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=41.0%  Sharpe=15.36
**Expected after fixes:** Precision≈46.7%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 56/100
   > HOLD=56.1% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 56.2% | ⚠ |
| Dev OOF Precision | 93.2% | ✓ |
| Holdout Precision | 41.0% | ✗ |
| Holdout Coverage | 19.9% | ✓ |
| 95% CI Precision | [35.2%, 47.0%] | — |
| OOF→Holdout Gap | -52.2% | ⚠ degradation |
| Holdout Fired | 266 trades | ✓ |
| SELL Win Rate | 73.1% (134 trades) | ✓ |
| BUY Win Rate | 91.7% (12 trades) | ✓ |
| Sharpe (annualised) | 15.36 | ✓ |
| Max Drawdown | 216.61% | ✗ |
| Profit Factor | 2.82 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.7497% | ✓ |
...
| Statistical Sig. | p=0.0032 (z=-2.94) | ✓ significant |

### Class Distribution
- HOLD: **56.1%** — ⚠ severe imbalance
- SELL: **26.4%**
- BUY:  **17.5%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 56.1% HOLD labels biases model toward neutrality.
...
Average hold-signal realized return: **2.1221%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 50 | 26 | 24 | 53.8% | +106.11% |
| quality | 5 | 3 | 2 | 61.5% | +10.61% ⚠ |
| hmm | 3 | 1 | 2 | 57.6% | +6.37% ⚠ |
| confluence | 2 | 1 | 1 | 54.5% | +4.24% |
| fake_breakout | 1 | 0 | 1 | 60.0% | +2.12% ⚠ |
| portfolio | 1 | 0 | 1 | 57.6% | +2.12% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0561 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2701 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 41.0% | -0.34 | ⚠ overconfident |
| 80-90% | 51.0% | -0.34 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 147 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.270 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +52.22pp |
...
| Holdout Sharpe | 15.36 | ✓ |
| Holdout Max DD | 216.61% | ✗ |
...
**2 root causes identified.**  Combined top-5 impact score: **118/500**
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 56/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=56.1% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 41.0%  →  **Expected precision (all fixes):** 46.7%  (+5.8pp)
...
| Rejected signals | 1072 |
| Selected precision | 41.0% |
| Rejected precision | 34.8% |
| Meta gate lift (precision) | +6.2% |
| Selected expectancy | +0.750% |
| Rejected expectancy | +0.342% |
| Selected Sharpe | +31.17 |
| Rejected Sharpe | +23.27 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 83.9 | 83.8 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep PYTH/USDT vs BTC/ETH Comparison
...
| Actual Precision | 41.0% |
| Gap | -15.9% |
| Coverage | 6.9% |
| Gating Strategy | ADAPTIVE_PER_REGIME |
| Gate Trust Score | 56/100 |
