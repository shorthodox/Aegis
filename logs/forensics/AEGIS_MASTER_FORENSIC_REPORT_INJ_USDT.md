...
**Symbol:** INJ/USDT  |  **Generated:** 2026-06-07 16:38:31
...
> Measured precision is 37.3% — estimated 7.2pp below achievable ceiling of ~44.5%. Primary contributors: HOLD over-representation in labels (−2.7pp), Barrier skew suppresses BUY labels (−2.1pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  37.3%
...
Achievable precision       :  ~44.5%  (+7.2pp precision)
Recall deficit (BUY side)  :  −3.2pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +5.8pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 65 BUY proposals / 65 total directional
   - PASS — primary fires BUY on some bars.

❌ **2. pick_threshold_by_side(BUY) can qualify**
   - `scripts/retrain_model.py:1363-1397`
   - Check: `MAX_SIDE_COVERAGE=0.35×pool(52)=18 ≥ min_fires=35`
   - Value: 18 max fires vs 35 required
   - FAIL (FIXED) — 18 < 35. Deadlock: every quantile rejected before precision is checked. Fix: MAX_SIDE_COVERAGE→0.35 + adaptive effective_min_fires.
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=12, buy_win_rate=83.3%
   - PASS — 12 trades, 83.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -16.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 37.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -16.5pp |
| OOF → Holdout gap | +51.8pp |
| thr_buy / thr_sell | 70.202 / 70.134 |
| Blocked signals | 0 (29 would-win / 25 would-lose) |

> ⚠ Gate is DESTROYING 16.5pp of precision. Blocked signals (53.8%) would have beaten gated (37.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (89.2%) exceeds holdout (37.3%) by 51.8pp.
...
**Symbol:** INJ/USDT  |  **Audit:** 2026-06-07 16:38  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=37.3%  Sharpe=13.47
**Expected after fixes:** Precision≈43.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 67/100
   > HOLD=67.8% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 65.5% | ✓ |
| Dev OOF Precision | 89.2% | ✓ |
| Holdout Precision | 37.3% | ✗ |
| Holdout Coverage | 11.8% | ✓ |
| 95% CI Precision | [30.2%, 45.1%] | — |
| OOF→Holdout Gap | -51.8% | ⚠ degradation |
| Holdout Fired | 158 trades | ✓ |
| SELL Win Rate | 90.7% (54 trades) | ✓ |
| BUY Win Rate | 83.3% (12 trades) | ✓ |
| Sharpe (annualised) | 13.47 | ✓ |
| Max Drawdown | 2498394463.63% | ✗ |
| Profit Factor | 4.40 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4628% | ✓ |
...
| Statistical Sig. | p=0.0015 (z=-3.18) | ✓ significant |

### Class Distribution
- HOLD: **67.8%** — ⚠ severe imbalance
- SELL: **19.1%**
- BUY:  **13.1%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 67.8% HOLD labels biases model toward neutrality.
...
```
Generated (directional):        65  (100%)
Blocked by Meta Gate:      -   54  (83%)
...
Average hold-signal realized return: **2.0732%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 54 | 29 | 25 | 53.8% | +111.95% |
| quality | 5 | 3 | 2 | 61.5% | +10.37% ⚠ |
| hmm | 3 | 1 | 2 | 57.6% | +6.22% ⚠ |
| confluence | 2 | 1 | 1 | 54.5% | +4.15% |
| fake_breakout | 1 | 0 | 1 | 60.0% | +2.07% ⚠ |
| portfolio | 1 | 0 | 1 | 57.6% | +2.07% ⚠ |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0751 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0892 | ~1.0 | ✓ |
...
| 70-80% | 37.3% | -0.38 | ⚠ overconfident |
| 80-90% | 47.3% | -0.38 | ⚠ overconfident |
...
| Confidence Drift | 🟢 OK | T=1.089 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +51.81pp |
...
| Holdout Sharpe | 13.47 | ✓ |
| Holdout Max DD | 2498394463.63% | ✗ |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 67/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=67.8% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 37.3%  →  **Expected precision (all fixes):** 43.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 158 |
| Rejected signals | 1184 |
| Selected precision | 37.3% |
| Rejected precision | 21.3% |
| Meta gate lift (precision) | +16.1% |
| Selected expectancy | +0.463% |
| Rejected expectancy | +0.298% |
| Selected Sharpe | +35.46 |
| Rejected Sharpe | +17.16 |
...
## Section 19 — Deep INJ/USDT vs BTC/ETH Comparison
