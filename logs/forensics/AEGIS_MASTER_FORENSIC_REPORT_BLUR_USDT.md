...
**Symbol:** BLUR/USDT  |  **Generated:** 2026-06-07 16:35:52
...
> Measured precision is 37.9% — estimated 3.1pp below achievable ceiling of ~41.0%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.7pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  37.9%
...
Achievable precision       :  ~41.0%  (+3.1pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 33 holdout BUY signals fired
   - PASS — 33 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=33, buy_win_rate=57.6%
   - PASS — 33 trades, 57.6% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -16.0pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 37.9% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -16.0pp |
| OOF → Holdout gap | +41.5pp |
| thr_buy / thr_sell | 46.473 / 46.473 |
| Blocked signals | 0 (107 would-win / 93 would-lose) |

> ⚠ Gate is DESTROYING 16.0pp of precision. Blocked signals (53.8%) would have beaten gated (37.9%). Meta model is anti-selective. | OOF overfit warning: dev_prec (79.3%) exceeds holdout (37.9%) by 41.5pp.
...
**Symbol:** BLUR/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=37.9%  Sharpe=25.74
**Expected after fixes:** Precision≈43.6%  (+5.8pp)
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 66.2% | ✓ |
| Dev OOF Precision | 79.3% | ✓ |
| Holdout Precision | 37.9% | ✗ |
| Holdout Coverage | 41.1% | ✓ |
| 95% CI Precision | [33.9%, 42.0%] | — |
| OOF→Holdout Gap | -41.5% | ⚠ degradation |
| Holdout Fired | 549 trades | ✓ |
| SELL Win Rate | 83.3% (227 trades) | ✓ |
| BUY Win Rate | 57.6% (33 trades) | ✓ |
| Sharpe (annualised) | 25.74 | ✓ |
| Max Drawdown | 700000000.00% | ✗ |
| Profit Factor | 3.52 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.5825% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-5.68) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 200 | 107 | 93 | 53.8% | +137.62% |
...
| Cal. Temperature | 1.1152 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 37.9% | -0.37 | ⚠ overconfident |
| 80-90% | 47.9% | -0.37 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 402 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.115 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +41.47pp |
...
| Holdout Sharpe | 25.74 | ✓ |
| Holdout Max DD | 700000000.00% | ✗ |
...
**Base precision:** 37.9%  →  **Expected precision (all fixes):** 43.6%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 549 |
| Rejected signals | 785 |
| Selected precision | 37.9% |
| Rejected precision | 24.7% |
| Meta gate lift (precision) | +13.2% |
| Selected expectancy | +0.583% |
| Rejected expectancy | +0.346% |
| Selected Sharpe | +36.35 |
| Rejected Sharpe | +13.39 |
...
## Section 19 — Deep BLUR/USDT vs BTC/ETH Comparison
