...
**Symbol:** TIA/USDT  |  **Generated:** 2026-06-07 16:39:56
...
> Measured precision is 21.2% — estimated 7.2pp below achievable ceiling of ~28.4%. Primary contributors: Barrier skew suppresses BUY labels (−1.8pp), HOLD over-representation in labels (−2.0pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  21.2%

  [✅ FIXED       ]  −1.8pp prec    Barrier skew suppresses BUY labels
  [✅ FIXED       ]  −2.0pp prec    HOLD over-representation in labels
  [🔴 ACTIVE      ]  −2.0pp prec    Brier score above target
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=1.236 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~28.4%  (+7.2pp precision)
Recall deficit (BUY side)  :  −2.6pp  (signals not firing)
```
...
| Temperature T=1.236 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=1.236 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **2 issues already applied** in the codebase (expected gain: +4.6pp once retrained).
...
✅ **1. Primary model generates BUY labels**
   ...
   - Value: 4 BUY proposals / 26 total directional
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 73 holdout BUY signals fired
   - PASS — 73 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=73, buy_win_rate=83.6%
   - PASS — 73 trades, 83.6% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -32.7pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 21.2% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -32.7pp |
| OOF → Holdout gap | +75.6pp |
| thr_buy / thr_sell | 84.615 / 84.258 |
| Blocked signals | 0 (12 would-win / 11 would-lose) |

> ⚠ Gate is DESTROYING 32.7pp of precision. Blocked signals (53.8%) would have beaten gated (21.2%). Meta model is anti-selective. | OOF overfit warning: dev_prec (96.8%) exceeds holdout (21.2%) by 75.6pp.
...
**Symbol:** TIA/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=21.2%  Sharpe=11.27
**Expected after fixes:** Precision≈26.9%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 63/100
   > HOLD=63.2% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 59.7% | ⚠ |
| Dev OOF Precision | 96.8% | ✓ |
| Holdout Precision | 21.2% | ✗ |
| Holdout Coverage | 32.2% | ✓ |
| 95% CI Precision | [17.6%, 25.3%] | — |
| OOF→Holdout Gap | -75.6% | ⚠ degradation |
| Holdout Fired | 430 trades | ✓ |
| SELL Win Rate | 62.5% (48 trades) | ✓ |
| BUY Win Rate | 83.6% (73 trades) | ✓ |
| Sharpe (annualised) | 11.27 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 2.19 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.5422% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-11.96) | ✓ significant |

### Class Distribution
- HOLD: **63.2%** — ⚠ severe imbalance
- SELL: **22.7%**
- BUY:  **14.2%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 63.2% HOLD labels biases model toward neutrality.
...
Average hold-signal realized return: **2.1567%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 23 | 12 | 11 | 53.8% | +49.60% |
| quality | 2 | 1 | 1 | 61.5% | +4.31% ⚠ |
| hmm | 1 | 0 | 1 | 57.6% | +2.16% ⚠ |
| confluence | 1 | 0 | 1 | 54.5% | +2.16% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0521 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.2357 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 21.2% | -0.54 | ⚠ overconfident |
| 80-90% | 31.2% | -0.54 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 62 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.236 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +75.61pp |
...
| Holdout Sharpe | 11.27 | ✓ |
| Holdout Max DD | 100000000.00% | ✗ |
...
**2 root causes identified.**  Combined top-5 impact score: **125/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 63/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=63.2% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 21.2%  →  **Expected precision (all fixes):** 26.9%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 430 |
| Rejected signals | 904 |
| Selected precision | 21.2% |
| Rejected precision | 25.7% |
| Meta gate lift (precision) | -4.5% |
| Selected expectancy | +0.542% |
| Rejected expectancy | +0.526% |
| Selected Sharpe | +17.99 |
| Rejected Sharpe | +17.53 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 84.6 | 84.3 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep TIA/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 56.0% |
| Actual Precision | 21.2% |
| Gap | -34.8% |
| Coverage | 3.3% |
