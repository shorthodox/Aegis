...
**Symbol:** EGLD/USDT  |  **Generated:** 2026-06-07 16:35:57
...
> Measured precision is 29.3% — estimated 6.9pp below achievable ceiling of ~36.2%. Primary contributors: Barrier skew suppresses BUY labels (−2.2pp), HOLD over-representation in labels (−2.3pp)

#### Precision Waterfall

```
Measured holdout precision :  29.3%
...
Achievable precision       :  ~36.2%  (+6.9pp precision)
Recall deficit (BUY side)  :  −11.1pp  (signals not firing)
```
...
✅ **3 issues already applied** in the codebase (expected gain: +7.8pp once retrained).
...
🔴 **Gate status: HURTING**  (lift: -24.6pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 29.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -24.6pp |
| OOF → Holdout gap | +58.3pp |
| thr_buy / thr_sell | 73.318 / 46.636 |
| Blocked signals | 0 (43 would-win / 37 would-lose) |

> ⚠ Gate is DESTROYING 24.6pp of precision. Blocked signals (53.8%) would have beaten gated (29.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (87.6%) exceeds holdout (29.3%) by 58.3pp.
...
**Symbol:** EGLD/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=29.3%  Sharpe=17.59
**Expected after fixes:** Precision≈35.0%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 65/100
   > HOLD=65.4% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 67.2% | ✓ |
| Dev OOF Precision | 87.6% | ✓ |
| Holdout Precision | 29.3% | ✗ |
| Holdout Coverage | 50.9% | ✓ |
| 95% CI Precision | [26.0%, 32.8%] | — |
| OOF→Holdout Gap | -58.3% | ⚠ degradation |
| Holdout Fired | 683 trades | ✓ |
| SELL Win Rate | 72.7% (275 trades) | ✓ |
| BUY Win Rate | 0.0% (0 trades) | ✗ disabled |
| Sharpe (annualised) | 17.59 | ✓ |
| Max Drawdown | 3398616790.19% | ✗ |
| Profit Factor | 2.23 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2504% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-10.83) | ✓ significant |

### Class Distribution
- HOLD: **65.4%** — ⚠ severe imbalance
- SELL: **21.8%**
- BUY:  **12.8%** — ⚠ minority class

### Issues Detected
- **CRITICAL** — BUY side fully disabled (buy_n=0). Model is SELL-only. Directional asymmetry score: 80/100.
- **WARNING** — Class imbalance: 65.4% HOLD labels biases model toward neutrality.
...
| 70-80% | 29.3% | -0.46 | ⚠ overconfident |
| 80-90% | 39.3% | -0.46 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 467 dev samples):** `isotonic`
...
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +58.30pp |
...
| Holdout Sharpe | 17.59 | ✓ |
| Holdout Max DD | 3398616790.19% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 65/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=65.4% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 29.3%  →  **Expected precision (all fixes):** 35.0%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 683 |
| Rejected signals | 659 |
| Selected precision | 29.3% |
| Rejected precision | 21.1% |
| Meta gate lift (precision) | +8.2% |
| Selected expectancy | +0.250% |
| Rejected expectancy | +0.069% |
| Selected Sharpe | +22.27 |
| Rejected Sharpe | +6.55 |
...
## Section 19 — Deep EGLD/USDT vs BTC/ETH Comparison
