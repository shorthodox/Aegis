...
**Symbol:** BOME/USDT  |  **Generated:** 2026-06-07 16:35:52
...
> Measured precision is 26.0% — estimated 4.3pp below achievable ceiling of ~30.3%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Temperature T=0.718 — overconfidence (−1.0pp)
...
Measured holdout precision :  26.0%
  ...
  [🔴 ACTIVE      ]  −1.0pp prec    Temperature T=0.718 — overconfidence
  [🔴 ACTIVE      ]  −0.4pp prec    Gate blocking signals that would have won

Achievable precision       :  ~30.3%  (+4.3pp precision)
...
| Temperature T=0.718 — overconfidence | `scripts/retrain_model.py` | 1909-1916 | `_hold_w = clip(_n_dir×0.5 / _n_hold` | 1.0pp | 0.0pp |
...
**2. Temperature T=0.718 — overconfidence**
> 📍 `scripts/retrain_model.py:1909-1916` — `_hold_w = clip(_n_dir×0.5 / _n_hold, 0.10, 0.60)`
> HOLD bars always have meta_y=0 (primary proposes BUY/SELL but true label=HOLD). With HOLD=66%, 60% of meta training targets are al…
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 3 holdout BUY signals fired
   - PASS — 3 BUY holdout trades.

❌ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=3, buy_win_rate=33.3%
   - PASS — 3 trades, 33.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -27.9pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 26.0% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -27.9pp |
| OOF → Holdout gap | +36.5pp |
| thr_buy / thr_sell | 46.519 / 46.519 |
| Blocked signals | 0 (226 would-win / 194 would-lose) |

> ⚠ Gate is DESTROYING 27.9pp of precision. Blocked signals (53.8%) would have beaten gated (26.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (62.5%) exceeds holdout (26.0%) by 36.5pp.
...
**Symbol:** BOME/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=26.0%  Sharpe=0.19
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 44.9% | ⚠ |
| Dev OOF Precision | 62.5% | ✓ |
| Holdout Precision | 26.0% | ✗ |
| Holdout Coverage | 3.8% | ✓ |
| 95% CI Precision | [15.9%, 39.6%] | — |
| OOF→Holdout Gap | -36.5% | ⚠ degradation |
| Holdout Fired | 50 trades | ✓ |
| SELL Win Rate | 60.0% (20 trades) | ✓ |
| BUY Win Rate | 33.3% (3 trades) | ✓ |
| Sharpe (annualised) | 0.19 | ✗ |
| Max Drawdown | 800000000.00% | ✗ |
| Profit Factor | 1.03 | ✗ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.0183% | ✓ |
...
| Statistical Sig. | p=0.0007 (z=-3.39) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 420 | 226 | 194 | 53.8% | +289.00% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0513 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.7184 | ~1.0 | ✓ |
...
**Recommended calibrator (for 32 dev samples):** `platt`
...
| Confidence Drift | 🟢 OK | T=0.718 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +36.50pp |
...
|--------|-------|
| Selected signals | 50 |
| Rejected signals | 1284 |
| Selected precision | 26.0% |
| Rejected precision | 23.8% |
| Meta gate lift (precision) | +2.2% |
| Selected expectancy | +0.018% |
| Rejected expectancy | +0.441% |
| Selected Sharpe | +0.89 |
| Rejected Sharpe | +16.21 |
...
## Section 19 — Deep BOME/USDT vs BTC/ETH Comparison
