...
**Symbol:** CRV/USDT  |  **Generated:** 2026-06-07 16:35:56
...
> Measured precision is 38.8% — estimated 3.7pp below achievable ceiling of ~42.5%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.3pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  38.8%
...
Achievable precision       :  ~42.5%  (+3.7pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 7 holdout BUY signals fired
   - PASS — 7 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=7, buy_win_rate=57.1%
   - PASS — 7 trades, 57.1% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -15.1pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 38.8% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -15.1pp |
| OOF → Holdout gap | +55.9pp |
| thr_buy / thr_sell | 78.889 / 78.998 |
| Blocked signals | 0 (184 would-win / 158 would-lose) |

> ⚠ Gate is DESTROYING 15.1pp of precision. Blocked signals (53.8%) would have beaten gated (38.8%). Meta model is anti-selective. | OOF overfit warning: dev_prec (94.7%) exceeds holdout (38.8%) by 55.9pp.
...
**Symbol:** CRV/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=38.8%  Sharpe=18.88
**Expected after fixes:** Precision≈44.5%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 59/100
   > HOLD=60.0% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 94.7% | ✓ |
| Holdout Precision | 38.8% | ✗ |
| Holdout Coverage | 17.9% | ✓ |
| 95% CI Precision | [32.8%, 45.0%] | — |
| OOF→Holdout Gap | -55.9% | ⚠ degradation |
| Holdout Fired | 240 trades | ✓ |
| SELL Win Rate | 84.8% (105 trades) | ✓ |
| BUY Win Rate | 57.1% (7 trades) | ✓ |
| Sharpe (annualised) | 18.88 | ✓ |
| Max Drawdown | 64.10% | ✗ |
| Profit Factor | 4.11 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.6584% | ✓ |
...
### Class Distribution
- HOLD: **60.0%** — ⚠ severe imbalance
- SELL: **24.3%**
- BUY:  **15.8%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 60.0% HOLD labels biases model toward neutrality.
...
| 70-80% | 38.8% | -0.36 | ⚠ overconfident |
| 80-90% | 48.8% | -0.36 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 150 dev samples):** `isotonic`
...
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +55.92pp |
...
| Holdout Sharpe | 18.88 | ✓ |
| Holdout Max DD | 64.10% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 59/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=60.0% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 38.8%  →  **Expected precision (all fixes):** 44.5%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 240 |
| Rejected signals | 1097 |
| Selected precision | 38.8% |
| Rejected precision | 21.3% |
| Meta gate lift (precision) | +17.4% |
| Selected expectancy | +0.658% |
| Rejected expectancy | +0.090% |
| Selected Sharpe | +40.32 |
| Rejected Sharpe | +6.50 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 78.9 | 79.0 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep CRV/USDT vs BTC/ETH Comparison
