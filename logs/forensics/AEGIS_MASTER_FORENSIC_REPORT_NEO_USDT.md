...
**Symbol:** NEO/USDT  |  **Generated:** 2026-06-07 16:39:28
...
> Measured precision is 40.3% — estimated 3.3pp below achievable ceiling of ~43.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−0.9pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  40.3%
...
Achievable precision       :  ~43.6%  (+3.3pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 69 holdout BUY signals fired
   - PASS — 69 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=69, buy_win_rate=79.7%
   - PASS — 69 trades, 79.7% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -13.5pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 40.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -13.5pp |
| OOF → Holdout gap | +54.6pp |
| thr_buy / thr_sell | 46.569 / 85.035 |
| Blocked signals | 0 (185 would-win / 160 would-lose) |

> ⚠ Gate is DESTROYING 13.5pp of precision. Blocked signals (53.8%) would have beaten gated (40.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (94.9%) exceeds holdout (40.3%) by 54.6pp.
...
**Symbol:** NEO/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=40.3%  Sharpe=13.37
**Expected after fixes:** Precision≈46.1%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 55/100
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 53.7% | ⚠ |
| Dev OOF Precision | 94.9% | ✓ |
| Holdout Precision | 40.3% | ✗ |
| Holdout Coverage | 17.5% | ✓ |
| 95% CI Precision | [34.2%, 46.8%] | — |
| OOF→Holdout Gap | -54.6% | ⚠ degradation |
| Holdout Fired | 233 trades | ✓ |
| SELL Win Rate | 78.0% (50 trades) | ✓ |
| BUY Win Rate | 79.7% (69 trades) | ✓ |
| Sharpe (annualised) | 13.37 | ✓ |
| Max Drawdown | 70.22% | ✗ |
| Profit Factor | 2.63 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2981% | ✓ |
...
| Statistical Sig. | p=0.0032 (z=-2.95) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 345 | 185 | 160 | 53.8% | +237.39% |
...
| 70-80% | 40.3% | -0.35 | ⚠ overconfident |
| 80-90% | 50.3% | -0.35 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 157 dev samples):** `isotonic`
...
**States:** 7  |  **Global precision:** 66.0%  |  **Max state concentration:** 46.0%
...
| Confidence Drift | 🟢 OK | T=0.940 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +54.56pp |
...
| Holdout Sharpe | 13.37 | ✓ |
| Holdout Max DD | 70.22% | ✗ |
...
**1 root causes identified.**  Combined top-5 impact score: **55/500**

| Rank | Cause | Category | Score | Source | Evidence |
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 55/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=55.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 40.3%  →  **Expected precision (all fixes):** 46.1%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 233 |
| Rejected signals | 1101 |
| Selected precision | 40.3% |
| Rejected precision | 32.9% |
| Meta gate lift (precision) | +7.5% |
| Selected expectancy | +0.298% |
| Rejected expectancy | +0.223% |
| Selected Sharpe | +28.99 |
| Rejected Sharpe | +19.99 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 46.6 | 85.0 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep NEO/USDT vs BTC/ETH Comparison
