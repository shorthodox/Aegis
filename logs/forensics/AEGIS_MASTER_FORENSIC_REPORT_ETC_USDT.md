...
**Symbol:** ETC/USDT  |  **Generated:** 2026-06-07 16:36:00
...
> Measured precision is 32.2% — estimated 3.4pp below achievable ceiling of ~35.6%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  32.2%
...
Achievable precision       :  ~35.6%  (+3.4pp precision)
...
❌ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=11, buy_win_rate=27.3%
   - PASS — 11 trades, 27.3% WR.
...
| Metric | Value |
|--------|-------|
| Gated-in precision | 32.2% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -21.6pp |
| OOF → Holdout gap | +59.7pp |
| thr_buy / thr_sell | 80.368 / 80.022 |
| Blocked signals | 0 (172 would-win / 148 would-lose) |

> ⚠ Gate is DESTROYING 21.6pp of precision. Blocked signals (53.8%) would have beaten gated (32.2%). Meta model is anti-selective. | OOF overfit warning: dev_prec (92.0%) exceeds holdout (32.2%) by 59.7pp.
...
**Symbol:** ETC/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=32.2%  Sharpe=15.29
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 55/100
   > HOLD=55.8% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 92.0% | ✓ |
| Holdout Precision | 32.2% | ✗ |
| Holdout Coverage | 24.9% | ✓ |
| 95% CI Precision | [27.4%, 37.4%] | — |
| OOF→Holdout Gap | -59.7% | ⚠ degradation |
| Holdout Fired | 332 trades | ✓ |
| SELL Win Rate | 78.2% (133 trades) | ✓ |
| BUY Win Rate | 27.3% (11 trades) | ✓ |
| Sharpe (annualised) | 15.29 | ✓ |
| Max Drawdown | 112.81% | ✗ |
| Profit Factor | 2.69 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.3334% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.48) | ✓ significant |

### Class Distribution
- HOLD: **55.8%** — ⚠ severe imbalance
- SELL: **27.4%**
- BUY:  **16.8%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 55.8% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 320 | 172 | 148 | 53.8% | +220.19% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0862 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.0331 | ~1.0 | ✓ |
...
| 70-80% | 32.2% | -0.43 | ⚠ overconfident |
| 80-90% | 42.2% | -0.43 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 199 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=1.033 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +59.73pp |
...
| Holdout Sharpe | 15.29 | ✓ |
| Holdout Max DD | 112.81% | ✗ |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 55/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=55.8% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 32.2%  →  **Expected precision (all fixes):** 38.0%  (+5.8pp)
...
| Rejected signals | 1002 |
| Selected precision | 32.2% |
| Rejected precision | 27.0% |
| Meta gate lift (precision) | +5.3% |
| Selected expectancy | +0.333% |
| Rejected expectancy | +0.039% |
| Selected Sharpe | +27.77 |
| Rejected Sharpe | +4.08 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 80.4 | 80.0 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ETC/USDT vs BTC/ETH Comparison
...
| Metric | Value |
|--------|-------|
| Precision Target | 57.7% |
| Actual Precision | 32.2% |
| Gap | -25.5% |
| Coverage | 9.0% |
