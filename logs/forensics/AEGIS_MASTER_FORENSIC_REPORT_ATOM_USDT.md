...
**Symbol:** ATOM/USDT  |  **Generated:** 2026-06-07 16:35:49
...
> Measured precision is 31.4% — estimated 5.4pp below achievable ceiling of ~36.8%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.4pp), HOLD over-representation in labels (−1.6pp)

#### Precision Waterfall

```
Measured holdout precision :  31.4%
...
Achievable precision       :  ~36.8%  (+5.4pp precision)
...
✅ **5. tradeable_buy_holdout = True**
   ...
   - Value: buy_h_n=13, buy_win_rate=92.3%
   - PASS — 13 trades, 92.3% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -22.4pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 31.4% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -22.4pp |
| OOF → Holdout gap | +63.1pp |
| thr_buy / thr_sell | 80.574 / 80.503 |
| Blocked signals | 0 (184 would-win / 158 would-lose) |

> ⚠ Gate is DESTROYING 22.4pp of precision. Blocked signals (53.8%) would have beaten gated (31.4%). Meta model is anti-selective. | OOF overfit warning: dev_prec (94.6%) exceeds holdout (31.4%) by 63.1pp.
...
**Symbol:** ATOM/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=31.4%  Sharpe=9.51
**Expected after fixes:** Precision≈37.2%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 60/100
   > HOLD=60.6% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 60.4% | ⚠ |
| Dev OOF Precision | 94.6% | ✓ |
| Holdout Precision | 31.4% | ✗ |
| Holdout Coverage | 22.3% | ✓ |
| 95% CI Precision | [26.4%, 36.9%] | — |
| OOF→Holdout Gap | -63.2% | ⚠ degradation |
| Holdout Fired | 299 trades | ✓ |
| SELL Win Rate | 76.6% (107 trades) | ✓ |
| BUY Win Rate | 92.3% (13 trades) | ✓ |
| Sharpe (annualised) | 9.51 | ✓ |
| Max Drawdown | 93.84% | ✗ |
| Profit Factor | 1.94 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.2358% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.42) | ✓ significant |

### Class Distribution
- HOLD: **60.6%** — ⚠ severe imbalance
- SELL: **24.1%**
- BUY:  **15.4%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 60.6% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 342 | 184 | 158 | 53.8% | +235.33% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0738 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 1.1903 | ~1.0 | ⚠ model overconfident |
...
| 70-80% | 31.4% | -0.44 | ⚠ overconfident |
| 80-90% | 41.4% | -0.44 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 148 dev samples):** `isotonic`
...
| Confidence Drift | 🟡 WARNING | T=1.190 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +63.16pp |
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 60/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=60.6% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 31.4%  →  **Expected precision (all fixes):** 37.2%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 299 |
| Rejected signals | 1041 |
| Selected precision | 31.4% |
| Rejected precision | 25.9% |
| Meta gate lift (precision) | +5.5% |
| Selected expectancy | +0.236% |
| Rejected expectancy | +0.124% |
| Selected Sharpe | +18.19 |
| Rejected Sharpe | +13.16 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 80.6 | 80.5 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ATOM/USDT vs BTC/ETH Comparison
