...
**Symbol:** ALGO/USDT  |  **Generated:** 2026-06-07 16:35:47
...
> Measured precision is 41.8% — estimated 6.8pp below achievable ceiling of ~48.6%. Primary contributors: Barrier skew suppresses BUY labels (−2.1pp), HOLD over-representation in labels (−2.3pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  41.8%
...
Achievable precision       :  ~48.6%  (+6.8pp precision)
...
🔴 **Gate status: HURTING**  (lift: -12.0pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 41.8% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -12.0pp |
| OOF → Holdout gap | +45.5pp |
| thr_buy / thr_sell | 46.508 / 70.050 |
| Blocked signals | 0 (163 would-win / 140 would-lose) |

> ⚠ Gate is DESTROYING 12.0pp of precision. Blocked signals (53.8%) would have beaten gated (41.8%). Meta model is anti-selective. | OOF overfit warning: dev_prec (87.4%) exceeds holdout (41.8%) by 45.5pp.
...
**Symbol:** ALGO/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=41.8%  Sharpe=20.78
**Expected after fixes:** Precision≈47.6%  (+5.8pp)
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 60.1% | ⚠ |
| Dev OOF Precision | 87.4% | ✓ |
| Holdout Precision | 41.8% | ✗ |
| Holdout Coverage | 17.3% | ✓ |
| 95% CI Precision | [35.6%, 48.2%] | — |
| OOF→Holdout Gap | -45.6% | ⚠ degradation |
| Holdout Fired | 232 trades | ✓ |
| SELL Win Rate | 91.2% (102 trades) | ✓ |
| BUY Win Rate | 57.1% (7 trades) | ✓ |
| Sharpe (annualised) | 20.78 | ✓ |
| Max Drawdown | 1333526620.98% | ✗ |
| Profit Factor | 3.99 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.3784% | ✓ |
...
| Statistical Sig. | p=0.0126 (z=-2.50) | ✓ significant |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.1049 | <0.10 | ✗ overcalibrated |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9059 | ~1.0 | ✓ |
...
| 70-80% | 41.8% | -0.33 | ⚠ overconfident |
| 80-90% | 51.8% | -0.33 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 174 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=0.906 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +45.55pp |
...
| Holdout Sharpe | 20.78 | ✓ |
| Holdout Max DD | 1333526620.98% | ✗ |
...
**Base precision:** 41.8%  →  **Expected precision (all fixes):** 47.6%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 232 |
| Rejected signals | 1110 |
| Selected precision | 41.8% |
| Rejected precision | 22.0% |
| Meta gate lift (precision) | +19.8% |
| Selected expectancy | +0.378% |
| Rejected expectancy | +0.151% |
| Selected Sharpe | +45.14 |
| Rejected Sharpe | +12.26 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 46.5 | 70.1 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep ALGO/USDT vs BTC/ETH Comparison
