...
**Symbol:** GRT/USDT  |  **Generated:** 2026-06-07 16:36:04
...
> Measured precision is 32.3% — estimated 6.6pp below achievable ceiling of ~38.9%. Primary contributors: HOLD over-representation in labels (−2.4pp), Barrier skew suppresses BUY labels (−1.8pp), Brier score above target (−2.0pp)

#### Precision Waterfall

```
Measured holdout precision :  32.3%
...
Achievable precision       :  ~38.9%  (+6.6pp precision)
Recall deficit (BUY side)  :  −2.8pp  (signals not firing)
```
...
✅ **2 issues already applied** in the codebase (expected gain: +5.0pp once retrained).
...
❌ **1. Primary model generates BUY labels**
   ...
   - Value: 0 BUY proposals / 25 total directional
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 1 holdout BUY signals fired
   - PASS — 1 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=1, buy_win_rate=100.0%
   - PASS — 1 trades, 100.0% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -21.6pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 32.3% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -21.6pp |
| OOF → Holdout gap | +67.7pp |
| thr_buy / thr_sell | 70.173 / 70.219 |
| Blocked signals | 0 (12 would-win / 12 would-lose) |

> ⚠ Gate is DESTROYING 21.6pp of precision. Blocked signals (53.8%) would have beaten gated (32.3%). Meta model is anti-selective. | OOF overfit warning: dev_prec (100.0%) exceeds holdout (32.3%) by 67.7pp.
...
**Symbol:** GRT/USDT  |  **Audit:** 2026-06-07 16:36  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=32.3%  Sharpe=4.81
**Expected after fixes:** Precision≈38.0%  (+5.8pp)

### Top 5 Problems

1. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 65/100
   > HOLD=65.9% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Metric | Value | Status |
|--------|-------|--------|
| CV Accuracy (OOF) | 60.9% | ⚠ |
| Dev OOF Precision | 32.3% | ✗ |
| Holdout Precision | 32.3% | ✗ |
| Holdout Coverage | 4.6% | ✓ |
| 95% CI Precision | [21.9%, 44.6%] | — |
| OOF→Holdout Gap | +0.0% | ⚠ degradation |
| Holdout Fired | 62 trades | ✓ |
| SELL Win Rate | 82.6% (23 trades) | ✓ |
| BUY Win Rate | 100.0% (1 trades) | ✓ |
| Sharpe (annualised) | 4.81 | ✓ |
| Max Drawdown | 400000000.00% | ✗ |
| Profit Factor | 1.87 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.1307% | ✓ |
...
| Statistical Sig. | p=0.0052 (z=-2.79) | ✓ significant |

### Class Distribution
- HOLD: **65.9%** — ⚠ severe imbalance
- SELL: **20.1%**
- BUY:  **14.0%** — ⚠ minority class

### Issues Detected
- **WARNING** — Class imbalance: 65.9% HOLD labels biases model toward neutrality.
...
```
Generated (directional):        25  (100%)
Blocked by Meta Gate:      -   24  (96%)
...
Average hold-signal realized return: **1.7259%/bar**
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 24 | 12 | 12 | 53.8% | +41.42% |
| quality | 2 | 1 | 1 | 61.5% | +3.45% ⚠ |
| hmm | 1 | 0 | 1 | 57.6% | +1.73% ⚠ |
| confluence | 1 | 0 | 1 | 54.5% | +1.73% |
...
| 70-80% | 32.3% | -0.43 | ⚠ overconfident |
| 80-90% | 42.3% | -0.43 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 1 dev samples):** `platt`
...
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +67.74pp |
...
|------|-------|---------|-------|--------|---------|
| 1 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 65/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=65.9% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 32.3%  →  **Expected precision (all fixes):** 38.0%  (+5.8pp)
...
|--------|-------|
| Selected signals | 62 |
| Rejected signals | 1280 |
| Selected precision | 32.3% |
| Rejected precision | 21.0% |
| Meta gate lift (precision) | +11.2% |
| Selected expectancy | +0.131% |
| Rejected expectancy | +0.131% |
| Selected Sharpe | +20.21 |
| Rejected Sharpe | +12.34 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 70.1 | 70.2 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep GRT/USDT vs BTC/ETH Comparison
