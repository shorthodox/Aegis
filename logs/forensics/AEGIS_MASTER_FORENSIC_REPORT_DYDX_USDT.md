...
**Symbol:** DYDX/USDT  |  **Generated:** 2026-06-07 16:35:57
...
> Measured precision is 34.0% — estimated 2.4pp below achievable ceiling of ~36.4%. Primary contributors: Brier score above target (−2.0pp), Gate blocking signals that would have won (−0.4pp)
...
Achievable precision       :  ~36.4%  (+2.4pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 4 holdout BUY signals fired
   - PASS — 4 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=4, buy_win_rate=75.0%
   - PASS — 4 trades, 75.0% WR.
...
| OOF → Holdout gap | +50.5pp |
| thr_buy / thr_sell | 56.735 / 56.796 |
| Blocked signals | 0 (112 would-win / 97 would-lose) |

> ⚠ Gate is DESTROYING 19.9pp of precision. Blocked signals (53.8%) would have beaten gated (34.0%). Meta model is anti-selective. | OOF overfit warning: dev_prec (84.5%) exceeds holdout (34.0%) by 50.5pp.
...
**Symbol:** DYDX/USDT  |  **Audit:** 2026-06-07 16:35  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=34.0%  Sharpe=18.77
...
| Dev OOF Precision | 84.5% | ✓ |
| Holdout Precision | 34.0% | ✗ |
| Holdout Coverage | 31.5% | ✓ |
| 95% CI Precision | [29.6%, 38.6%] | — |
| OOF→Holdout Gap | -50.5% | ⚠ degradation |
| Holdout Fired | 421 trades | ✓ |
| SELL Win Rate | 76.1% (184 trades) | ✓ |
| BUY Win Rate | 75.0% (4 trades) | ✓ |
| Sharpe (annualised) | 18.77 | ✓ |
| Max Drawdown | 300000000.00% | ✗ |
| Profit Factor | 2.88 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.6101% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.58) | ✓ significant |
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 209 | 112 | 97 | 53.8% | +143.81% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0583 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9671 | ~1.0 | ✓ |
...
**Recommended calibrator (for 407 dev samples):** `isotonic`
...
| Confidence Drift | 🟢 OK | T=0.967 |
| Prediction Drift | 🔴 CRITICAL | OOF vs holdout gap: +50.55pp |
...
**Base precision:** 34.0%  →  **Expected precision (all fixes):** 39.7%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 421 |
| Rejected signals | 916 |
| Selected precision | 34.0% |
| Rejected precision | 32.0% |
| Meta gate lift (precision) | +2.0% |
| Selected expectancy | +0.610% |
| Rejected expectancy | +0.558% |
| Selected Sharpe | +30.27 |
| Rejected Sharpe | +22.18 |
...
|--------|--------|---------|---------|----------|--------|-----------|---------|
| ACCUMULATION         | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| CHOPPY               | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| COMPRESSION          | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| DISTRIBUTION         | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BEAR        | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| TRENDING_BULL        | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
| VOLATILE_EXPANSION   | ✅ | ✅ | 56.7 | 56.8 | ✅ ENABLED | 60.0% | 1.25 |
...
## Section 19 — Deep DYDX/USDT vs BTC/ETH Comparison
