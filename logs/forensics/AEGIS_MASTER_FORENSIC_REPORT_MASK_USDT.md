...
**Symbol:** MASK/USDT  |  **Generated:** 2026-06-07 16:39:27
...
> Measured precision is 35.7% — estimated 3.4pp below achievable ceiling of ~39.1%. Primary contributors: Brier score above target (−2.0pp), Barrier skew suppresses BUY labels (−1.0pp), Gate blocking signals that would have won (−0.4pp)

#### Precision Waterfall

```
Measured holdout precision :  35.7%
...
Achievable precision       :  ~39.1%  (+3.4pp precision)
...
✅ **4. buy_fire mask fires BUY holdout signals**
   ...
   - Value: buy_h_n = 58 holdout BUY signals fired
   - PASS — 58 BUY holdout trades.

✅ **5. tradeable_buy_holdout = True**
   - `scripts/retrain_model.py:2288-2292`
   - Check: `hit_buy AND buy_h_n > 0 AND buy_h_prec ≥ 0.50`
   - Value: buy_h_n=58, buy_win_rate=63.8%
   - PASS — 58 trades, 63.8% WR.

#### Meta Gate Audit

🔴 **Gate status: HURTING**  (lift: -18.1pp)

| Metric | Value |
|--------|-------|
| Gated-in precision | 35.7% |
| Blocked signals win rate | 53.8% |
| Precision lift from gate | -18.1pp |
| OOF → Holdout gap | +56.5pp |
| thr_buy / thr_sell | 60.150 / 60.149 |
| Blocked signals | 0 (120 would-win / 104 would-lose) |

> ⚠ Gate is DESTROYING 18.1pp of precision. Blocked signals (53.8%) would have beaten gated (35.7%). Meta model is anti-selective. | OOF overfit warning: dev_prec (92.2%) exceeds holdout (35.7%) by 56.5pp.
...
**Symbol:** MASK/USDT  |  **Audit:** 2026-06-07 16:39  |  **Confidence Level:** MEDIUM — based on 47 holdout signals; widen to 200+ for HIGH

**Current:** Precision=35.7%  Sharpe=23.26
**Expected after fixes:** Precision≈41.5%  (+5.8pp)
...
2. 🟡 **Severe Class Imbalance (HOLD dominates)** — Score: 57/100
   > HOLD=57.3% of labels. Meta model sees 60% zero-labels → calibration distorted.
...
| Dev OOF Precision | 92.2% | ✓ |
| Holdout Precision | 35.7% | ✗ |
| Holdout Coverage | 41.8% | ✓ |
| 95% CI Precision | [31.9%, 39.8%] | — |
| OOF→Holdout Gap | -56.5% | ⚠ degradation |
| Holdout Fired | 557 trades | ✓ |
| SELL Win Rate | 84.4% (192 trades) | ✓ |
| BUY Win Rate | 63.8% (58 trades) | ✓ |
| Sharpe (annualised) | 23.26 | ✓ |
| Max Drawdown | 100000000.00% | ✗ |
| Profit Factor | 3.30 | ✓ |
| Kelly Fraction | 25.0% | — |
| Expectancy/Trade | +0.4501% | ✓ |
...
| Statistical Sig. | p=0.0000 (z=-6.74) | ✓ significant |

### Class Distribution
- HOLD: **57.3%** — ⚠ severe imbalance
- SELL: **26.2%**
- BUY:  **16.5%** — OK

### Issues Detected
- **WARNING** — Class imbalance: 57.3% HOLD labels biases model toward neutrality.
...
|--------|---------|-----------|------------|----------|-----------|
| meta_gate | 224 | 120 | 104 | 53.8% | +154.13% |
...
|--------|-------|--------|--------|
| ECE (before cal.) | 0.0857 | <0.10 | ✓ |
| ECE (after cal.)  | 0.0000 | <0.10 | ✓ |
| Brier Score | 0.3328 | <0.25 | ✗ |
| Cal. Temperature | 0.9611 | ~1.0 | ✓ |
...
| 70-80% | 35.7% | -0.39 | ⚠ overconfident |
| 80-90% | 45.7% | -0.39 | ⚠ overconfident |
| 90-100% | 80.0% | -0.15 | ⚠ overconfident |

**Confidence inflation detected:** No
**Recommended calibrator (for 348 dev samples):** `isotonic`
...
| 2 | 🟡 **Severe Class Imbalance (HOLD dominates)** | Training Quality | 57/100 | `scripts/retrain_model.py:836` ✅ FIXED | HOLD=57.3% of labels. Meta model sees 60% zero-labels → calibration di… |
...
**Base precision:** 35.7%  →  **Expected precision (all fixes):** 41.5%  (+5.8pp)
...
| Metric | Value |
|--------|-------|
| Selected signals | 557 |
| Rejected signals | 777 |
| Selected precision | 35.7% |
| Rejected precision | 26.0% |
| Meta gate lift (precision) | +9.7% |
| Selected expectancy | +0.450% |
| Rejected expectancy | +0.153% |
| Selected Sharpe | +32.61 |
| Rejected Sharpe | +13.46 |
...
## Section 19 — Deep MASK/USDT vs BTC/ETH Comparison
