...
def _short_below_resistance(**kw):
    """A bear rallying toward resistance, price still short of the level.

    rp 0.72 in a TRENDING_BEAR is PULLBACK_RP_SHORT territory -> TREND_PULLBACK
    SELL, which is exactly what was left armed in production.
    """
    d = mk(price=100.0, atr=1.5, support=94.0, resistance=102.3,
           rsi=55.0, **TURNED_DOWN)
    d.update(kw)
    return d


LEVELS = [(102.3, 4), (94.0, 4), (106.0, 3)]
...
def test_the_stop_is_still_beyond_the_resistance():
    ...
    plan = _plan(TURNED)
    assert plan.action == ACTION_ENTER
    assert plan.stop > 102.3, (
        f'stop {plan.stop:.6g} is not clear of the 102.3 resistance — an early '
...
def test_the_early_entry_is_worse_than_the_level_and_that_is_priced_in():
    """A short filled at 100.0 instead of 102.3 is a worse entry with a wider
