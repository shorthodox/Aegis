...
# ============================================================
# V2 — TIMEFRAME & STORE CONFIGURATION
# ============================================================
TRAIN_TIMEFRAME  = '15m'              # primary candle timeframe for this trainer
CANDLE_MINUTES   = 15                 # minutes per candle (drives scaling below)
MODEL_STORE_NAME = 'model_store_15m'  # isolated from production 1h store

# Soft-confluence window scaled to ~5 days: 5d × 24h × (60/15) = 480 bars on 15m
SOFT_CONF_WINDOW = int(5 * 24 * (60 // CANDLE_MINUTES))   # 480

# Default candle count equiv. to 5000h: 5000h × (60/15) = 20000 bars on 15m
DEFAULT_CANDLES  = int(5000 * (60 // CANDLE_MINUTES))      # 20000

# ============================================================
# CONFIG
# ============================================================
NUM_CLASS = 3                 # 0=SELL, 1=HOLD, 2=BUY
# MAX_LOOKAHEAD in bars.
# 1h trainer:  96 bars = 4 days.
# 15m trainer: 96 bars = 24 hours — appropriate for shorter-term signals.
MAX_LOOKAHEAD = 96
...
# ============================================================
def train_token(symbol: str, candles: int = DEFAULT_CANDLES) -> dict[str, Any] | None:  # pyright: ignore[reportGeneralTypeIssues]
    print(f"\n{'='*60}\nTraining model for {symbol}\n{'='*60}")
    try:
        p = Predictor(symbol)
        _hours_equiv = candles * CANDLE_MINUTES // 60
        print(f"Fetching {candles} {TRAIN_TIMEFRAME} candles (~{_hours_equiv}h) for {symbol}...")
        df = p.fetch_live_data(timeframe=TRAIN_TIMEFRAME, limit=candles)
        if df is None or df.empty:
            print(f"No data for {symbol}")
            return None

        print("Fetching BTC market context...")
        btc_df = p.fetch_btc_data(timeframe=TRAIN_TIMEFRAME, limit=candles)
        ...
        try:
            # Daily context: derive day count from candle window
            _days = max(1000, candles * CANDLE_MINUTES // (24 * 60) + 10)
            df_1d = p.fetch_live_data(timeframe='1d', limit=_days)
        except Exception:
            df_1d = None
        ...
        try:
            soft_conf = compute_soft_confluence_features(df, window=SOFT_CONF_WINDOW)
...
def train_fleet(candles: int = DEFAULT_CANDLES, resume: bool = True):
    ...
    store_dir = Path(root_dir) / "src" / "ml" / MODEL_STORE_NAME
    ...
    for idx, symbol in enumerate(FLEET_SYMBOLS, 1):
        ...
        m = train_token(symbol, candles=candles)
        if m:
            results.append(m)
    ...
    with open(logs_dir / "training_summary.json", "w") as f:
        json.dump({"timestamp": datetime.now().isoformat(), "timeframe": TRAIN_TIMEFRAME,
                   "candles": candles, "hours_equiv": candles * CANDLE_MINUTES // 60,
...
if __name__ == "__main__":
    ...
    _parser.add_argument("--candles", type=int, default=DEFAULT_CANDLES,
                         help=f"Number of {TRAIN_TIMEFRAME} candles to fetch per token "
                              f"(default {DEFAULT_CANDLES} ≈ {DEFAULT_CANDLES * CANDLE_MINUTES // 60}h)")
    ...
    if _args.symbol:
        train_token(_args.symbol, candles=_args.candles)
    else:
        train_fleet(candles=_args.candles, resume=not _args.full)