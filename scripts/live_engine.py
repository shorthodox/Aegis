...
class LiveEngine:
    ...
    async def _structure_gate(
        ...
        # ── Fire against the SWING S/R (the chart's levels), not the 24h rolling
        # extreme ─────────────────────────────────────────────────────────────
        # The 24h rolling low/high is frequently mid-range relative to the real
        # swings, so an "at support" fire on the rolling range landed structurally
        # mid-range and looked mid-range on the chart. Classify location against
        # the SAME major swing S/R the chart draws (_swing_sr, k+3) so entries sit
        # at the visible levels.
        #
        # This basis was the v25 fire-rate collapse (swing levels are far apart, so
        # nearly every signal read mid-range and got BLOCKED -> zero fires). What
        # changed: v35 now holds a mid-range signal as PENDING and waits for price
        # to reach the level, so switching the basis no longer zeroes the rate — it
        # converts premature signals into pending ones. The swing levels are also
        # written back into result so the hybrid SL/TP and the display anchor to the
        # SAME S/R. Falls back to the rolling 24h range_position when pivots are
        # sparse (_swing_sr returns None).
        _sw = await self._swing_sr(symbol, price, atr_g)
        if _sw is not None and 0 < _sw[0] < _sw[1]:
            support, resistance = _sw
        ...
        # v10 restore: the STRICT-direction block (v30) is GONE. A confirmed
        # breakout (buy after resistance breaks + retest-holds, sell after support
        # breaks + retest-holds) is allowed again via the breakout_level branch
        # below — this is a big chunk of the signal volume that strict direction
        # removed. Reversals stay primary; breakouts still require the retest-hold.
