...
class LiveEngine:
    ...
    # The model must actually be DECIDED, not a coin flip. Minimum directional
    # margin on the raw dual-model probabilities: p_buy − p_sell for a BUY
    # (mirror for a SELL). FIL fired a LONG at p_buy 50.7% vs p_sell 49.3% —
    # a 1.4pt margin — into a "Strong Bearish" tape. The old UWGS damped such a
    # signal to ~zero directional weight; model-first must reject it outright.
    MIN_MODEL_CONVICTION = 0.10   # 10pt spread (e.g. 55/45)

    # HTF (4h/1d) S/R is DIRECTIONAL: a 4h SUPPORT belongs to BUYs, a 4h
    # RESISTANCE belongs to SELLs. Never trade INTO the big level — a SELL with
    # a 4h support right below gets bounced off the floor (LDO SHORT @0.3209
    # with the 4H/D support ~0.31 just under it: TP2/TP3 sat below the support,
    # unreachable), and a BUY with 4h resistance right overhead hits the wall.
    # Require at least this much ROOM (in ATR) to the OPPOSING HTF level.
    # The legacy Gate 1.8 used the same 2.5 ATR distance but only blocked when
    # RSI was also extreme (>72 / <28) — LDO's RSI 65 slipped through. Strict now.
    HTF_MIN_ROOM_ATR = 2.5
            ...
            if (not existing or _reversal_flip) and result.get('fire') \
                ...
                if _reversal_flip or cooldown_elapsed >= required_cooldown:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        # ── Guard D: model conviction floor — no coin-flip fires ──
                        # The model must actually be DECIDED. FIL fired a LONG on
                        # p_buy 50.7% vs p_sell 49.3% (a 1.4pt margin). The model
                        # still picks the side; it just has to mean it.
                        _p_buy  = float(result.get('p_buy', 0) or 0)
                        _p_sell = float(result.get('p_sell', 0) or 0)
                        _conv   = (_p_buy - _p_sell) if new_side == 'BUY' else (_p_sell - _p_buy)
                        if _conv < self.MIN_MODEL_CONVICTION:
                            print(f'[{symbol}] MODEL BLOCK {new_side}: conviction '
                                  f'{_conv * 100:.1f}pt < {self.MIN_MODEL_CONVICTION * 100:.0f}pt '
                                  f'(p_buy={_p_buy:.3f} p_sell={_p_sell:.3f}) — coin flip')
                            if symbol in self.last_signals:
                                self.last_signals[symbol]['fire']             = False
                                self.last_signals[symbol]['signal']           = 'HOLD'
                        ...
                        # ── Guard E: technical composite says NO-TRADE ────────────
                        # UWGS cannot pick the side any more, but when its HOLD score
                        # beats the model's own side the whole technical picture is
                        # against the trade (FIL: HOLD 49.3 > BUY 40.1 with trend 8%,
                        # momentum 16%, MACD bearish). The old engine held these.
                        ...
                        if _sc and _hold_sc > _side_sc:
                            print(f'[{symbol}] MODEL BLOCK {new_side}: composite says HOLD '
                                  f'(hold={_hold_sc:.1f} > {new_side.lower()}={_side_sc:.1f}) '
                                  f'— technicals oppose')
                            if symbol in self.last_signals:
                                self.last_signals[symbol]['fire']            = False
                                self.last_signals[symbol]['signal']          = 'HOLD'
                                self.last_signals[symbol]['composite_blocked'] = True
                            self.bootstrap_done = min(self.bootstrap_done + 1, self.bootstrap_total)
                            return
                        ...
                        if _htf is not None:
                            ...
                            _need = self.HTF_MIN_ROOM_ATR * _atr_h
                            _wall = ''
                            if new_side == 'BUY' and _htf_res > 0:
                                _room = _htf_res - price
                                if _room < _need:
                                    _wall = (f'4h/1d RESISTANCE {_htf_res:.6g} only '
                                             f'{_room / _atr_h:.2f} ATR overhead '
                                             f'(< {self.HTF_MIN_ROOM_ATR}) — a BUY may not '
                                             f'trade into resistance')
                            elif new_side == 'SELL' and _htf_sup > 0:
                                _room = price - _htf_sup
                                if _room < _need:
                                    _wall = (f'4h/1d SUPPORT {_htf_sup:.6g} only '
                                             f'{_room / _atr_h:.2f} ATR below '
                                             f'(< {self.HTF_MIN_ROOM_ATR}) — a SELL may not '
                                             f'trade into support')
                            if _wall:
                                print(f'[{symbol}] MODEL BLOCK {new_side}: HTF_WALL — {_wall}')
                                if symbol in self.last_signals:
                                    self.last_signals[symbol]['fire']        = False
                                    self.last_signals[symbol]['signal']      = 'HOLD'
                                    self.last_signals[symbol]['htf_blocked'] = True
                                    self.last_signals[symbol]['structure_reason'] = _wall
                                self.bootstrap_done = min(self.bootstrap_done + 1, self.bootstrap_total)
                                return
