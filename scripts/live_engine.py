            ...
            if (not existing or _reversal_flip) and result.get('fire') \
                ...
                if _reversal_flip or cooldown_elapsed >= required_cooldown:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        if _target_m is None or _near_pct_m > self.PENDING_NEAR_PCT:
                            _why_m = (f'no tested {_role_m} to wait for'
                                      if _target_m is None else
                                      f'{_near_pct_m:.2f}% from {_role_m} {_target_m:.6g} '
                                      f'(needs <= {self.PENDING_NEAR_PCT}%)')
                            ...
                            self.bootstrap_done = min(self.bootstrap_done + 1, self.bootstrap_total)
                            return
                        # Price is AT the level — record it and fall through to the
                        # 3x5m confirmation (Guard J) below, then fire.
                        result['at_pending_level'] = {
                            'level': round(_target_m, 10), 'role': _role_m,
                            'dist_pct': round(_near_pct_m, 3)}
                        if symbol in self.last_signals:
