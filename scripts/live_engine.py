...
class LiveEngine:
    ...
    # already turned its way: >= ENTRY_5M_MIN of the last ENTRY_5M_WINDOW closed
    # 5m candles must close in the signal direction. Otherwise WAIT — the setup
    # is re-checked next scan and fires the moment the turn actually starts.
    ENTRY_5M_WINDOW = 4
    ENTRY_5M_MIN    = 3
                ...
                if _loss_ago < self.LOSS_COOLDOWN_SECONDS:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        if len(_c5m) >= self.ENTRY_5M_WINDOW:
                            _last5m  = _c5m[-self.ENTRY_5M_WINDOW:]
                            _aligned = sum(
                                1 for c in _last5m
                                if (float(c[4]) > float(c[1])) == _want_up
                                and float(c[4]) != float(c[1])
                            )
                            _confirmed = _aligned >= self.ENTRY_5M_MIN
                            _why5m = (f'5m not turned {"up" if _want_up else "down"} '
                                      f'({_aligned}/{self.ENTRY_5M_MIN} of last '
                                      f'{self.ENTRY_5M_WINDOW})')
                        else:
                            _confirmed = False
                            _why5m = '5m confirmation unavailable (feed down)'
                        if not _confirmed:
                            print(f'[{symbol}] MODEL WAIT {new_side}: {_why5m} — '
                                  f'not entering without the 3x5m turn')
                            if symbol in self.last_signals:
                                self.last_signals[symbol]['fire']            = False
                                self.last_signals[symbol]['signal']          = 'HOLD'
