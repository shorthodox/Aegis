            ...
            if (not existing or _reversal_flip) and result.get('fire') \
                ...
                if _reversal_flip or cooldown_elapsed >= required_cooldown:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        try:
                            _lvls_m = await self._important_levels(symbol, price, _atr_m)
                        except Exception:
                            _lvls_m = []
                        if new_side == 'BUY':
                            _target_m = max([l for l, _t in _lvls_m if l < price], default=None)
                            _role_m   = 'support'      # BUY waits at SUPPORT below
                        else:
                            _target_m = min([l for l, _t in _lvls_m if l > price], default=None)
                            _role_m   = 'resistance'   # SELL waits at RESISTANCE above
