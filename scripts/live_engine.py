...
class LiveEngine:
    ...
    # After a LOSING close, bench THAT token for this long — no revenge-trading a
    # loser (LDO was shorted, lost, then re-shorted ~3h later and lost again).
    # Applies even to a reversal flip: a token that just cost us waits it out.
    LOSS_COOLDOWN_SECONDS = 14400   # 4 h
    ...
    GATE_VERSION = 'model-first-v70 (STOP MARGINAL FLOORS KILLING THE FADES THE ENGINE EXISTS TO TAKE — three fixes, all measured on the live 60-token fleet. User: "gate that are not too important is being hard block". CONFIRMED by measurement: of 11 model fires, ZERO fired; the model itself declined 44/60 (73%) so the gates were never the whole story, but the survivors were dying on rounding errors — ARKM edge_score 100/100, conviction 0.61/0.39, RSI 22.2 AT support (rp 0.167) hard-blocked for ATR 0.472% vs 0.500% (a 0.028pp miss), and FIL edge 100/100, RSI 18.2 at rp 0.00 blocked for conviction 0.095 vs 0.100 (a 0.005 miss). (1) THE rp=0.0 BUG — `float(result.get("range_position", 0.5) or 0.5)` rewrote a GENUINE 0.0 into 0.5 because 0.0 is falsy in Python, so price sitting at the ABSOLUTE BOTTOM of its range (the most extreme oversold-at-support reading there is) was read as dead-centre MID-RANGE by every location check. That silently denied Guard L its reversal exemption (rp <= 0.35 AND rsi <= 32): FIL matched on BOTH counts and was still killed by confluence net=-3. Fixed via new _range_pos() which preserves 0.0 and still falls back to 0.5 on missing/garbage. Measured: FIL went from hard-BLOCKED at Guard L to PENDING at Guard J, one reversal candle from firing. This bug also silently disabled the two relaxations below at exactly the setups they target. (2) GUARD A is no longer a flat cross-sectional line — it conflated "this token is dead" with "the whole tape is quiet". Below MIN_FIRE_ATR_HARD_PCT (0.35) is still a HARD block (ETH at 0.29% correctly blocked as dead market); the band between 0.35 and MIN_FIRE_ATR_PCT (0.5) fires ONLY at a structural extreme, tagged RISKY (thin_atr). The floor asks "can the move cover costs" and at 0.47% ATR TP1 (0.55 ATR) is ~0.26%, comfortably above fees+spread — that block was protecting nothing, and a reversal FROM an extreme is precisely where volatility EXPANDS, so demanding the prior 14h be lively is backwards. (3) GUARD D no longer double-counts: a model emitting edge_score >= EXTREME_EDGE_MIN (80, the same bar the tier system already calls STRONG) with fire=True has ALREADY proved the side through its own calibrated meta gate, so at a structural extreme the conviction floor drops to MIN_DIR_CONVICTION_EXTREME (0.06), tagged RISKY (thin_conviction). The LDO-style coin flip Guard D exists to stop is STILL blocked (GMX 0.005 rejected at the relaxed 0.06). STRUCTURAL EXTREME is shared by both relaxations and is side-aware: BUY needs rp <= 0.25 AND rsi <= 35, SELL needs rp >= 0.75 AND rsi >= 65 — so a SELL at rp 0.0 (LDO) is NOT extreme and keeps the 0.10 floor. CRITICALLY THIS IS NOT A MID-RANGE LOOSENING (the user explicitly rejected that): SUI at rp 0.474 was still blocked "not at a structural extreme", and both relaxations require an exhaustion location, so the engine takes MORE of the fades it is built for, not more trades generally. MEASURED FLEET IMPACT: fired 0 -> 2, plus 3 PENDING one reversal candle away (ARKM, FIL, GRT); dead tape still blocked; mid-range still blocked; zero RELAX events actually needed to fire, so the relaxations are conservative not signal-manufacturing. Engine-side only; model/training/edge_engine untouched; non-repainting) # v69 (GUARD J CONFIRMS ON A REAL REVERSAL CANDLE, not "3 random green 5m candles". User: "it was waiting for the support, but in between 3 random 5 min green candle made him throw the signal — if reversal arrives i want it to see candlestick patterns like hammer, harami or bullish/bearish engulfing, not any random stuff." The old confirmation counted >= ENTRY_5M_MIN (3) of the last ENTRY_5M_WINDOW (4) CLOSED 5m candles closing in the signal direction — i.e. 3 up candles was enough to fire a BUY, which in a downtrend is just noise: a dead-cat bounce of 3 green candles threw the pending long before support actually rejected. Now the entry confirmation is a genuine candlestick REVERSAL PATTERN completing at the last closed 5m candle (new module-level _reversal_candle): a BUY needs a BULLISH reversal — hammer, bullish engulfing, bullish harami, piercing line, or morning star; a SELL needs the BEARISH mirror — shooting star, bearish engulfing, bearish harami, dark cloud cover, or evening star. No pattern yet => WAIT (pending, momentum_blocked), re-checked next scan. This keeps the v68 fail-CLOSED behaviour (no usable 5m data => WAIT, never fire blind) and REPLACES only the "what counts as confirmed" test. The confirming pattern name is written to result[reversal_pattern] for display/explainability. Quality-over-quantity by design: fewer, cleaner reversal entries — a bounce must LOOK like a reversal, not merely tick up 3 times. Engine-side only; model/training/edge_engine untouched; uses CLOSED candles only, non-repainting) # v68 (GUARD J FAILS CLOSED — no 5m confirmation, no entry. The 3x5m turn confirmation only ran when the 5m fetch SUCCEEDED (len >= ENTRY_5M_WINDOW); when the fetch failed or returned too few candles (rate limit / error) the whole check was SKIPPED and the signal fell through and fired UNCONFIRMED — which is how a BUY entered into a hard-falling 5m (AXS long, RSI 10.2, all-red 5m). In this rate-limited tape the 5m fetch misses often, so the confirmation was silently bypassed. Now Guard J fails CLOSED: no usable 5m data => WAIT (pending, momentum_blocked), never fire blind. The signal re-evaluates next scan when the 5m is back and the 3/4 turn has formed. Tradeoff: if the 5m feed is persistently down the feed quiets, but the user requires the confirmation ALWAYS enforced. Everything else unchanged) # v67 (GUARD N — FADE THE TREND, NEVER FOLLOW IT. User: "SELL IN TRENDING BULL WHEN OVERBOUGHT AND BUY IN TRENDING BEAR WHEN OVERSOLD" — the only valid trend entries are counter-trend reversals at exhaustion; a trend-FOLLOWING entry (SELL in a bear / BUY in a bull, which the engine used to treat as "aligned" and fire) is the wrong-way entry that was losing. Guard N BLOCKS trend-following: SELL in TRENDING_BEAR and BUY in TRENDING_BULL are rejected outright. The reversal entries the user WANTS (SELL in a bull / BUY in a bear) fall through to Guard B, which already demands the level TAG + RSI extreme + 5m turn = the "when overbought/oversold" confirmation. RANGING regime triggers neither branch, so location-gated sell-at-resistance / buy-at-support (Guards K/M) still fire. This INVERTS the engine from trend-following to mean-reversion at exhaustion — a large strategic change (fewer signals, only fades) matching the reversal approach the user says the older 14/14 version used. Direction still comes from the MODEL; Guard N only vetoes the with-trend half. NOTE if the deployed engine still fires SELL-in-bear it is running stale code — verify engine_version) # v66 (TIGHT SL FOR RISKY TIER — cap the loss on the least-certain trades. The winners were small and the RISKY losers were big (LDO -3%, SUI open -2.1%) because the v42 logic gave LOOSE entries (mid_range) the WIDEST stop (_sl_cap 2.5 ATR) "so the structural stop is not noise-tight" — exactly backwards for risk management. Now a RISKY-tier signal gets RISKY_SL_CAP_ATR (1.2) instead, so a failed low-conviction thesis is a small loss not a 2.2-2.5x ATR bleed: measured on a SUI-like RISKY short the SL risk drops 2.35% -> 1.13% (max loss ~halved) and RR only improves (tighter risk leg). STRONG/NORMAL keep the structural/normal cap — they earned the room; only RISKY is tightened. SL levels themselves were already correct (right side of entry, anchored to the invalidation level) — the problem was purely the DISTANCE on the risky ones. Tradeoff: a tighter stop is hit more often, but on a low-quality setup a quick small loss beats a big one) # v65 (POST-LOSS COOLDOWN — stop revenge-trading a loser. Track record showed LDO shorted, lost, then re-shorted ~3h later and lost AGAIN (all RISKY counter-trend reversal shorts). The only cooldown was COOLDOWN_SECONDS=300 (5 min, its "30 min" comment was stale from the v42 aggressive-fire drop) applied to ANY close, so nothing stopped the engine re-firing a token minutes after it lost. NEW: after a LOSING close, _last_loss_time[symbol] is set and the token is benched for LOSS_COOLDOWN_SECONDS (4h) — a hard skip that takes precedence over the normal cooldown AND a reversal flip, so a token that just cost us waits out the failed thesis before it can fire again. NOTE this stops the REPEAT; the first counter-trend reversal short still fires (weak conviction 0.103 just over the v64 0.10 floor + Guard L reversal exemption + Guard M tag_reject) — tightening THAT trades off feed volume and is the open quality/quantity dial. Also still UNVERIFIED whether the deployed engine runs this code — engine_version must be checked. Comments on COOLDOWN_SECONDS/FLIP corrected to the real 5min/10min) # v64 (GUARD D FLOOR 0.15 -> 0.10 — revive the feed in a directionless tape. Diagnosed a total blackout (no fires AND no pending): NOT the recent Guard M work — measured the full chain live and 14-15 of ~17 model fires died at Guard D, only 1 even reached Guard M (and it fired). The market had gone directionless: the model buy-vs-sell lean was weak market-wide (median conviction 0.076 = 54/46; only TIA 0.207 and ENJ 0.157 cleared 0.15). Guard D BLOCKS (does not pend) so the feed went dark. Lowered the conviction floor to 0.10, which sits at a natural break — 55/45 moderate leans (ETH 0.146, LDO 0.119, STRK 0.113) pass while 54/46 weak leans and true coin-flips (SAND 51/49=0.018, GMX 49/51=0.014, the LDO ~0.008 case Guard D exists to stop) stay blocked. LOW RISK: Guard D is the FIRST of the chain — a survivor still clears Guard L (confluence >= 2), Guard M (at a tested level + tag/reject/wrong-loc) and Guard J (5m confirm), so a moderate lean only PROCEEDS to the real quality filters, it is not fired blindly. Everything else unchanged) # v63 (GUARD M — PENDING TARGET FROM DEEP MULTI-TF STRUCTURE. The pending resistance a SELL waited at was way too high (HBAR: waiting at 0.0705 = +4.3%, while the real level it should watch — a broken support now acting as 4h/1d resistance — sat at 0.0678 = +0.3%). Cause: the target came from _important_levels, which scans only 1h/1000 (~41 days) and keeps the 4 MOST-TOUCHED levels — the big old highs — so a near, freshly relevant level never makes the top-4. NEW _structural_levels() pools CONFIRMED non-repainting pivots across 1h + 4h + 1d (weeks on the 1h to ~4 years on the 1d — the "study the whole history" the user asked for, done in 3 cached fetches instead of the ~40 paginated calls a literal 60k-1h-candle scan needs), clusters within LEVEL_MERGE_ATR, keeps levels touched >= PENDING_TARGET_MIN_TOUCHES (3), and Guard M picks the NEAREST significant level on its side. Measured: HBAR nearest resistance 4.29% -> 0.29%, OP 2.26% -> 0.23%. _important_levels is unchanged and still feeds Guard K (its wrong-side-at-a-STRONG-level veto still wants the few strongest levels); only Guard M\'s pending target moved to the richer nearest-significant set. tag+reject (v61) and wrong-location (v62) unchanged) # v62 (GUARD M — WRONG-LOCATION REJECT. A SELL was being HELD waiting for a resistance FAR above it while price sat down near SUPPORT — the "sell at support, why is it even waiting" case (SUI rp 0.03, SAND 0.12, SEI 0.24, OP 0.44 were all pending shorts sitting in the support half, waiting for a resistance they were far from). A SELL belongs AT resistance / a BUY at support, so Guard M now blocks a signal that is on the WRONG side of the range instead of holding it: it uses range_position (0=at support, 1=at resistance — the chart\'s own read, always present) and REJECTS a SELL in the support half (rp < PENDING_WRONG_LOC_RP=0.5) or a BUY in the resistance half (rp > 0.5). NOTE the first cut used _important_levels\' nearest opposite level, but that gate keeps only the top-4 most-touched levels so it frequently has NO support below / NO resistance above (HBAR/SEI/SUI all had none) — the comparison silently no-oped; range_position never does. at-level and tag+reject (v61) fired earlier and are EXEMPT (a genuine tag-and-reject can sit anywhere). Measured live (23 fires): rejects 6/11 support-half sells + 4/12 resistance-half buys, keeps the on-side signals waiting; healthy rp spread 0.00-0.65 so no degeneracy. Fails open if range_position is missing) # v61 (GUARD M REFINED — HOLD only when APPROACHING the level, FIRE when it just REJECTED it. v60 held EVERY not-at-level signal, which delayed the exact trades it should take: a SELL where price rallied UP to resistance, tagged it, and is now FALLING away (the SEI/SUI pending-shorts) got held waiting for a second touch that may never come, while the move down — often the best coverage, with the opposite level far away — was missed. The hold is meant only for a signal fired MID-RANGE while still travelling TOWARD its level (a SELL coming up from support). So Guard M now fires immediately (still via Guard J 3x5m confirmation) when price has TAGGED the target level within the last PENDING_TAG_LOOKBACK (6) closed 1h candles — a wick within PENDING_TAG_TOL_PCT (0.4%) of it — and is now on the correct side (SELL: recent high >= resistance and price now below it; BUY: recent low <= support and price now above it). Guard J still gates it: a tag+reject only fires if the 5m is actually moving AWAY from the level (down for a SELL), so a price climbing BACK toward the level is held, not fired. Only a genuinely approaching mid-range signal (no recent tag) still waits. Everything else unchanged from v60) # v60 (GUARD M — HOLD EVERY SIGNAL UNTIL PRICE IS AT THE LEVEL. Per user spec: no signal fires mid-range; every signal that clears the direction guards is HELD (pending) until price reaches the level it should reverse at — nearest important SUPPORT below for a BUY, nearest important RESISTANCE above for a SELL — measured in PRICE not ATR (within PENDING_NEAR_PCT = 0.3%, "as near as possible"). Only then does the 3x5m confirmation (Guard J, which runs next) fire the held signal. No tested level on the correct side (price at the extreme of its range) => stays pending, never fires into open air (user chose strict over model-only). The scan re-evaluates every symbol every cycle and last_signals is rebuilt fresh each scan, so a pending signal simply fires on the FIRST scan where price is within 0.3% of its level and the 5m confirms — no persistent queue needed; pending is surfaced via last_signals[pending_entry/pending_side/pending_target] for display. MEASURED IMPACT (28-signal book): only ~2-5 sit within 0.3% of their level; ~23-25 go pending. This is a deliberate ~80%+ cut in immediate fires and is EXACTLY the "at a level is mandatory to fire" design that collapsed the rate to zero in v25/v38 — the difference is the pending setups are NOT discarded (they wait for price), and they are pre-filtered for direction by Guards D/L/F/J so the ones that do fire sit right on a tested level with 5m confirmation. Expect a much quieter, spikier feed BY DESIGN. Guard M placed after the direction guards (D/L) and before the 5m confirmation (J) so the sequence is: right side -> at the level -> confirmed -> fire. Engine-side only; model untouched; non-repainting (_important_levels uses closed pivots)) # v59 (GUARD L — DIRECTIONAL CONFLUENCE, makes DIRECTION visible to the selector. The edge score that decides `fire` is DIRECTION-BLIND (EdgeScoringEngine takes a `side` arg and never uses it: regime maps BULL->1.0 AND BEAR->1.0 identically, momentum is |rsi-50|, so a SELL at RSI 90 out-scores a SELL at RSI 55) and direction re-enters ONLY through the near-coin-flip p_buy vs p_sell. So it fired LONGS into a tape where the predictor\'s OWN market_bias, MACD, supertrend, both HTF trends and RSI slope all read bearish — measured on an 18-signal book, 7 such longs, each 5-6 signed indicators opposing, kept alive only by the coin flip. The fix CANNOT go in edge_engine.py (retrain_model + meta_gate_optimizer import it; all 116 meta_threshold=60 were calibrated against that exact function; retraining is forbidden) so Guard L reconstructs the SIGN engine-side: it tallies the signed directional evidence ALREADY in the result dict FOR the model\'s side (+1 favours / -1 opposes) across model lean, market_bias, macd_signal, supertrend, macro_daily, macro_weekly, rsi_slope and location (support favours BUY / resistance favours SELL), and requires net >= MIN_DIR_CONFLUENCE (2) to fire. These signals are correlated (several are trend proxies) so the net is a weighted lean not 8 independent votes, but it beats a direction-blind magnitude x coin flip. A REVERSAL is counter-trend by definition so a trend-confluence floor would kill it: reversals at a level (SELL into resistance while overbought / BUY into support while oversold) are EXEMPT and prove themselves through Guards J/B/K (mirrors the Guard F exemption) — this is the "buy the oversold dip at support in a downtrend" trade the user explicitly asked for. The net + per-signal breakdown is written to result[\'dir_confluence\'] for display (the visible "why this direction"). Measured live (28 fireable): 16 trend-aligned SELLs survive, 2 oversold-reversal BUYs at support fire via the exemption, 10 counter-everything longs blocked — two-sided and directionally coherent. NEVER overrides the model\'s side (only gates it); engine-side only (model/training/edge_engine untouched); uses existing fields; non-repainting. Stacks ON TOP of Guards D (model lean) / J (5m turn) / K (wrong-side at level) / F (HTF veto), which the user chose to keep. Does NOT fix the edge score itself [[project_edge_score_defects]] — it adds the direction the score discards) # v58 (REVIVED THE HTF DAILY/WEEKLY BIAS — it was dead code, not a missing feature. The engine ALREADY had the directional-focus logic the user wanted (score_signal HTF tiers: +15 both-aligned, +10 pullback/bounce, -20 both-opposing, -10 weekly-opposing; plus Guard F hard-block when weekly AND daily both oppose) — "bearish day => focus shorts, bullish day => focus longs". It was ENTIRELY DORMANT because macro_daily/macro_weekly arrived 0.0 on all 63 tokens, ALWAYS. Root cause (proven live): add_macro_regime_features (feature_engine.py:1706) pre-creates df["macro_trend_1d"]=0.0 as a default, then merge_asof at 1786 COLLIDES that column with the real computed value, pandas renames both to macro_trend_1d_x/_y, the very next line reads the plain name -> KeyError -> a bare except silently resets the trend to 0.0. Every call since inception. The same bug hits TRAINING (retrain_model uses the same prepare_features), so the models were trained with macro=0 too — a constant feature a tree model cannot split on, i.e. the model never used it either. FIX (engine-side per the constraint "never touch the model/training"): _daily_bias() recomputes the two flags from daily candles as pure GATE inputs — macro_daily=+1 if price>EMA50(daily) else -1 (medium trend); macro_weekly=+1 if price>EMA200(daily) else -1 (long trend). EMA50/EMA200 on daily closes map exactly onto the existing tier semantics (above the long EMA but below the medium one IS the pullback-buy). Injected into result before score_signal so BOTH the tiers and Guard F come alive; closed daily candles only (non-repainting); fails open to (0,0) = the dormant state the tiers already guard against, so a missing feed never fires a wrong direction. Guard F given a REVERSAL EXEMPTION (mirrors score_signal is_reversal): a SELL into resistance while overbought / a BUY into support while oversold — the user\'s exact "bull + at resistance + model sells => fire" case — is NOT hard-blocked; it proves itself through Guards J/B and is tagged RISKY. Measured live (18 fires): Guard F now hard-blocks the 2 counter-trend BUYs into a both-bearish HTF (GRT, ZK), 14 trend-aligned SELLs get +15/+10, weekly-only-opposing BUYs (AAVE, LDO) get -10 => RISKY not blocked. IMPACT VARIES WITH THE TAPE: today the market is broadly daily-bearish so most model shorts already align (the bias mainly culls counter-trend longs); the real value is structural — in a BULLISH tape nothing previously stopped shorts into the rally, which is the all-short failure mode. NOTE this does not touch the direction-blind edge score [[project_edge_score_defects]]; it adds the HTF directional context on TOP of Guards D/J/K) # v57 (GUARD K — ONLY IMPORTANT LEVELS, and a WRONG-SIDE VETO on them. The engine had TWO S/R systems and NEITHER could answer "is price at a level right now?" — which is the one question the whole structure gate depends on. (1) rolling_support/resistance, the levels the gate and range_position actually use, are just `high.shift(1).rolling(24).max()` — the 24-HOUR HIGH/LOW. Not a level: never tested, never bounced off, and it slides every hour. In any rally every token sits near its own 24h high BY CONSTRUCTION, so it read "AT RESISTANCE" on 79% of the fleet SIMULTANEOUSLY (49 of 62 tokens). A level that 79% of assets stand on carries no information — which is why the direction-blind edge score was left to pick sides on its own and fired 20 SELLs. (2) _swing_sr finds REAL pivots but EXCLUDES every level within 1 ATR of price (the `gap` filter), so it is mathematically incapable of reporting the level price is STANDING ON — it only ever returns the next level beyond a blind spot. Measured: 0 of 62 tokens were EVER "at" a swing level; the minimum distance was 1.00 ATR, by construction. NEW: _important_levels() — confirmed non-repainting pivots, CLUSTERED within LEVEL_MERGE_ATR (0.5) so the cluster size IS the touch count, keep only levels touched >= LEVEL_MIN_TOUCHES (2), then keep only the LEVEL_TOP_K (4) MOST-TOUCHED. That last cap is load-bearing: uncapped it returns 10-24 levels/token so price is always within 0.6 ATR of something and "at a level" fires on 88% of the fleet — noise again. Capped at the 4 strongest: ~4 levels/token and a realistic 27% "at a level". Measured sweep (60 tokens): topK=inf -> 24.3 lvls, 88% at-level, 11 fires kept / 19 vetoed; topK=6 -> 6.0, 32%, 22/8; topK=4 -> 4.0, 27%, 23/7. GUARD K then vetoes the WRONG SIDE AT a level: a BUY into a resistance price has repeatedly respected, or a SELL into such a support, is BLOCKED unless that level is already confirmed broken (resistance_broken_recent / support_broken_recent) — which preserves the dynamic breakout case: once resistance is taken out its role flips and a BUY through it fires. CRITICAL DESIGN POINT: this is a veto AT a level, NOT a requirement to BE at one. v25 and v38 BOTH made "at a level" mandatory to fire and BOTH collapsed the rate to zero, because real levels are far apart and price is usually BETWEEN them. Here mid-range fires untouched and only the wrong side at a level is blocked — 23 of 30 model fires survive. Tier: only a Guard-K survivor that is AT a level with the side agreeing can reach STRONG; a mid-range fire is legitimate but has no structural backing, so it caps at NORMAL however good the edge looks) # v56 (GUARD D REINSTATED — DIRECTIONAL CONVICTION FLOOR. Root cause of the all-SHORT book, found by auditing the SELECTOR instead of the gates. TWO structural defects, both measured on the live 62-token fleet. (1) THE EDGE SCORE IS DIRECTION-BLIND: EdgeScoringEngine.compute_signal_strength_scores() takes a `side` argument and NEVER USES IT — every component is an absolute magnitude (regime maps TRENDING_BULL->1.0 AND TRENDING_BEAR->1.0 identically; momentum = |rsi-50|/50; trend = |macro_trend_1d|, a feature that is always 0.0). So a SELL at RSI 90 scores a HIGHER edge than a SELL at RSI 55: measured, the ETH short at RSI 90 carried the 3rd-HIGHEST absolute edge of all 62 tokens. Conviction PEAKS exactly where the trade is most dangerous — the 12 losing shorts were not slipping past the score, they WERE its top picks. (2) THE FIRE THRESHOLD IS A QUANTILE, NOT A QUALITY BAR: edge_percentile = edge_rank.rank(pct=True) ranks the bar inside its own trailing window, so `edge_score >= 60` fires ~40-48% of bars in ANY tape — multiply the whole edge window by 0.01 and the fire rate does not move. The model is STRUCTURALLY INCAPABLE of saying "I have no edge right now" (measured live: 30/62 = 48% firing). Direction never enters `fire` at all, so the engine fired 20 SELLs on coin-flip conviction (LDO shorted on a 0.8pt p_buy-p_sell spread) while the model\'s genuinely strongest reads sat FLAT (HBAR -37pt, CRV +24pt). THE FIX CANNOT GO IN edge_engine.py: retrain_model.py and meta_gate_optimizer.py both import EdgeScoringEngine, so all 116 meta_threshold=60 values were calibrated against that exact function — rewriting it would silently invalidate every threshold, and retraining is forbidden. So the compensation is engine-side: Guard D asks the question `fire` never asks — does the model actually PREFER this side? conviction = (p_buy - p_sell)/(p_buy + p_sell), normalised so p_hold mass cannot distort the scale across models; require >= MIN_DIR_CONVICTION (0.15, i.e. beat a coin flip at 57.5/42.5). Replayed against the live fleet: removes ALL 10 of the shorts that lost, and turns a 10-BUY/20-SELL one-way book into a two-sided 6-BUY/2-SELL. NOTE an absolute floor on edge_rank was the obvious fix and is WRONG — defect (1) means it selects FOR the knife-catchers (ETH ranked #3 of 30 fires); it was measured and rejected. NOTE v51 shipped a conviction floor and v54 removed it as "blocking 100% of genuine fires", but that verdict came from a SIX-token sample with TWO fires; on the full fleet the floor keeps a healthy book. The gate was right, the measurement behind its removal was under-powered) # v55 (GUARD J — 5m MOMENTUM ALIGNMENT, universal. Fixes the 12-short pile-up (18 signals, 1W/8L, 11.1% WR, -$87.85, EVERY fired signal a SHORT) by never entering against a move that is still running. Root cause was NOT the HTF macro trend: I measured the real 1d/1w EMA trend for all 12 shorted tokens and 10 of 12 were bearish on BOTH — the shorts were HTF-trend-ALIGNED, and a working Guard F would have blocked exactly 1 of them (UNI). The actual defect: every one of those shorts entered at 1h RSI 76-86 — i.e. shorting INTO a hard rally — and the reversal confirmation (Guard B) only engaged when the regime was TRENDING_BULL, so the 8 shorts tagged VOLATILE_COMPRESSION / RANGING (incl. ETH at RSI 86) received NO turn-confirmation whatsoever. Guard J makes that confirmation UNIVERSAL and regime-independent: EVERY entry now requires >= ENTRY_5M_MIN (3) of the last ENTRY_5M_WINDOW (4) CLOSED 5m candles to have closed in the signal direction — a SELL needs the 5m already turning down, a BUY already turning up. It never changes the side and never discards the setup: it WAITs, and the setup fires on the next scan the moment the turn actually starts. Measured on the exact 12 bad shorts: Guard J blocks 11/12. Measured pass-rate across 24 tokens: BUY 25%, SELL 29% — a real filter, symmetric (no directional bias), and far from the zero-signal cliff, since it only multiplies the fire rate of tokens the MODEL already chose. Fails OPEN if the 5m feed is unavailable) # v54 (STRUCTURAL FIX of the zero-signal regression, measured not guessed. Funnel on live data showed: the MODEL itself only fires ~1/4 (edge_score >= its 60 percentile threshold) and BOTH of my v51 gates then killed the survivor. (1) Guard D REMOVED — a raw p_buy-p_sell conviction floor is architecturally wrong: edge_score is a PERCENTILE RANK of the model edge vs its own history, not the raw spread; these 3-class models sit near 50/50 on buy-vs-sell (p_hold holds the mass), so the only real model fires had 0.9pt (LDO) and 1.4pt (FIL) spreads while the NON-firing tokens carried 32-35pt. A 10pt floor blocked 100% of genuine fires. The model threshold IS the conviction check. (2) Guard E DOWNGRADED to advisory — UWGS score_hold has a structural floor so HOLD outscores the model side whenever location/regime gates are weak, making "HOLD > side" a permanent veto that re-installs UWGS as the decider (LDO: SELL 3.1 vs HOLD 62.8). It now tags RISKY instead of blocking. ARCHITECTURE: model decides side+fire; HARD blocks only for genuinely broken setups (drift/dead-market/extreme-vol/news, ATR floor, liquidity trap, HTF macro both-opposing, HTF no-room or exhaustion chase, unconfirmed counter-trend reversal, safe mode, portfolio); everything else DOWNGRADES to RISKY and never silences a signal) # v53 (HTF S/R advisory again: v52 made HTF (4h/1d) S/R a blanket HARD gate requiring >=2.5 ATR of room to the opposing level. Measured live that admits only 1/11 BUYs and 4/11 SELLs — 4h/1d swings are DENSE, the nearest opposing level is typically 1-2 ATR away — and stacked on the other gates it took the engine to ZERO signals, while v51 without it ran 83% WR on 9 signals. _htf_sr own docstring says these levels "do NOT gate firing (that stays on the 1h zone, so the rate holds)". Guard I is now the OLD Gate 1.8 shape: HARD-block ONLY (a) wall < HTF_NO_ROOM_ATR (1.0) ATR — even TP1 (0.55 ATR) unreachable — or (b) wall < 2.5 ATR AND RSI extreme (>72 BUY / <28 SELL) = the exhaustion chase; otherwise a nearby wall (< 2.5 ATR) FIRES tagged RISKY (htf_wall_ahead) so the rate holds) # v52 (HTF hard gate — REGRESSION, reverted) # v51 (RESTORE the protective gates the model-first rewrite dropped, as VETOES on the model fire (they never change the side): Guard C no-trade regime (LIQUIDITY_TRAP — SAND/HBAR fired inside one), Guard D model conviction floor (p_buy-p_sell >= 10pt — FIL fired a LONG on 50.7/49.3, a coin flip), Guard E composite-HOLD veto (UWGS HOLD beat the model side: FIL HOLD 49.3 > BUY 40.1 with trend 8%/momentum 16%/MACD bearish — the old engine held these), Guard F HTF macro veto, Guard G safe mode, Guard H portfolio guard (7 positions had opened with no concurrency cap)) # v50 (fix the wide-entry contradiction: the 3 confirmation candles push price OFF the level, so the TAG is now measured on 5m LOWS/HIGHS over the last 6 closed candles — did price REACH support/resistance — and the entry is separately CAPPED at REVERSAL_MAX_CHASE_ATR (1.2) ATR from the level so the fill is never way-too-wide. A counter-trend BUY(bear)/SELL(bull) fires only when: price TAGGED the level (low/high within 0.5 ATR) AND is still within 1.2 ATR of it AND RSI oversold/overbought AND >=3 of last 4 closed 5m candles turned; else WAIT. Tagged RISKY) # v49 (counter-trend reversal must WAIT for the level + RSI + 3x 5m confirmation) # v48 (reversal-aware structural guards: ATR floor 0.5%; counter-trend reversal needs level+RSI) # v47 (ML MODEL decides side+fire again; UWGS demoted to chart-breakdown + risk-tier + 4 hard vetoes only; FAR_FROM_SR/NO_VALID_SR + mid-range => RISKY tier NOT a block; hard vetoes = DRIFT/DEAD_MARKET/EXTREME_VOL/NEWS; MODEL_DISAGREES deleted) # prev: structure-gate-v46 (v45 + REJECTION FAST-PATH: once price has TAGGED a level and moved back >STRUCT_REJECTION_PCT (10%) of the range off it — a SELL now >10% BELOW the resistance it hit, a BUY >10% ABOVE the support it hit — the reversal FIRES immediately. The rejection is the confirmation, so it bypasses the counter-trend 2-candle wait and the far-from-level pending. NOTE: direction still comes from the ML model — this fires a model SELL/BUY faster on a confirmed rejection, it does not create a SELL against a bullish model) # v45 (v44 + HTF-WALL EXHAUSTION BLOCK: Gate 1.8 now HARD-blocks a BUY buying into a nearby 4h/1d resistance (<2.5 ATR overhead) while RSI > 72 overbought — and the mirror SELL into a nearby 4h/1d support while RSI < 28 oversold. That is the exhaustion chase at the top/bottom of an extended move whose first target sits beyond the HTF wall (AAVE LONG, RSI 89, TP1 above 4H/D resistance). Mild overhead/below without an RSI extreme stays advisory) # v44 (v43 + COUNTER-TREND NEEDS 2 CANDLES: a counter-trend reversal at the level (SELL in an ADX-trending bull / BUY in a bear) no longer fires unconfirmed — it WAITS (pending) until >=2 of the 5m candles have turned in the signal direction, so we do not short a still-rising move / buy a still-falling one (the UNI 3.778 case). Aligned + range reversals still fire immediately at the level) # v43 (v42 + SMART STRICT (higher WR, count preserved via pending): the 3 WEAKEST fires now WAIT/pending instead of firing — reversal FAR from the level, mid-range with NO 5m momentum, and an UNCONFIRMED wrong-side breakout (no retest/momentum/pre-break). They fire later when price reaches the level or the move starts. Kept firing: at/near-level reversals (even unconfirmed), momentum mid-range, confirmed breakouts (retest/momentum/pre-break). Config: edge floor 55->60, ATR floor 0.4->0.5pct, advisory budget non-binding->4) # v42 (v41 + AGGRESSIVE FIRE-AT-S/R: reversals fire IMMEDIATELY at the level (confirmation is a BONUS, not required); MID-RANGE fires with a wider stop instead of pending; wrong-side breakouts fire on retest/momentum/pre-break; zones 0.20/0.80->0.35/0.65; proximity 0.9->1.5 ATR; advisory budget non-binding; edge floor 70->55; ATR floor 0.8->0.4pct; cooldowns 30m/1h->5m/10m; _swing_sr nearer k; loose entries get 2.2-2.5 ATR SL cap. TRADEOFF: max signal capture, more false positives / lower win-rate by design) # v41 (v40 + HTF S/R CONFLUENCE (Gate 1.8): pool the nearest 4h+1d swing S/R (_htf_sr) and, as an ADVISORY, downgrade a BUY with a major HTF resistance right overhead (<1 ATR room) or a SELL with major HTF support right below — trade WITH the big structure, not into it. Firing still gated on the 1h zone (rate unchanged); HTF only warns/tags. The 4h/1d levels are drawn on the chart as purple 4H/D lines) # v40 (v39 + REVERT v38 swing-S/R zone: the gate classifies location on the 24h rolling range_position again (that FIRES) — gating on the far-apart swing S/R put nearly everything mid-range/PENDING and collapsed the rate (v25 regression; pending only delays, does not rescue). Kept: v39 strict direction (sell@resistance/buy@support, wrong-side -> pending), v37 ATR+structure hybrid SL/TP, v36 zones 0.20/0.80. Swing S/R stays the chart display only) # v39 (v38 + STRICT DIRECTION via PENDING: a SELL fires ONLY at resistance, a BUY ONLY at support. Wrong-side signals (SELL at support / BUY at resistance — the "sold at support late" breakdown, e.g. HBAR) no longer fire the breakout; they are held PENDING and wait for the correct level. Genuine role-flips still fire via correct_level (a broken resistance that holds becomes support below price). The old breakout_level branch is now unreachable) # v38 (v37 + GATE FIRES ON SWING S/R: location is now classified against the major swing S/R (_swing_sr, the chart levels), not the 24h rolling range_position — so entries sit at the visible support/resistance instead of the rolling extreme that read mid-range. This was the v25 collapse basis, but v35 PENDING now holds the mid-range ones (waits for the level) instead of blocking, so the rate does not zero. Swing levels written back to result so SL/TP + display share them. Falls back to rolling when pivots sparse) # v37 (v36 + ATR+STRUCTURE HYBRID SL/TP: SL anchored just beyond the invalidation level (support for a LONG / resistance for a SHORT) + 0.5 ATR buffer, clamped to [0.7,1.8] ATR — tighter near-S/R entries => higher RR. TP ladder: TP1=1R, TP2=2R, TP3=structural target (liquidity), TP4=1.618 fib measured move, TP5=2.618 (trailing runner). Partial split 15/25/25/15/20. RR validated to the REAL structural target (cramped setups rejected). Position size scales inverse to the risk leg, bounded [0.5x,2x] and re-capped) # v36 (v35 + STRICT S/R LOCATION: zone tightened 0.35/0.65 -> 0.20/0.80 so a BUY fires only in the bottom 20% (near support) and a SELL only in the top 20% (near resistance); anything looser waits PENDING for a real S/R touch) # v35 (v34 + MID-RANGE HELD AS PENDING: a mid-range signal is no longer BLOCKed/discarded — the structure gate returns WAIT and the engine holds it (pending_side/pending_target) until price reaches its level (resistance for a SELL, support for a BUY), then the 5m 3/4 + 15m 2/2 confirmation fires it. Other BLOCKs unchanged) # v34: (RESTORE v10 gate behaviour: removed the _swing_sr repoint (v29-v33) and the STRICT-direction block (v30) from the structure gate. Location is read straight off the predictor 24h rolling S/R + range_position zones (0.35/0.65); reversals AND confirmed breakouts both fire again. This is the v10-era logic the user confirmed "was doing it great" — the swing-anchor machinery is what caused the no-signals / mid-range-phantom / entry-gap regressions. _swing_sr kept for the open-position chart display only)'  # v33: (swing no longer opens zone) # v32: (k+1) # v31: (cache-key+limit) # v30: (strict direction) # v29: (swing repoint)'  # v32: (k+1 significant swing) # v31: (v30 + cache key includes limit; gate/chart both k+3 @1500)'  # v30: (v29 + deep 1500x1h S/R scan, nearest+extreme major per side, STRICT direction: sell only at resistance / buy only at support)'  # v29: (v28 + entries land AT the visible swing S/R via always-on repoint; fine prox back to 0.9 ATR) # v28: (S/R zone as % of range) # v27: (fire at big-swing S/R on mid-range, additive; chart gap filter)'  # v28: (v27 + S/R zone as % of range) # v27: (v26 + fire at big-swing S/R on mid-range block, additive; chart _sr_levels gap filter)'  # v27: (v26 + fire at BIG-SWING S/R on a would-be mid-range BLOCK via _swing_sr, purely additive; chart _sr_levels gap filter drops price-hugging noise levels)'  # v26: (v25 + restore S/R reversal firing: proximity 0.5->0.9 ATR, counter-trend RSI/confirmation gate is ADVISORY unless BOTH fail; reversals are primary, breakouts secondary)' # v25: (v24 + FIRING FIX: zone/range_pos uses the predictor 24h range_position again, not the wide swing S/R that made everything mid-range and zeroed the fire rate)' # v24: (v23 + STALL FIX: synchronous Firestore push moved off the event loop + circuit-breaker; sr_levels only for fired/open symbols to cut scan load)' # v23: (v22 + open positions show ENTRY edge/confidence, not the decayed live re-score — fixes Gate 3 rendering red on a healthy fired signal)' # v22: (v21 + declutter chart S/R: MAJOR swings only, congestion merged, max 2 levels per side)' # v21: (v20 + fully per-token dynamics: S/R zone width from each token ATR, pivot k sized to token volatility, reversal RSI thresholds from each token own RSI distribution)' # v20: (v19 + per-level state machine (NORMAL/PENDING/WAITING_RETEST/CONFIRMED/FAILED) exposed as sr_levels for the chart, + breakout-quality confidence: weak (wick/low-vol) breaks downgraded)' # v19: (v18 + Break->Retest->Confirmation: a broken level does NOT flip role until a retest holds; unconfirmed sweeps keep the original S/R)' # v18: (v17 + significant S/R: k=5 swing pivots + >=1 ATR gap from price, so levels land at real reversals not micro-wiggles next to price)' # v17: (v16 + LIVE role-reversed S/R for open positions: chart S/R updates as price moves instead of freezing the entry snapshot)' # v16: (v15 + S/R role reversal: every pivot is bidirectional, a broken resistance flips to support and vice versa; nearest pivot above=resistance, below=support)' # v15: (v14 + swing-based S/R: nearest confirmed 1h swing low/high around price instead of the crude 24h high/low)' # v14: (v13 + breakout-continuation must break the CURRENT support/resistance, never enter INTO the level: no more sell-at-support / buy-at-resistance)' # v13: (v12 + counter-trend reversal must prove the turn: no CONFLICT + RSI extreme; reversal proximity tightened 0.9->0.5 ATR)' # was: 'structure-gate-v12 (v11 + confirmed breakout-continuation: follow a break that runs and never retests when held 3+ closed 5m bars beyond + 5m/15m trend + not overextended) + reversal as-close-as-possible + Firestore-durable track record'
            ...
            if (not existing or _reversal_flip) and result.get('fire') \
                ...
                if _loss_ago < self.LOSS_COOLDOWN_SECONDS:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        # Target = the NEAREST significant S/R from DEEP MULTI-TF
                        # history (1h+4h+1d), not the most-touched far old high that
                        # _important_levels returns. This is what fixes the "pending
                        # resistance is way too high" case.
                        _atr_m  = float(result.get('atr', 0) or 0)
                        _lvls_m: List[Tuple[float, int]] = []
                        try:
                            _lvls_m = await self._structural_levels(symbol, price, _atr_m)
                        except Exception:
                            _lvls_m = []
                        if new_side == 'BUY':
                            _target_m = max([l for l, _t in _lvls_m if l < price], default=None)
                        ...
                        # the level rather than toward it) — if so, the reversal has
                        # begun; fire now instead of waiting for a second touch.
                        _came_from_m = False
                        if _target_m is not None and not _at_level_m:
                            try:
                                ...
                                _tol = self.PENDING_TAG_TOL_PCT / 100.0
