...
class LiveEngine:
    ...
    async def _swing_sr(self, symbol: str, price: float,
        ...
        try:
            if price <= 0:
                return None
            # SAME depth AND SAME swing window (_dynamic_k + 3) as the chart's
            # _sr_levels, so the gate fires against the EXACT major level drawn on
            # the chart. With k (not k+3) the gate found a nearer MINOR high the
            # chart never draws and shorted well below the real resistance (the
            # "gap between resistance and entry" bug). Shared cache key means this
            # 1500 fetch is reused with the chart's, not refetched.
            ...
            k = self._dynamic_k(atr, price) + 3   # MAJOR swings only — match the chart
