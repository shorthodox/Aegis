            ...
            if (not existing or _reversal_flip) and result.get('fire') \
                ...
                if _reversal_flip or cooldown_elapsed >= required_cooldown:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        # counter-trend REVERSAL. It is a genuine call ONLY when it
                        # is BOTH at the structural extreme (BUY at support / SELL
                        # at resistance) AND at an RSI extreme (oversold / overbought)
                        # — otherwise it is the naive falling-knife the user flagged.
                        ...
                        if _ct_buy or _ct_sell:
                            # Location (self-computed range_position fallback)
                            ...
                            if 0 < _sup < _res and price > 0:
                                ...
                                _loc_rp = _rp_now
                            _at_level = ((_ct_buy  and _loc_rp <= self.STRUCT_SUPPORT_ZONE) or
                                         (_ct_sell and _loc_rp >= self.STRUCT_RESISTANCE_ZONE))
                            # RSI extreme (per-token dynamic bounds, fallback 42/58)
                            ...
                            if not (_at_level and _rsi_extreme):
                                _lim = _rsi_lo if _ct_buy else _rsi_hi
                                _reason = ('not at ' + ('support' if _ct_buy else 'resistance')
                                           + f' (rp={_loc_rp:.2f})') if not _at_level else \
                                          (f'RSI {_rsi_now:.0f} not '
                                           + ('oversold' if _ct_buy else 'overbought')
                                           + f' ({"<=" if _ct_buy else ">="}{_lim:.0f})')
                                print(f'[{symbol}] MODEL BLOCK {new_side}: counter-trend in '
                                      f'{_reg} — {_reason} (not a genuine reversal)')
                                if symbol in self.last_signals:
                                    self.last_signals[symbol]['fire']            = False
                                    self.last_signals[symbol]['signal']          = 'HOLD'
                                    self.last_signals[symbol]['regime_blocked']  = True
                                    self.last_signals[symbol]['structure_reason'] = (
                                        f'counter-trend {new_side} in {_reg}: {_reason}')
                                self.bootstrap_done = min(self.bootstrap_done + 1, self.bootstrap_total)
                                return
