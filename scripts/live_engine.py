...
class LiveEngine:
    ...
    # so it does not fire ~2 ATR above support like the AAVE LONG @94.66 vs
    ...
    REVERSAL_PROX_ATR  = 0.5   # "at the level" = within half an ATR of it
    REVERSAL_5M_WINDOW = 4     # look back this many closed 5m candles
                ...
                if _reversal_flip or cooldown_elapsed >= required_cooldown:
                    ...
                    if USE_WEIGHTED_SCORER:
                        ...
                        if _ct_buy or _ct_sell:
                            ...
                            # (1) Reached the level — tight ATR proximity to the REAL
                            #     support/resistance; falls back to the range-position
                            #     zone only when S/R or ATR is unavailable.
                            if _atr_abs > 0 and ((_sup > 0) if _ct_buy else (_res > 0)):
                                _dist = (price - _sup) if _ct_buy else (_res - price)
                                _reached  = _dist <= self.REVERSAL_PROX_ATR * _atr_abs
                                _loc_desc = (f'{_dist / _atr_abs:.2f} ATR from '
                                ...
                                if 0 < _sup < _res and price > 0:
                                    ...
                                    _loc_rp = _rp_now
                                _reached = ((_loc_rp <= self.STRUCT_SUPPORT_ZONE) if _ct_buy
                                            else (_loc_rp >= self.STRUCT_RESISTANCE_ZONE))
                                _loc_desc = f'rp={_loc_rp:.2f}'

                            # (2) RSI extreme (per-token dynamic bounds, fallback 42/58)
                            ...
                            # (3) 5m reversal confirmation — ≥3 of the last 4 closed
                            #     5m candles turned in the signal direction.
                            _n5 = -1   # -1 = candle feed unavailable → don't hard-block on it
                            try:
                                _c5 = await self._fetch_candles(
                                    symbol, '5m', self.REVERSAL_5M_WINDOW + 2)
                                _c5 = _c5[:-1] if len(_c5) >= 2 else _c5   # closed only
                                _last5 = _c5[-self.REVERSAL_5M_WINDOW:]
                            ...
                            if not (_reached and _rsi_extreme and _confirmed_5m):
                                _lvl = 'support' if _ct_buy else 'resistance'
                                if not _reached:
                                    _why = f'waiting for {_lvl} ({_loc_desc})'
